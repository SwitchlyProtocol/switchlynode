...
// TxArrayItem used to represent the tx out item coming from SWITCHLYChain, there is little difference between TxArrayItem
// and TxOutItem defined above , only Coin <-> Coins field are different.
// TxArrayItem from SWITCHLYChain has Coin , which only have a single coin
