...
func NewPoolMgr(bridge SwitchlyBridge) *PoolMgr {
	return &PoolMgr{
		bridge: bridge,
...
func (pm *PoolMgr) getPool(asset common.Asset) stypes.Pool {
	...
	if duration > constants.SwitchlyBlockTime {
		pm.updatePool()
...
func (pm *PoolMgr) GetValue(source, target common.Asset, amount cosmos.Uint) (cosmos.Uint, error) {
	...
	runeValue := sourcePool.AssetValueInSWITCH(amount)
	destPool := pm.getPool(target)
	if destPool.IsEmpty() {
		return cosmos.ZeroUint(), fmt.Errorf("pool:%s doesn't exist", destPool)
	}
	return destPool.SWITCHValueInAsset(runeValue), nil
}
