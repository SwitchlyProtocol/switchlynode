...
// Broadcast Broadcasts tx to switchly
func (b *switchlyBridge) Broadcast(msgs ...stypes.Msg) (common.TxID, error) {
	...
	start := time.Now()
	defer func() {
		b.m.GetHistograms(metrics.SendToSwitchlyDuration).Observe(time.Since(start).Seconds())
	}()
	...
	if blockHeight > b.blockHeight {
		...
		if err != nil {
			return noTxID, fmt.Errorf("fail to get account number and sequence number from switchly : %w", err)
		}
	...
	flags := flag.NewFlagSet("switchly", 0)
	...
	b.m.GetCounter(metrics.TxToSwitchlySigned).Inc()
	...
	if commit.Code > 0 {
		...
		// commit code 6 means `unknown request` , which means the tx can't be accepted by switchly
		// if that's the case, let's just ignore it and move on
		if commit.Code != 6 {
			return txHash, fmt.Errorf("fail to broadcast to SWITCHLYChain,code:%d, log:%s", commit.Code, commit.RawLog)
		}
	}
	b.m.GetCounter(metrics.TxToSwitchly).Inc()
	b.logger.Info().Msgf("Received a TxHash of %v from the switchly", commit.TxHash)
