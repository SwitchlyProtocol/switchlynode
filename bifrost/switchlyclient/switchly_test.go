...
func (s *SwitchlySuite) SetUpTest(c *C) {
	...
	s.server = httptest.NewServer(http.HandlerFunc(func(rw http.ResponseWriter, req *http.Request) {
		...
		case strings.HasPrefix(req.RequestURI, "/switchly/vaults") && strings.HasSuffix(req.RequestURI, "/signers"):
		...
		case strings.HasPrefix(req.RequestURI, SwitchlyConstants):
		...
		case strings.HasPrefix(req.RequestURI, "/switchly/switchlyname/"):
			httpTestHandler(c, rw, "../../test/fixtures/endpoints/switchlyname/switchlyname.json")
		}
	}))
	...
	var err error
	bridge, err := NewSwitchlyBridge(s.cfg, GetMetricForTest(c), NewKeysWithKeybase(kb, cfg.SignerName, cfg.SignerPasswd))
	var ok bool
	s.bridge, ok = bridge.(*switchlyBridge)
...
func (s *SwitchlySuite) TearDownTest(c *C) {
	s.server.Close()
}

func (s *SwitchlySuite) TestGetThorChainURL(c *C) {
...
func (s *SwitchlySuite) TestGet(c *C) {
...
func (s *SwitchlySuite) TestGetObservationsStdTx_OutboundShouldHaveConfirmationCounting(c *C) {
...
func (s *SwitchlySuite) TestSign(c *C) {
	pk := stypes.GetRandomPubKey()
	vaultAddr, err := pk.GetAddress(common.ETHChain)
	c.Assert(err, IsNil)
	tx := common.NewObservedTx(
...
func (SwitchlySuite) TestNewSwitchlyBridge(c *C) {
	testFunc := func(cfg config.BifrostClientConfiguration, errChecker, sbChecker Checker) {
		...
		c.Assert(err, IsNil)
		sb, err := NewSwitchlyBridge(cfg, m, NewKeysWithKeybase(kb, cfg.SignerName, cfg.SignerPasswd))
...
func (s *SwitchlySuite) TestGetAccountNumberAndSequenceNumber_Success(c *C) {
...
func (s *SwitchlySuite) TestGetAccountNumberAndSequenceNumber_Fail(c *C) {
	s.nodeAccountFixture = "../../test/fixtures/endpoints/nodeaccount/template.json"
...
func (s *SwitchlySuite) TestGetAccountNumberAndSequenceNumber_Fail_500(c *C) {
	s.nodeAccountFixture = "../../test/fixtures/endpoints/nodeaccount/template.json"
...
func (s *SwitchlySuite) TestGetAccountNumberAndSequenceNumber_Fail_Unmarshal(c *C) {
	s.nodeAccountFixture = "../../test/fixtures/endpoints/nodeaccount/template.json"
...
func (s *SwitchlySuite) TestEnsureNodeWhitelisted_Success(c *C) {
	s.authAccountFixture = "../../test/fixtures/endpoints/auth/accounts/template.json"
	s.nodeAccountFixture = "../../test/fixtures/endpoints/nodeaccount/template.json"
...
func (s *SwitchlySuite) TestEnsureNodeWhitelisted_Fail(c *C) {
	s.authAccountFixture = "../../test/fixtures/endpoints/auth/accounts/template.json"
...
func (s *SwitchlySuite) TestGetKeysignParty(c *C) {
...
func (s *SwitchlySuite) TestIsCatchingUp(c *C) {
	ok, err := s.bridge.IsCatchingUp()
	c.Assert(err, IsNil)
	c.Assert(ok, Equals, false)
}

func (s *SwitchlySuite) TestGetAsgards(c *C) {
	vaults, err := s.bridge.GetAsgards()
	c.Assert(err, IsNil)
	c.Assert(vaults, NotNil)
}

func (s *SwitchlySuite) TestGetPubKeys(c *C) {
	pks, err := s.bridge.GetPubKeys()
	c.Assert(err, IsNil)
	c.Assert(pks, HasLen, 4)
}

func (s *SwitchlySuite) TestPostNetworkFee(c *C) {
	s.authAccountFixture = "../../test/fixtures/endpoints/auth/accounts/template.json"
...
func (s *SwitchlySuite) TestGetConstants(c *C) {
	result, err := s.bridge.GetConstants()
	c.Assert(err, IsNil)
	c.Assert(result, NotNil)
}

func (s *SwitchlySuite) TestGetRagnarok(c *C) {
	result, err := s.bridge.RagnarokInProgress()
	c.Assert(err, IsNil)
	c.Assert(result, Equals, false)
}

func (s *SwitchlySuite) TestGetSwitchlyVersion(c *C) {
	result, err := s.bridge.GetSwitchlyVersion()
	c.Assert(err, IsNil)
	c.Assert(result.EQ(stypes.GetCurrentVersion()), Equals, true)
}

func (s *SwitchlySuite) TestGetMimir(c *C) {
	result, err := s.bridge.GetMimir("HaltETHChain")
	c.Assert(err, IsNil)
	c.Assert(result, Equals, int64(10))
}

func (s *SwitchlySuite) TestGetContractAddress(c *C) {
...
func (s *SwitchlySuite) TestSWITCHName(c *C) {
	result, err := s.bridge.GetSWITCHName("test1")
	c.Assert(err, IsNil)
