...
func (s *KeygenSuite) SetUpSuite(c *C) {
	...
	s.cfg, _, s.kb = SetupSwitchlyForTest(c)
	s.cfg.ChainHost = s.server.Listener.Addr().String()
	var err error
	bridge, err := NewSwitchlyBridge(s.cfg, GetMetricForTest(c), NewKeysWithKeybase(s.kb, s.cfg.SignerName, s.cfg.SignerPasswd))
	var ok bool
...
func (s *KeygenSuite) TestGetKeygen(c *C) {
	...
	// members := []string{
	// 	"tswitchpub1addwnpepq2kdyjkm6y9aa3kxl8wfaverka6pvkek2ygrmhx6sj3ec6h0fegws6rtmr8q",
	// 	"tswitchpub1addwnpepqt7qug8vk9r3saw8n4r803ydj2g3dqwx0mvq5akhnze86fc536xcyc6nxl76",
	// }
	...
	// c.Assert(err, IsNil)
	// sig, _, err := s.kb.Sign("switchly", buf)
	// c.Assert(err, IsNil)
	// fmt.Printf("Sig: %s\n", base64.StdEncoding.EncodeToString(sig))
	// fmt.Printf("KEYGEN1: %+v\n", block)

	pk := types.GetRandomPubKey()
	expectedKey, err := common.NewPubKey("tswitchpub1addwnpepq2kdyjkm6y9aa3kxl8wfaverka6pvkek2ygrmhx6sj3ec6h0fegws6rtmr8q")
	c.Assert(err, IsNil)
