...
func (s *BitcoinCashSignerSuite) SetUpTest(c *C) {
	...
	s.server = httptest.NewServer(http.HandlerFunc(func(rw http.ResponseWriter, req *http.Request) {
		if req.RequestURI == "/switchly/vaults/swtcpub1addwnpepq06smgna9nln5432hudgaelwz67w8nygk3d69dhza8awt7zegcauv4qrdku/signers" { // nolint
			_, err := rw.Write([]byte("[]"))
			c.Assert(err, IsNil)
...
func (s *BitcoinCashSignerSuite) TestSignTxHappyPathWithPrivateKey(c *C) {
	...
	txOutItem := stypes.TxOutItem{
		...
		ToAddress:   addr,
		VaultPubKey: "swtcpub1addwnpepq06smgna9nln5432hudgaelwz67w8nygk3d69dhza8awt7zegcauv4qrdku",
		Coins: common.Coins{
