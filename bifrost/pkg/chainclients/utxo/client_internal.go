...
func (c *Client) sendNetworkFee(height int64) error {
	...
	c.log.Debug().Msg("send network fee to SWITCHLYNode successfully")
	return nil
}

// sendNetworkFeeFromBlock will send network fee to Switchlynode based on the block result,
...
func (c *Client) getVinZeroTxs(block *btcjson.GetBlockVerboseTxResult) (map[string]*btcjson.TxRawResult, error) {
	...
	for i := range block.Tx {
		...
		// skip if sum of vout value is under switchly dust threshold
...
func (c *Client) ignoreTx(tx *btcjson.TxRawResult, height int64) bool {
	...
	// there are more than ten outputs with value in them, not SWITCHLYChain format
...
func (c *Client) getOutput(sender string, tx *btcjson.TxRawResult, consolidate bool) (btcjson.Vout, error) {
	...
	for _, vout := range tx.Vout {
		...
		// To be observed, either the sender or receiver must be an observed SWITCHLYChain vault;
