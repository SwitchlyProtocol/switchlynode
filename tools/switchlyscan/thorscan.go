...
func (t *Transport) RoundTrip(req *http.Request) (*http.Response, error) {
	req.Header.Set("X-Client-ID", "switchlyscan-go")
...
var (
	Parallelism = 4
	APIEndpoint = "https://switchlynode-archive.ninerealms.com"
		...
		go func(i int) {
			for height := range queue {
				for {
					...
					near := time.Now().Add(-constants.SwitchlyBlockTime * time.Duration(Parallelism))
