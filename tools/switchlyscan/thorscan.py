...
API_ENDPOINT = os.getenv("API_ENDPOINT", "https://switchlynode-v2.ninerealms.com")
...
def _get(*args, **kwargs) -> requests.Response:
    ...
    kwargs["headers"]["x-client-id"] = "switchlyscan"
...
def _fetch_block(height: int) -> Optional[Dict[str, Any]]:
    while True:
        if stop_scan:
            return None
        res = _get(f"{API_ENDPOINT}/switchly/block", params={"height": height})
