...
func (s SwapQueueVCURSuite) TestScoreMsgs(c *C) {
	...
	msgs := []*MsgSwap{
		...
		}, common.SwitchAsset(), GetRandomGAIAAddress(), cosmos.ZeroUint(), common.NoAddress, cosmos.ZeroUint(),
			"", "", nil,
			MarketSwap,
			0, 0, GetRandomBech32Addr()),
		NewMsgSwap(common.Tx{
...
func (s SwapQueueVCURSuite) TestStreamingSwapSelection(c *C) {
	...
	tx := common.NewTx(
		...
		ethAddr,
		ethAddr,
		common.NewCoins(common.NewCoin(common.SwitchAsset(), cosmos.NewUint(common.One*100))),
