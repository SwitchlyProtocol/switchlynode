...
func getMaxSwapQuantityV3_0_0(ctx cosmos.Context, mgr Manager, sourceAsset, targetAsset common.Asset, swp StreamingSwap) (uint64, error) {
	...
	for i, asset := range []common.Asset{sourceAsset, targetAsset} {
		...
		minRuneSwapSize := common.GetSafeShare(minSlip, cosmos.NewUint(constants.MaxBasisPts), pool.BalanceSwitch)
	...
	for _, asset := range []common.Asset{sourceAsset, targetAsset} {
		...
		newDbps := common.GetUncappedShare(dpool.BalanceSwitch, runeDepth, cosmos.NewUint(constants.MaxBasisPts))
...
func emitEndBlockTelemetry(ctx cosmos.Context, mgr Manager) error {
	...
	telemetry.SetGauge(runeUSDPrice, "thornode", "price", "usd", "switchly", "rune")
	...
	for _, pool := range pools {
		...
		telemetry.SetGaugeWithLabels([]string{"thornode", "pool", "balance", "rune"}, telem(pool.BalanceSwitch), labels)
		telemetry.SetGaugeWithLabels([]string{"thornode", "pool", "balance", "asset"}, telem(pool.BalanceAsset), labels)
		telemetry.SetGaugeWithLabels([]string{"thornode", "pool", "pending", "rune"}, telem(pool.PendingInboundSwitch), labels)
		...
		price := float32(0)
		if !pool.BalanceAsset.IsZero() {
			price = runeUSDPrice * telem(pool.BalanceSwitch) / telem(pool.BalanceAsset)
		}
...
func getAvailablePoolsRune(ctx cosmos.Context, keeper keeper.Keeper) (Pools, cosmos.Uint, error) {
	...
	for ; iterator.Valid(); iterator.Next() {
		...
		availablePoolsRune = availablePoolsRune.Add(pool.BalanceSwitch)
...
func reserveExitRUNEPool(ctx cosmos.Context, mgr Manager) error {
	...
	reserveExitCoins := common.NewCoins(common.NewCoin(common.SwitchNative, reserveExitRune))
...
func reserveEnterRUNEPool(ctx cosmos.Context, mgr Manager, rune cosmos.Uint) error {
	...
	coins := common.NewCoins(common.NewCoin(common.SwitchNative, rune))
