...
func (s TxOutStoreVCURSuite) TestAddOutTxItem(c *C) {
	...
	outboundFeeWithheldSwitch, err := w.keeper.GetOutboundFeeWithheldSwitch(w.ctx, common.ETHAsset)
	c.Assert(err, IsNil)
	outboundFeeSpentSwitch, err := w.keeper.GetOutboundFeeSpentSwitch(w.ctx, common.ETHAsset)
	c.Assert(err, IsNil)
	c.Check(outboundFeeWithheldSwitch.String(), Equals, "0")
	c.Check(outboundFeeSpentSwitch.String(), Equals, "0")
	...
	outboundFeeWithheldSwitch, err = w.keeper.GetOutboundFeeWithheldSwitch(w.ctx, common.DOGEAsset)
	c.Assert(err, IsNil)
	outboundFeeSpentSwitch, err = w.keeper.GetOutboundFeeSpentSwitch(w.ctx, common.DOGEAsset)
	c.Assert(err, IsNil)
	c.Check(outboundFeeWithheldSwitch.String(), Equals, "74999") // After slippage the 75000 fee is 74999 in SWITCH
	c.Check(outboundFeeSpentSwitch.String(), Equals, "0")
	...
	// Outbound fee withheld SWITCH should be updated
	outboundFeeWithheldSwitch, err = w.keeper.GetOutboundFeeWithheldSwitch(w.ctx, common.DOGEAsset)
	c.Assert(err, IsNil)
	c.Assert(outboundFeeWithheldSwitch.String(), Equals, "149997")
	...
	outboundFeeWithheldSwitch, err = w.keeper.GetOutboundFeeWithheldSwitch(w.ctx, common.DOGEAsset)
	c.Assert(err, IsNil)
	c.Assert(outboundFeeWithheldSwitch.String(), Equals, "224994")
	...
	testAndCheckModuleBalances(c, w.ctx, w.keeper,
		func() {
			item = TxOutItem{
				...
				ToAddress: GetRandomSwitchAddress(),
				InHash:    inTxID,
...
func (s TxOutStoreVCURSuite) TestAddOutTxItem_OutboundHeightDoesNotGetOverride(c *C) {
	...
	item.ToAddress = GetRandomSWITCHLYAddress()
	ok, err = txOutStore.TryAddTxOutItem(w.ctx, w.mgr, item, cosmos.ZeroUint())
	c.Assert(err, IsNil)
	c.Assert(ok, Equals, true)
...
func (s TxOutStoreVCURSuite) TestCalcTxOutHeight(c *C) {
	...
	// With the above values, a SWITCH value of 18,000 would be delayed for the full MaxTxOutOffset.
	...
	value := pool.AssetValueInSWITCH(toi.Coin.Amount)
	...
	normalGasValue := pool.AssetValueInSWITCH(normalGasTOI.MaxGas[0].Amount)
	...
	millionGasValue := pool.AssetValueInSWITCH(millionGasTOI.MaxGas[0].Amount)
	...
	thousandSizeTOIValue := pool.AssetValueInSWITCH(thousandSizeTOI.Coin.Amount)
...
func (s TxOutStoreVCURSuite) TestAddOutTxItemInteractionWithPool(c *C) {
	...
	c.Assert(err, IsNil)
	// Let:
	//   R_0 := the initial pool SWITCH balance
...
func (s TxOutStoreVCURSuite) TestAddOutTxItem_SecurityVersusOutboundNumber(c *C) {
	...
	item := TxOutItem{
		...
		// This Coin amount is an estimate, given slight changes to pool SWITCH amount in a block.
	}
...
func (s *TxOutStoreVCURSuite) TestSplitCloutEqualDistribution(c *check.C) {
	...
	switchValue := cosmos.NewUint(60)

	// Call function under test
	amountFromClout1, amountFromClout2, remainingSWITCH := tos.splitClout(ctx, swapperCloutLimit, clout1, clout2, switchValue)

	// Assert that the clouts were split equally and no switch value remains
	...
	c.Check(remainingSWITCH.IsZero(), check.Equals, true)
}
...
func (s *TxOutStoreVCURSuite) TestSplitCloutOneSideExceeds(c *check.C) {
	...
	c.Check(remainingSWITCH.Equal(cosmos.NewUint(0)), check.Equals, true)
}
...
func (s *TxOutStoreVCURSuite) TestSplitCloutOverLimit(c *check.C) {
	...
	switchValue := cosmos.NewUint(100)
...
func (s *TxOutStoreVCURSuite) TestSplitCloutWithZeroSWITCHValue(c *check.C) {
	ctx, _ := setupManagerForTest(c)
	tos := TxOutStorageVCUR{}
	...
	switchValue := cosmos.NewUint(0)

	// Call function under test
	amountFromClout1, amountFromClout2, remainingSWITCH := tos.splitClout(ctx, swapperCloutLimit, clout1, clout2, switchValue)

	// Assert that no clout was taken from either and switch value remains zero
...
func (s *TxOutStoreVCURSuite) TestSplitCloutSWITCHValueExceedingClouts(c *check.C) {
	ctx, _ := setupManagerForTest(c)
	tos := TxOutStorageVCUR{}
	swapperCloutLimit := cosmos.NewUint(100)
	clout1 := cosmos.NewUint(40)
	clout2 := cosmos.NewUint(40)
	switchValue := cosmos.NewUint(120)

	// Call function under test
	amountFromClout1, amountFromClout2, remainingSWITCH := tos.splitClout(ctx, swapperCloutLimit, clout1, clout2, switchValue)

	// Assert that the clouts took the maximum they could, and the remaining switch is correct
	...
	c.Check(remainingSWITCH.Equal(cosmos.NewUint(40)), check.Equals, true)
}
