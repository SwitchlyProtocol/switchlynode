...
func (vm *ValidatorMgrVCUR) EndBlock(ctx cosmos.Context, mgr Manager) []abci.ValidatorUpdate {
	...
	if (len(activeNodes) >= int(minimumNodesForBFT) && nodesAfterChange < int(minimumNodesForBFT)) ||
		...
		// SWITCHLYNode don't have enough validators for BFT
...
func (vm *ValidatorMgrVCUR) payNodeAccountBondAward(ctx cosmos.Context, lastChurnHeight int64, na NodeAccount, totalBondReward, totalEffectiveBond, bondHardCap cosmos.Uint, mgr Manager) error {
	...
	// Minus the number of switch SWITCHLYNode have awarded them
	network.BondRewardSwitch = common.SafeSub(network.BondRewardSwitch, reward)
...
func (vm *ValidatorMgrVCUR) processRagnarok(ctx cosmos.Context, mgr Manager) error {
	...
	// SWITCHLYNode have to request the fund back now, because once it get to the rotate block height ,
	// SWITCHLYNode won't have validators anymore
...
func (vm *ValidatorMgrVCUR) ragnarokProtocolStage2(ctx cosmos.Context, nth int64, mgr Manager) error {
	// Ragnarok Protocol
	// If SWITCHLYNode can no longer be BFT, do a graceful shutdown of the entire network.
	// 1) SWITCHLYNode will refund the validator's bond
...
func (vm *ValidatorMgrVCUR) distributeBondReward(ctx cosmos.Context, mgr Manager) error {
	...
	// https://switchlynode.ninerealms.com/switchly/nodes?height=5336942
	// (Nodes .cxmy and .uy3a .)
	// https://switchlynode.ninerealms.com/switchly/vaults/asgard?height=5336942
	...
	for _, item := range active {
		if err := vm.payNodeAccountBondAward(ctx, lastChurnHeight, item, network.BondRewardSwitch, totalEffectiveBond, bondHardCap, mgr); err != nil {
			resultErr = err
	...
	for i := len(pools) - 1; i >= 0; i-- { // iterate backwards
		...
		for ; iterator.Valid(); iterator.Next() {
			...
			if j == position.Number {
				...
				if err != nil {
					ctx.Logger().Error("fail to withdraw", "liquidity provider", lp.SwitchAddress, "error", err)
				} else if !withdrawAsset.Equals(common.SwitchNative) {
					// when withdraw asset is only SWITCH , then it should process more , because SWITCH asset doesn't leave SwitchlyChain
					count++
					pending, err := vm.k.GetRagnarokPending(ctx)
					if err != nil {
						return fmt.Errorf("fail to get ragnarok pending: %w", err)
...
func (vm *ValidatorMgrVCUR) setupValidatorNodes(ctx cosmos.Context, height int64) error {
	...
	for ; iter.Valid(); iter.Next() {
		...
		// when SWITCHLYNode first start , SWITCHLYNode only care about these two status
		switch na.Status {
		case NodeReady:
