...
func validateTssAuth(ctx cosmos.Context, k keeper.Keeper, signer cosmos.AccAddress) error {
	...
	minBond := k.GetConfigInt64(ctx, constants.MinimumBondInSWITCH)
	if nodeSigner.Bond.LT(cosmos.NewUint(uint64(minBond))) {
		return fmt.Errorf("signer doesn't have enough switch")
	}
	return nil
}
...
func MsgTssPoolHandleV3_0_0(ctx cosmos.Context, mgr Manager, msg *MsgTssPool) (*cosmos.Result, error) {
	...
	if !msg.Blame.IsEmpty() {
		for i := range msg.Blame.BlameNodes {
			...
			if err != nil {
				ctx.Logger().Error("fail to get tss keygen switchly address", "pubkey", msg.Blame.BlameNodes[i].Pubkey, "error", err)
				continue
			}
	...
	if voter.BlockHeight == 0 {
		...
		for _, member := range msg.PubKeys {
			...
			if err != nil {
				ctx.Logger().Error("fail to get switchly address", "error", err)
				continue
			}
