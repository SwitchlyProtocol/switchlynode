...
func (s *SwitchlySuite) TestLiquidityProvision(c *C) {
	var err error
	ctx, keeper := setupKeeperForTest(c)
	user1rune := GetRandomSwitchAddress()
	user1asset := GetRandomETHAddress()
	user2rune := GetRandomSwitchAddress()
...
func (s *SwitchlySuite) TestChurn(c *C) {
...
func (s *SwitchlySuite) TestRagnarok(c *C) {
	SetupConfigForTest()
	var err error
	ctx, mgr := setupManagerForTest(c)
	...
	lp1 := GetRandomSwitchAddress() // LiquidityProvider1
	...
	c.Assert(err, IsNil)
	lp2 := GetRandomSwitchAddress() // liquidity provider 2
	...
	c.Assert(err, IsNil)
	lp3 := GetRandomSwitchAddress() // liquidity provider 3
	...
	lp4 := GetRandomSWITCHLYAddress() // liquidity provider 4 , BTC
	...
	lp5 := GetRandomSWITCHLYAddress() // SWITCH only
	...
	// ////////////////////////////////////////////////////////
	network := Network{
		BondRewardSwitch: cosmos.NewUint(1000_000 * common.One),
...
func (s *SwitchlySuite) TestRagnarokNoOneLeave(c *C) {
	var err error
	ctx, mgr := setupManagerForTest(c)
	ctx = ctx.WithBlockHeight(10)
	ver := GetCurrentVersion()
