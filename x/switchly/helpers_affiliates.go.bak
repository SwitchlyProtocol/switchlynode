...
import (
	...
	switchly "github.com/switchlyprotocol/switchlynode/v3/x/switchly/memo"
)

func triggerPreferredAssetSwap(ctx cosmos.Context, mgr Manager, affiliateAddress common.Address, txID common.TxID, tn SWITCHName, affcol AffiliateFeeCollector, queueIndex int) error {
...
func triggerPreferredAssetSwapV3_0_0(ctx cosmos.Context, mgr Manager, tn SWITCHName, affcol AffiliateFeeCollector, queueIndex int) error {
	// Check that the SWITCHName has an address alias for the PreferredAsset, if not skip
	// the swap
	...
	if affcol.SwitchAmount.IsZero() {
		return fmt.Errorf("can't execute preferred asset swap, accrued SWITCH amount is zero")
	}
	// Sanity check: ensure the swap amount isn't more than the entire AffiliateCollector module
	acBalance := mgr.Keeper().GetSWITCHBalanceOfModule(ctx, AffiliateCollectorName)
	if affcol.SwitchAmount.GT(acBalance) {
		return fmt.Errorf("switch amount greater than module balance: (%s/%s)", affcol.SwitchAmount.String(), acBalance.String())
	}

	affSWITCH := affcol.SwitchAmount
	affCoin := common.NewCoin(common.SwitchNative, affSWITCH)
	...
	ctx.Logger().Debug("trigger preferred asset swap", "switchlyname", tn.Name, "amt", affSWITCH.String(), "dest", alias.String(), "asset", tn.PreferredAsset.String())

	// Generate a unique ID for the preferred asset swap, which is a hash of the SWITCHName,
	...
	ctx.Logger().Info("preferred asset swap hash", "hash", hash, "switchlyname", tn.Name)
	...
	// Send SWITCH from AffiliateCollector to Asgard and update AffiliateCollector
	if err = mgr.Keeper().SendFromModuleToModule(ctx, AffiliateCollectorName, AsgardName, common.NewCoins(affCoin)); err != nil {
		return fmt.Errorf("failed to send switch to asgard: %w", err)
	}

	affcol.SwitchAmount = cosmos.ZeroUint()
...
//     - If coin is SWITCH transfer to the affiliate
//     - If coin is not SWITCH, swap the coin to SWITCH and transfer to the affiliate
//     - If affiliate is a switchlyname and has a preferred asset, send SWITCH to the affiliate collector
...
func skimAffiliateFeesV3_0_0(ctx cosmos.Context, mgr Manager, mainTx common.Tx, signer cosmos.AccAddress, coin common.Coin, memoStr string) (cosmos.Uint, error) {
	...
	// Parse memo
	memo, err := ParseMemoWithSWITCHNames(ctx, mgr.Keeper(), memoStr)
	if err != nil {
	...
	for i, affiliate := range affiliates {
		...
		// Determine if affiliate is address or switchlyname. If it's an address it must be a SWITCH address.
		var switchAddr cosmos.AccAddress
		var switchlyname *SWITCHName
		tnString := ""

		// Fetch switchlyname + SWITCH alias for SWITCHLYChain
		if mgr.Keeper().SWITCHNameExists(ctx, affiliate) {
			tn, errTn := mgr.Keeper().GetSWITCHName(ctx, affiliate)
			if errTn != nil {
				ctx.Logger().Error("fail to get switchlyname, skipping fee", "err", err)
				continue
			}
			switchlyname = &tn

			// If affiliate is switchlyname, check if it can receive an affiliate fee. If not, skip.
			if !switchlyname.CanReceiveAffiliateFee() {
				ctx.Logger().Info("affiliate cannot receive affiliate fee", "affiliate", affiliate)
				continue
			}
			addr := switchlyname.GetAlias(common.SWITCHLYChain)
			if !addr.IsEmpty() {
				switchAddr, err = addr.AccAddress()
				if err != nil {
					ctx.Logger().Error("fail to convert address into AccAddress, skipping fee", "msg", addr, "error", err)
					continue
				}
			} else {
				// If this is reached, the switchlyname has a preferred asset + no alias for SWITCH,
				// so set swap destination to switchlyname owner
				switchAddr = switchlyname.Owner
			}
		} else {
			...
			if !addr.GetChain().IsSWITCHLYChain() {
				ctx.Logger().Error("affiliate address is not SWITCHLYChain, skipping fee", "msg", affiliate)
				continue
			}
			switchAddr, err = addr.AccAddress()
			if err != nil {
		...
		if !feeBps.IsZero() {
			...
			if coin.Asset.IsSwitch() {
				// Transfer to SWITCH address or affiliate collector module
				if switchlyname != nil && !switchlyname.PreferredAsset.IsEmpty() {
					// Send SWITCH to the affiliate collector and update the account
					err = addSWITCHToAffiliateCollector(ctx, mgr, affCoin, switchlyname, &swapIndex)
					if err != nil {
						ctx.Logger().Error("fail to update affiliate collector", "error", err)
						continue
					}
				} else {
					// Send SWITCH to the affiliate address
					err = mgr.Keeper().SendFromModuleToAccount(ctx, AsgardName, switchAddr, common.NewCoins(affCoin))
					if err != nil {
						ctx.Logger().Error("fail to send switch to affiliate", "affiliate", affiliate, "error", err)
						continue
					}
				}
			} else {
				// Swap to SWITCH and transfer to SWITCH address or affiliate collector module
				err := affiliateSwapToSWITCH(ctx, mgr, mainTx, signer, affAmt, switchAddr, memo, switchlyname, &swapIndex)
				if err != nil {
					ctx.Logger().Error("fail to swap to switch", "affiliate", affiliate, "error", err)
					continue
				}
			}
			if switchlyname != nil && switchlyname.Name != "" {
				tnString = switchlyname.Name
			}
			...
			feeEvent := NewEventAffiliateFee(
				...
				mainTx.Memo,
				tnString,
				common.Address(switchAddr.String()),
				coin.Asset,
				feeBps.Uint64(),
				coin.Amount,
				affCoin.Amount)
...
func affiliateSwapToSWITCH(ctx cosmos.Context, mgr Manager, mainTx common.Tx, signer cosmos.AccAddress, affAmt cosmos.Uint, affAcc cosmos.AccAddress, memo switchly.Memo, tn *SWITCHName, swapIndex *int) error {
	version := mgr.GetVersion()
	switch {
	case version.GTE(semver.MustParse("3.0.0")):
		return affiliateSwapToSWITCHV3_0_0(ctx, mgr, mainTx, signer, affAmt, affAcc, memo, tn, swapIndex)
	default:
		return errBadVersion
	}
}

func affiliateSwapToSWITCHV3_0_0(ctx cosmos.Context, mgr Manager, mainTx common.Tx, signer cosmos.AccAddress, affAmt cosmos.Uint, affAcc cosmos.AccAddress, memo switchly.Memo, tn *SWITCHName, swapIndex *int) error {
	...
	if tn != nil && !tn.Owner.Empty() && !tn.PreferredAsset.IsEmpty() {
		...
		if err != nil {
			return fmt.Errorf("failed to get affiliate collector for switchlyname: %w", err)
		}
		...
		// Check if accrued SWITCH is 100x current outbound fee of preferred asset chain, if
		// so trigger the preferred asset swap
		ofSWITCH, err := mgr.GasMgr().GetAssetOutboundFee(ctx, tn.PreferredAsset, true)
		if err != nil {
		...
		ctx.Logger().Info("check preferred asset threshold", "threshold", threshold.String(), "accrued", affCol.SwitchAmount.String())
		...
		if err == nil && affCol.SwitchAmount.GT(threshold) {
			*swapIndex++
			if err = triggerPreferredAssetSwap(ctx, mgr, common.NoAddress, "", *tn, affCol, *swapIndex); err != nil {
				ctx.Logger().Error("fail to swap to preferred asset", "switchlyname", tn.Name, "err", err)
			}
		}
	}
...
// addSWITCHToAffiliateCollector - accrue SWITCH in the AffiliateCollector module and check if
// a PreferredAsset swap should be triggered. Returns an error if the fee distribution fails.
func addSWITCHToAffiliateCollector(ctx cosmos.Context, mgr Manager, coin common.Coin, switchlyname *SWITCHName, swapIndex *int) error {
	version := mgr.GetVersion()
	switch {
	case version.GTE(semver.MustParse("3.0.0")):
		return updateAffiliateCollectorV3_0_0(ctx, mgr, coin, switchlyname, swapIndex)
	default:
		return fmt.Errorf("unsupported version: %s", version)
	}
}

// updateAffiliateCollector - accrue SWITCH in the AffiliateCollector module and check if
// a PreferredAsset swap should be triggered. Returns an error if the fee distribution fails.
func updateAffiliateCollectorV3_0_0(ctx cosmos.Context, mgr Manager, coin common.Coin, switchlyname *SWITCHName, swapIndex *int) error {
	affcol, err := mgr.Keeper().GetAffiliateCollector(ctx, switchlyname.Owner)
	if err != nil {
		...
		if err = mgr.Keeper().SendFromModuleToModule(ctx, AsgardName, AffiliateCollectorName, common.NewCoins(coin)); err != nil {
			...
			affcol.SwitchAmount = affcol.SwitchAmount.Add(coin.Amount)
			mgr.Keeper().SetAffiliateCollector(ctx, affcol)
		}
	}

	// Check if balance exceeds threshold for preferred asset swap. Don't return error if preferred asset swap fails.
	ofSWITCH, err := mgr.GasMgr().GetAssetOutboundFee(ctx, switchlyname.PreferredAsset, true)
	if err != nil {
		ctx.Logger().Error("failed to get outbound fee for preferred asset, skipping preferred asset swap", "name", switchlyname.Name, "asset", switchlyname.PreferredAsset, "error", err)
		return nil
	}

	multiplier := mgr.Keeper().GetConfigInt64(ctx, constants.PreferredAssetOutboundFeeMultiplier)
	threshold := ofSWITCH.Mul(cosmos.NewUint(uint64(multiplier)))
	if affcol.SwitchAmount.GT(threshold) {
		*swapIndex++
		if err = triggerPreferredAssetSwap(ctx, mgr, common.NoAddress, "", *switchlyname, affcol, *swapIndex); err != nil {
			ctx.Logger().Error("fail to swap to preferred asset", "switchlyname", switchlyname.Name, "err", err)
		}
	}
	return nil
}
