...
func (vm *SwapQueueAdvVCUR) checkFeelessSwap(pools Pools, pair tradePair, indexRatio uint64) bool {
	...
	case !pair.HasSWITCH():
		...
		switchAmt := common.GetSafeShare(one, sourcePool.BalanceAsset, sourcePool.BalanceSwitch)
		emit := common.GetSafeShare(switchAmt, targetPool.BalanceSwitch, targetPool.BalanceAsset)
...
func (vm *SwapQueueAdvVCUR) EndBlock(ctx cosmos.Context, mgr Manager) error {
	...
	for i := int64(0); i < vm.getTodoNum(int64(len(swaps)), minSwapsPerBlock, maxSwapsPerBlock); i++ {
		...
		if !msg.AffiliateBasisPoints.IsZero() && msg.AffiliateAddress.IsChain(common.SWITCHLYChain) {
		...
		if err != nil {
			...
			if !affiliateSwap.Tx.IsEmpty() {
				// if asset sent in is native switch, no need
