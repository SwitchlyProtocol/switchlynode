...
func (HandlerLeaveSuite) TestLeaveHandler_NotActiveNodeLeave(c *C) {
	...
	tx := common.NewTx(
		...
		acc2.BondAddress,
		GetRandomETHAddress(),
		common.Coins{common.NewCoin(common.SwitchNative, cosmos.ZeroUint())},
		common.Gas{
...
func (HandlerLeaveSuite) TestLeaveBondProvider(c *C) {
	...
	minBond := w.keeper.GetConfigInt64(w.ctx, constants.MinimumBondInSWITCH)

	bpBelowMin := GetRandomSWITCHLYAddress()
	bpBelowMinAccAddress, err := bpBelowMin.AccAddress()
	c.Assert(err, IsNil)
	bp1 := types.NewBondProvider(bpBelowMinAccAddress)
	// bpBelowMin bonds 0.000001 SWITCH (below min bond)
	bp1.Bond = cosmos.NewUint(100)

	bpAboveMin := GetRandomSWITCHLYAddress()
