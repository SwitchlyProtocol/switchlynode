...
func (h LoanRepaymentHandler) validateV3_0_0(ctx cosmos.Context, msg MsgLoanRepayment) error {
	...
	// if a swap is required (ie the coin is NOT SWITCHLY), then ensure that the
	...
	// is ETH.ETH
	if !msg.Coin.Asset.Equals(common.SWITCHLY) && !msg.From.IsChain(msg.Coin.Asset.GetChain()) {
...
func (h LoanRepaymentHandler) handleV3_0_0(ctx cosmos.Context, msg MsgLoanRepayment) error {
	...
	if msg.Coin.Asset.Equals(common.SWITCHLY) {
...
func (h LoanRepaymentHandler) swap(ctx cosmos.Context, msg MsgLoanRepayment) error {
	...
	swapMsg := NewMsgSwap(tx, common.SWITCHLY, common.NoopAddress, cosmos.ZeroUint(), common.NoAddress, cosmos.ZeroUint(), "", "", nil, 0, 0, uint64(ssInterval), msg.Signer)
