...
func (m *EventMgrVCUR) EmitSwapEvent(ctx cosmos.Context, swap *EventSwap) error {
	...
	// events change doesn't break midgard and smoke test, for double swap , we first swap the source asset to SWITCH ,
	// and then from SWITCH to target asset, so the first will be marked as success
