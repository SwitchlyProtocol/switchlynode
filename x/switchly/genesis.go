...
func initGenesis(ctx cosmos.Context, keeper keeper.Keeper, data GenesisState) []abci.ValidatorUpdate {
	...
	for _, item := range data.SwcyClaimers {
		if err := keeper.SetSWCYClaimer(ctx, item); err != nil {
			panic(err)
		}
	}

	for _, item := range data.SwcyStakers {
	...
	swcyStakeAddr, _ := keeper.GetModuleAddress(SWCYStakeName)
	ctx.Logger().Info("SWCYStake Module", "address", swcyStakeAddr.String())
...
func ExportGenesis(ctx cosmos.Context, k keeper.Keeper) GenesisState {
	...
	swcyClaimers := make([]SWCYClaimer, 0)
	...
	for ; iterator.Valid(); iterator.Next() {
		...
		swcyClaimers = append(swcyClaimers, claimer)
	}

	swcyStakers := make([]SWCYStaker, 0)
	...
	for _, staker := range stakers {
		...
		swcyStakers = append(swcyStakers, staker)
	}
	...
	return GenesisState{
		...
		SwcyClaimers:              swcyClaimers,
		SwcyStakers:               swcyStakers,
	}
}
