...
func (HandlerBondSuite) TestBondHandler_ValidateActive(c *C) {
	...
	txIn := common.NewTx(
		...
		GetRandomETHAddress(),
		common.Coins{
			common.NewCoin(common.SwitchNative, cosmos.NewUint(10*common.One)),
		},
		common.Gas{
...
func (HandlerBondSuite) TestBondHandler_Run(c *C) {
	...
	minimumBondInSWITCH := constAccessor.GetInt64Value(constants.MinimumBondInSWITCH)
	txIn := common.NewTx(
		GetRandomTxHash(),
		GetRandomSWITCHLYAddress(),
		GetRandomSWITCHLYAddress(),
		common.Coins{
			common.NewCoin(common.SwitchNative, cosmos.NewUint(uint64(minimumBondInSWITCH+common.One))),
		},
		common.Gas{
			common.NewCoin(common.ETHAsset, cosmos.NewUint(10000)),
		},
	...
	FundModule(c, ctx, k1, BondName, uint64(minimumBondInSWITCH))
	msg := NewMsgBond(txIn, GetRandomValidatorNode(NodeStandby).NodeAddress, cosmos.NewUint(uint64(minimumBondInSWITCH)+common.One), GetRandomSWITCHLYAddress(), nil, standbyNodeAccount.NodeAddress, -1)
	_, err := handler.Run(ctx, msg)
	c.Assert(err, IsNil)
	coin := common.NewCoin(common.SwitchNative, cosmos.NewUint(common.One))
	nativeSWITCHCoin, err := coin.Native()
	c.Assert(err, IsNil)
	c.Assert(k1.HasCoins(ctx, msg.NodeAddress, cosmos.NewCoins(nativeSWITCHCoin)), Equals, true)
	na, err := k1.GetNodeAccount(ctx, msg.NodeAddress)
	c.Assert(err, IsNil)
	c.Assert(na.Status.String(), Equals, NodeWhiteListed.String())
	c.Assert(na.Bond.Equal(cosmos.NewUint(uint64(minimumBondInSWITCH))), Equals, true)

	// simulate fail to get node account
	handler = NewBondHandler(NewDummyMgrWithKeeper(k))
	msg = NewMsgBond(txIn, k.failGetNodeAccount.NodeAddress, cosmos.NewUint(uint64(minimumBondInSWITCH)), GetRandomSWITCHLYAddress(), nil, standbyNodeAccount.NodeAddress, -1)
	_, err = handler.Run(ctx, msg)
	c.Assert(errors.Is(err, errInternal), Equals, true)

	// When node account is standby , it is ok to bond
	msg = NewMsgBond(txIn, k.notEmptyNodeAccount.NodeAddress, cosmos.NewUint(uint64(minimumBondInSWITCH)), common.Address(k.notEmptyNodeAccount.NodeAddress.String()), nil, standbyNodeAccount.NodeAddress, -1)
	_, err = handler.Run(ctx, msg)
	c.Assert(err, IsNil)
}
...
func (HandlerBondSuite) TestBondHandlerFailValidation(c *C) {
	...
	txIn := common.NewTx(
		...
		GetRandomSWITCHLYAddress(),
		common.Coins{
			common.NewCoin(common.SwitchNative, cosmos.NewUint(uint64(minimumBondInSWITCH))),
		},
		common.Gas{
			common.NewCoin(common.ETHAsset, cosmos.NewUint(10000)),
		},
	...
	testCases := []struct {
	...
	}{
		{
			...
			msg:         NewMsgBond(txIn, cosmos.AccAddress{}, cosmos.NewUint(uint64(minimumBondInSWITCH)), GetRandomSWITCHLYAddress(), nil, standbyNodeAccount.NodeAddress, -1),
			expectedErr: se.ErrInvalidAddress,
		},
		{
			name:        "zero bond",
			msg:         NewMsgBond(txIn, GetRandomValidatorNode(NodeStandby).NodeAddress, cosmos.ZeroUint(), GetRandomSWITCHLYAddress(), nil, standbyNodeAccount.NodeAddress, -1),
			expectedErr: se.ErrUnknownRequest,
		},
		{
			name:        "empty bond address",
			msg:         NewMsgBond(txIn, GetRandomValidatorNode(NodeStandby).NodeAddress, cosmos.NewUint(uint64(minimumBondInSWITCH)), common.Address(""), nil, standbyNodeAccount.NodeAddress, -1),
			expectedErr: se.ErrInvalidAddress,
		},
		{
			name:        "empty request hash",
			msg:         NewMsgBond(txInNoTxID, GetRandomValidatorNode(NodeStandby).NodeAddress, cosmos.NewUint(uint64(minimumBondInSWITCH)), GetRandomSWITCHLYAddress(), nil, standbyNodeAccount.NodeAddress, -1),
			expectedErr: se.ErrUnknownRequest,
		},
		{
			name:        "empty signer",
			msg:         NewMsgBond(txIn, GetRandomValidatorNode(NodeStandby).NodeAddress, cosmos.NewUint(uint64(minimumBondInSWITCH)), GetRandomSWITCHLYAddress(), nil, cosmos.AccAddress{}, -1),
			expectedErr: se.ErrInvalidAddress,
		},
		{
			name:        "active node",
			msg:         NewMsgBond(txIn, GetRandomValidatorNode(NodeActive).NodeAddress, cosmos.NewUint(uint64(minimumBondInSWITCH)), GetRandomETHAddress(), nil, cosmos.AccAddress{}, -1),
			expectedErr: se.ErrInvalidAddress,
		},
	}
...
func (HandlerBondSuite) TestBondProvider_Validate(c *C) {
	...
	txIn.Coins = common.NewCoins(common.NewCoin(common.SwitchNative, amt))
	...
	msg = NewMsgBond(txIn, standbyNA, amt, GetRandomSWITCHLYAddress(), nil, activeNA, -1)
	err = handler.validate(ctx, *msg)
...
func (HandlerBondSuite) TestBondProvider_OperatorFee(c *C) {
	...
	standbyNodeAccount := GetRandomValidatorNode(NodeStandby)
	operatorBondAddress := GetRandomSwitchAddress()
	operatorAccAddress, _ := operatorBondAddress.AccAddress()
	providerBondAddress := GetRandomSwitchAddress()
...
func (HandlerBondSuite) TestBondProvider_Handler(c *C) {
	...
	// This means there are 300 SWITCH in rewards to distribute (NodeAccount.Bond - sum(BondProviders bond))
	bp, _ = k.GetBondProviders(ctx, standbyNA)
	...
	// BP#2 should have 1/3rd rewards - 5% operator fee + the 100 SWITCH new bond (295)
	bp, _ = k.GetBondProviders(ctx, standbyNA)
	c.Assert(bp.Providers, HasLen, 2)
