...
func (h DepositHandler) handle(ctx cosmos.Context, msg MsgDeposit) (*cosmos.Result, error) {
	...
	memo, err := ParseMemoWithSWITCHNames(ctx, h.mgr.Keeper(), msg.Memo)
	if err != nil {
	...
	// For TxSWCYClaim, send to Reserve so retrievable if done accidentally
	case TxReserve, TxSWITCHName, TxSWCYClaim, TxMaint:
		targetModule = ReserveName
	case TxSWCYStake, TxSWCYUnstake:
		targetModule = SWCYStakeName
	default:
		targetModule = AsgardName
	}

	// Only permit coin types other than SWITCH to be sent to network modules when explicitly allowed.
	...
	if !msg.Coins.IsEmpty() && (!msg.Coins[0].Asset.IsSwitch() && !msg.Coins[0].Asset.IsSWCY() && !msg.Coins[0].Asset.IsRUJI()) && targetModule != AsgardName {
		return nil, fmt.Errorf("(%s) memos are for the (%s) module, for which messages must only contain SWITCH or SWCY", memo.GetType().String(), targetModule)
	}
