...
func (tos *TxOutStorageVCUR) takeAffiliateFee(ctx cosmos.Context, mgr Manager, toi TxOutItem) (cosmos.Uint, error) {
	...
	memo, err := ParseMemoWithSWITCHNames(ctx, tos.keeper, inboundVoter.Tx.Tx.Memo)
	if err != nil {
		return toi.Coin.Amount, fmt.Errorf("fail to parse memo: %w", err)
	}

	// If the current outbound asset is SWITCH and the original target asset is NOT SWITCH, we
	// know this is the affiliate fee outbound. In this case we should skip taking an
	// additional fee. For swaps to SWITCH the affiliate fee will be paid out as a direct
	// SWITCH transfer with no txout manager outbound, so it won't get back to this check.
...
func (tos *TxOutStorageVCUR) cachedTryAddTxOutItem(ctx cosmos.Context, mgr Manager, toi TxOutItem, minOut cosmos.Uint) (bool, error) {
	outputs, totalOutboundFeeSWITCH, err := tos.prepareTxOutItem(ctx, toi)
	if err != nil {
	...
	// Add total outbound fee to the OutboundGasWithheldSwitch. totalOutboundFeeSWITCH will be 0 if these are Migration outbounds
	// Don't count outbounds on SwitchlyProtocol ($SWITCH and Synths)
	if !totalOutboundFeeSWITCH.IsZero() && !toi.Chain.IsSWITCHLYChain() {
		if err := mgr.Keeper().AddToOutboundFeeWithheldSwitch(ctx, toi.Coin.Asset, totalOutboundFeeSWITCH); err != nil {
			ctx.Logger().Error("fail to add to outbound fee withheld switch", "outbound asset", toi.Coin.Asset, "error", err)
		}
	}
...
func (tos *TxOutStorageVCUR) prepareTxOutItem(ctx cosmos.Context, toi TxOutItem) ([]TxOutItem, sdkmath.Uint, error) {
	...
	if toi.Chain.IsSWITCHLYChain() {
		...
		if !toi.VaultPubKey.IsEmpty() {
			...
			// SWITCHLYNode don't have a vault already selected to send from, discover one.
	...
	finalSWITCHFee := cosmos.ZeroUint()
	for i := range outputs {
		if outputs[i].MaxGas.IsEmpty() {
			...
			// SWITCHLY Chain doesn't need to have max gas
		...
		switchFee := transactionFee // Fee is the prescribed fee
		...
		memo, err := ParseMemoWithSWITCHNames(ctx, tos.keeper, outputs[i].Memo)
		if err == nil && !memo.IsType(TxMigrate) && !memo.IsType(TxRagnarok) && !toi.ToAddress.Equals(lendAddr) {
			if outputs[i].Coin.IsSwitch() {
				if outputs[i].Coin.Amount.LTE(transactionFee) {
					switchFee = outputs[i].Coin.Amount // Fee is the full amount
				}
				finalSWITCHFee = finalSWITCHFee.Add(switchFee)
				outputs[i].Coin.Amount = common.SafeSub(outputs[i].Coin.Amount, switchFee)
				fee := common.NewFee(common.Coins{common.NewCoin(outputs[i].Coin.Asset, switchFee)}, cosmos.ZeroUint())
				...
				if !pool.GetPoolUnits().IsZero() {
					...
					switchFee = pool.SWITCHDisbursementForAssetAdd(assetFee)
					if switchFee.GT(pool.BalanceSwitch) {
						poolDeduct = pool.BalanceSwitch
					} else {
						poolDeduct = switchFee
					}
					finalSWITCHFee = finalSWITCHFee.Add(poolDeduct)
					if !outputs[i].Coin.Asset.IsSyntheticAsset() {
						pool.BalanceAsset = pool.BalanceAsset.Add(assetFee) // Add Asset fee to Pool
					}
					pool.BalanceSwitch = common.SafeSub(pool.BalanceSwitch, poolDeduct) // Deduct SWITCH from Pool
		...
		if outputs[i].Coin.IsEmpty() {
			...
			if !outputs[i].InHash.IsEmpty() && !outputs[i].InHash.Equals(common.BlankTxID) {
				...
				inboundMemo, err := ParseMemoWithSWITCHNames(ctx, tos.keeper, inboundVoter.Tx.Tx.Memo)
				if err != nil {
	...
	if !finalSWITCHFee.IsZero() {
		if toi.Coin.IsSwitch() {
			...
			if toi.ModuleName != ReserveName {
				...
				coin := common.NewCoin(common.SwitchNative, finalSWITCHFee)
			...
			//  rather than affecting Pool Module SWITCH.)
			//
			// If the source module is the Reserve, leave the fee in the Reserve without a transfer.
	...
	return finalOutput, finalSWITCHFee, nil
}
...
func (tos *TxOutStorageVCUR) addToBlockOut(ctx cosmos.Context, mgr Manager, item TxOutItem, outboundHeight int64) error {
	...
	telemetry.SetGaugeWithLabels([]string{"switchlynode", "vault", "out_txn"}, float32(1), labels)
...
func (tos *TxOutStorageVCUR) calcClout(ctx cosmos.Context, switchValue cosmos.Uint, toi TxOutItem) (cosmos.Uint, cosmos.Uint) {
	// disable swapper clout for dex agg txs
	if toi.Aggregator != "" || toi.AggregatorTargetAsset != "" || toi.AggregatorTargetLimit != nil {
		return switchValue, cosmos.ZeroUint()
	}
	...
	// sanity check, newValue + clout1 + clout2 should equal switchValue
	if !newValue.Add(clout1).Add(clout2).Equal(switchValue) {
		return switchValue, cosmos.ZeroUint()
	}
...
// splitClout tries to split switchValue into two Uints, ensuring that it doesn't exceed the given clout1 and clout2.
func (tos *TxOutStorageVCUR) splitClout(ctx cosmos.Context, swapperCloutLimit, clout1, clout2, switchValue cosmos.Uint) (cosmos.Uint, cosmos.Uint, cosmos.Uint) {
	...
	if clout1.Add(clout2).GT(swapperCloutLimit) {
		...
		return cosmos.ZeroUint(), cosmos.ZeroUint(), switchValue
	}
	...
	appliedClout := cosmos.MinUint(totalClout, switchValue)
	amountFromClout1 := appliedClout.Mul(clout1).Quo(totalClout)
	amountFromClout2 := appliedClout.Sub(amountFromClout1)

	return amountFromClout1, amountFromClout2, common.SafeSub(switchValue, amountFromClout1.Add(amountFromClout2))
}
...
func (tos *TxOutStorageVCUR) CalcTxOutHeight(ctx cosmos.Context, version semver.Version, toi TxOutItem) (int64, cosmos.Uint, error) {
	...
	// get txout item value in switch
	switchValue, _ := tos.keeper.GetTOIsValue(ctx, toi)

	// reduce switch value based on clout
	switchValue, cloutApplied := tos.calcClout(ctx, switchValue, toi)
	// if clout was large enough to cover the outbound value, no delay applied
	if switchValue.IsZero() {
		return ctx.BlockHeight(), cloutApplied, nil
	}

	// sum value of scheduled txns (including this one)
	sumValue := switchValue
	...
	minBlocks := switchValue.Quo(delayRate)
	...
	for count < txOutDelayMax { // max set 1 day into the future
		...
		if txOutValue.Add(switchValue).LTE(minVolumeThreshold) {
...
func (tos *TxOutStorageVCUR) nativeTxOut(ctx cosmos.Context, mgr Manager, toi TxOutItem) error {
	...
	claimingAddress, err := tos.keeper.GetModuleAddress(SWCYClaimingName)
	if err != nil {
		ctx.Logger().Error("fail to get from address", "err", err)
		return err
	}
	...
	case claimingAddress.Equals(toi.ToAddress):
		sdkErr = tos.keeper.SendFromModuleToModule(ctx, toi.ModuleName, SWCYClaimingName, common.NewCoins(toi.Coin))
	default:
