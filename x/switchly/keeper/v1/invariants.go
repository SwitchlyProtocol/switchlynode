...
func (k KVStore) InvariantRoutes() []common.InvariantRoute {
	return []common.InvariantRoute{
		...
		common.NewInvariantRoute("switchpool", SwitchPoolInvariant(k)),
		common.NewInvariantRoute("lending", LendingInvariant(k)),
	}
}

// AsgardInvariant the asgard module backs pool switch, savers synths, and native
...
func AsgardInvariant(k KVStore) common.Invariant {
	return func(ctx cosmos.Context) (msg []string, broken bool) {
		// sum all switch liquidity on pools, including pending
		...
		// sum all switch in pending swaps
...
func BondInvariant(k KVStore) common.Invariant {
	return func(ctx cosmos.Context) (msg []string, broken bool) {
		// sum all switch bonded to nodes
		bondedSWITCH := cosmos.ZeroUint()
		naIter := k.GetNodeAccountIterator(ctx)
		defer naIter.Close()
		for ; naIter.Valid(); naIter.Next() {
			var na NodeAccount
			k.Cdc().MustUnmarshal(naIter.Value(), &na)
			bondedSWITCH = bondedSWITCH.Add(na.Bond)
		}

		// get pending bond reward switch
		network, _ := k.GetNetwork(ctx)
		bondRewardSWITCH := network.BondRewardSwitch

		// get switch balance of bond module
		bondModuleSWITCH := k.GetBalanceOfModule(ctx, BondName, common.SwitchNative.Native())

		// bond module is expected to equal bonded switch and pending rewards
		expectedSWITCH := bondedSWITCH.Add(bondRewardSWITCH)
		if expectedSWITCH.GT(bondModuleSWITCH) {
			broken = true
			diff := expectedSWITCH.Sub(bondModuleSWITCH)
			coin, _ := common.NewCoin(common.SwitchNative, diff).Native()
			msg = append(msg, fmt.Sprintf("insolvent: %s", coin))

		} else if expectedSWITCH.LT(bondModuleSWITCH) {
			broken = true
			diff := bondModuleSWITCH.Sub(expectedSWITCH)
			coin, _ := common.NewCoin(common.SwitchNative, diff).Native()
			msg = append(msg, fmt.Sprintf("oversolvent: %s", coin))
		}
...
// SWITCHLYChainInvariant the switchly module should never hold a balance
func SWITCHLYChainInvariant(k KVStore) common.Invariant {
	return func(ctx cosmos.Context) (msg []string, broken bool) {
		// module balance of switchly
		tcAddr := k.GetModuleAccAddress(ModuleName)
		tcCoins := k.GetBalance(ctx, tcAddr)

		// switchly module should never carry a balance
		if !tcCoins.Empty() {
			broken = true
...
func AffilliateCollectorInvariant(k KVStore) common.Invariant {
	return func(ctx cosmos.Context) (msg []string, broken bool) {
		affColModuleSWITCH := k.GetBalanceOfModule(ctx, AffiliateCollectorName, common.SwitchNative.Native())
		affCols, err := k.GetAffiliateCollectors(ctx)
		if err != nil {
			if affColModuleSWITCH.IsZero() {
				return nil, false
			}
		...
		totalAffSWITCH := cosmos.ZeroUint()
		for _, ac := range affCols {
			totalAffSWITCH = totalAffSWITCH.Add(ac.SwitchAmount)
		}

		if totalAffSWITCH.GT(affColModuleSWITCH) {
			broken = true
			diff := totalAffSWITCH.Sub(affColModuleSWITCH)
			coin, _ := common.NewCoin(common.SwitchNative, diff).Native()
			msg = append(msg, fmt.Sprintf("insolvent: %s", coin))
		} else if totalAffSWITCH.LT(affColModuleSWITCH) {
			broken = true
			diff := affColModuleSWITCH.Sub(totalAffSWITCH)
			coin, _ := common.NewCoin(common.SwitchNative, diff).Native()
			msg = append(msg, fmt.Sprintf("oversolvent: %s", coin))
		}
...
// PoolsInvariant pool units and pending switch/asset should match the sum
// of units and pending switch/asset for all lps
func PoolsInvariant(k KVStore) common.Invariant {
	return func(ctx cosmos.Context) (msg []string, broken bool) {
		...
		for _, pool := range pools {
			...
			lpPendingSwitch := cosmos.ZeroUint()
			...
			for ; lpIter.Valid(); lpIter.Next() {
				...
				lpPendingSwitch = lpPendingSwitch.Add(lp.PendingSwitch)
			...
			check(pool.PendingInboundSwitch, lpPendingSwitch, "pending switch")
...
// SwitchPoolInvariant asserts that the SwitchPool units and provider units are consistent.
func SwitchPoolInvariant(k KVStore) common.Invariant {
	return func(ctx cosmos.Context) (msg []string, broken bool) {
		switchPool, err := k.GetSwitchPool(ctx)
		if err != nil {
			ctx.Logger().Error("error getting switch pool", "error", err)
		...
		if !providerUnits.Equal(switchPool.PoolUnits) {
			m := fmt.Sprintf(
				"pool units %s != provider units %s",
				switchPool.PoolUnits, providerUnits,
			)
			msg = append(msg, m)
			broken = true
		}

		if !providerDeposited.Equal(switchPool.SwitchDeposited) {
			m := fmt.Sprintf(
				"switch deposited %s != provider switch deposited %s",
				switchPool.SwitchDeposited, providerDeposited,
			)
			msg = append(msg, m)
			broken = true
		}

		if !providerWithdrawn.Equal(switchPool.SwitchWithdrawn) {
			m := fmt.Sprintf(
				"switch withdrawn %s != provider switch withdrawn %s",
				switchPool.SwitchWithdrawn, providerWithdrawn,
			)
			msg = append(msg, m)
			broken = true
		}
