...
func UpdateActiveValidatorVersions(
	ctx cosmos.Context,
	switchlyKeeper keeper.Keeper,
	version string,
) error {
	activeVals, err := switchlyKeeper.ListActiveValidators(ctx)
	if err != nil {
	...
	for _, v := range activeVals {
		v.Version = version
		if err = switchlyKeeper.SetNodeAccount(ctx, v); err != nil {
	...
	switchlyKeeper.SetMinJoinLast(ctx)
