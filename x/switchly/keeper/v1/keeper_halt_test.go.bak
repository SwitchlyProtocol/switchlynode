...
func (s *KeeperHaltSuite) TestIsTradingHalt(c *C) {
	...
	// SWCY trading halt
	k.SetMimir(ctx, "HaltSWCYTrading", 1)

	txSWCY := common.Tx{Coins: common.Coins{common.Coin{Asset: common.SwitchNative}}}
	swapSWCYMsg := &MsgSwap{Tx: txSWCY, TargetAsset: common.SWCY}
	addSWCYMsg := &MsgAddLiquidity{Asset: common.SWCY}
	withdrawSWCYMsg := &MsgWithdrawLiquidity{Asset: common.SWCY}

	c.Check(k.IsTradingHalt(ctx, swapSWCYMsg), Equals, true)
	c.Check(k.IsTradingHalt(ctx, addSWCYMsg), Equals, true)
	c.Check(k.IsTradingHalt(ctx, withdrawSWCYMsg), Equals, false)

	txSWCY = common.Tx{Coins: common.Coins{common.Coin{Asset: common.SWCY}}}
	swapSWCYMsg = &MsgSwap{Tx: txSWCY, TargetAsset: common.SwitchNative}
	c.Check(k.IsTradingHalt(ctx, swapSWCYMsg), Equals, true)

	_ = k.DeleteMimir(ctx, "HaltSWCYTrading")

	// ETH trading halt from SWCY
	k.SetMimir(ctx, "HaltETHTrading", 1)

	txSWCY = common.Tx{Coins: common.Coins{common.Coin{Asset: common.SWCY}}}
	swapSWCYMsg = &MsgSwap{Tx: txSWCY, TargetAsset: common.ETHAsset}
	c.Check(k.IsTradingHalt(ctx, swapSWCYMsg), Equals, true)

	_ = k.DeleteMimir(ctx, "HaltETHTrading")

	// ETH trading halt to SWCY
	k.SetMimir(ctx, "HaltETHTrading", 1)

	txSWCY = common.Tx{Coins: common.Coins{common.Coin{Asset: common.ETHAsset}}}
	swapSWCYMsg = &MsgSwap{Tx: txSWCY, TargetAsset: common.SWCY}
	c.Check(k.IsTradingHalt(ctx, swapSWCYMsg), Equals, true)
