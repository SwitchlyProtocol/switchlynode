...
const (
	...
	prefixSWITCHProvider            types.DbPrefix = "rune_provider/"
	prefixSwitchPool                types.DbPrefix = "rune_pool/"
	...
	prefixOutboundFeeWithheldSwitch types.DbPrefix = "outbound_fee_withheld_rune/"
	prefixOutboundFeeSpentSwitch    types.DbPrefix = "outbound_fee_spent_rune/"
	...
	prefixSWITCHName                types.DbPrefix = "switchlyname/"
	...
	prefixSWCYClaimer               types.DbPrefix = "tcy_claimer/"
	prefixSWCYStaker                types.DbPrefix = "tcy_staker/"
)
...
// GetSWITCHBalanceOfModule get the SWITCH balance
func (k KVStore) GetSWITCHBalanceOfModule(ctx cosmos.Context, moduleName string) cosmos.Uint {
...
func (k KVStore) MintToModule(ctx cosmos.Context, module string, coin common.Coin) error {
	// circuit breaker
	// mint new switch coins until we hit the cap (500m). Once we do, borrow
	// from the reserve instead of minting new tokens
	maxAmt, _ := k.GetMimir(ctx, constants.MaxSWITCHSupply.String())
	if coin.IsSwitch() && maxAmt > 0 {
		currentSupply := k.GetTotalSupply(ctx, common.SwitchNative) // current circulating supply of switch
		maxSupply := cosmos.NewUint(uint64(maxAmt))                 // max supply of switch (ie 500m)
		...
		if availableSupply.LT(coin.Amount) {
			// Never mint an amount that would exceed MaxSWITCHSupply.
	...
	// check if we've exceeded max switch supply cap. If we have, there could
	// be an issue (infinite mint bug/exploit), or maybe runway switch
	...
	maxAmt, _ = k.GetMimir(ctx, "MaxSWITCHSupply")
...
func (k KVStore) GetNativeTxFee(ctx cosmos.Context) cosmos.Uint {
	if k.usdFeesEnabled(ctx) {
		return k.DollarConfigInSWITCH(ctx, constants.NativeTransactionFeeUSD)
	}
	fee := k.GetConfigInt64(ctx, constants.NativeTransactionFee)
	return cosmos.NewUint(uint64(fee))
}

func (k KVStore) GetSWITCHNameRegisterFee(ctx cosmos.Context) cosmos.Uint {
	if k.usdFeesEnabled(ctx) {
		return k.DollarConfigInSWITCH(ctx, constants.TNSRegisterFeeUSD)
	}
	fee := k.GetConstants().GetInt64Value(constants.TNSRegisterFee)
	return cosmos.NewUint(uint64(fee))
}

func (k KVStore) GetSWITCHNamePerBlockFee(ctx cosmos.Context) cosmos.Uint {
	if k.usdFeesEnabled(ctx) {
		return k.DollarConfigInSWITCH(ctx, constants.TNSFeePerBlockUSD)
	}
	fee := k.GetConstants().GetInt64Value(constants.TNSFeePerBlock)
	return cosmos.NewUint(uint64(fee))
}

// DollarConfigInSWITCH returns the dollar denominated config value in SWITCH. If the SWITCH
// price feed returns zero, the USD value will be returned.
func (k KVStore) DollarConfigInSWITCH(ctx cosmos.Context, value constants.ConstantName) cosmos.Uint {
	usd := cosmos.SafeUintFromInt64(k.GetConfigInt64(ctx, value))
	switchUSDPrice := k.DollarsPerSWITCH(ctx)
	if !switchUSDPrice.IsZero() {
		switchValue := usd.MulUint64(common.One).Quo(switchUSDPrice)

		// round to the first 2 significant digits of the USD price
		return cosmos.NewUint(common.RoundSignificantFigures(switchValue.Uint64(), k.GetConfigInt64(ctx, constants.FeeUSDRoundSignificantDigits)))
	}
	return usd
}
