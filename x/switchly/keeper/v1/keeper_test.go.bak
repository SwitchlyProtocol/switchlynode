...
func setupKeeperForTest(c *C) (cosmos.Context, KVStore) {
	...
	k := NewKVStore(encodingConfig.Codec, bk, ak, uk, keySwitchly, GetCurrentVersion())
...
func (KeeperTestSuit) TestKeeperVersion(c *C) {
	...
	c.Check(k.GetSWITCHBalanceOfModule(ctx, AsgardName).Equal(cosmos.NewUint(100000000*common.One)), Equals, true)
	coinsToSend := common.NewCoins(common.NewCoin(common.SwitchNative, cosmos.NewUint(1*common.One)))
...
func (KeeperTestSuit) TestMaxMint(c *C) {
	...
	// mint more switch into reserve to max the supply
	...
	// mint more switch into another module
	...
	reserve := k.GetSWITCHBalanceOfModule(ctx, ReserveName)
	mod := k.GetSWITCHBalanceOfModule(ctx, ModuleName)
