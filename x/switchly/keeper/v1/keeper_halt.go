...
func (k KVStore) IsTradingHalt(ctx cosmos.Context, msg cosmos.Msg) bool {
	...
	case *MsgSwap:
		...
		if sourceAsset.IsSWCY() {
			return k.IsSWCYTradingHalted(ctx) || k.IsChainTradingHalted(ctx, target)
		} else if m.TargetAsset.IsSWCY() {
			return k.IsSWCYTradingHalted(ctx) || k.IsChainTradingHalted(ctx, source)
		}

		return k.IsChainTradingHalted(ctx, source) || k.IsChainTradingHalted(ctx, target) || k.IsGlobalTradingHalted(ctx)
	case *MsgAddLiquidity:
		if m.Asset.IsSWCY() {
			return k.IsSWCYTradingHalted(ctx)
		}
...
func (k KVStore) IsSWCYTradingHalted(ctx cosmos.Context) bool {
	haltSWCYTrading, err := k.GetMimir(ctx, "HaltSWCYTrading")
	if err == nil && (haltSWCYTrading > 0 && haltSWCYTrading < ctx.BlockHeight()) {
		ctx.Logger().Debug("SWCY trading is halt")
		return true
	}
