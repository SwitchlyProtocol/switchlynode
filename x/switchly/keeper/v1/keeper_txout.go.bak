...
func (k KVStore) GetTxOutValue(ctx cosmos.Context, height int64) (cosmos.Uint, cosmos.Uint, error) {
	...
	switchValue, cloutValue := k.GetTOIsValue(ctx, txout.TxArray...)
	return switchValue, cloutValue, nil
}

func (k KVStore) GetTOIsValue(ctx cosmos.Context, tois ...TxOutItem) (cosmos.Uint, cosmos.Uint) {
	switchValue := cosmos.ZeroUint()
	...
	for i := range tois {
		for _, coin := range append(common.Coins{tois[i].Coin}, tois[i].MaxGas...) {
			if coin.IsSwitch() {
				switchValue = switchValue.Add(coin.Amount)
				continue
			}
			...
			switchValue = switchValue.Add(pool.AssetValueInSWITCH(coin.Amount))
		}
