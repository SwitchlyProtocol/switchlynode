...
type KeeperSWCYClaimerSuite struct{}

var _ = Suite(&KeeperSWCYClaimerSuite{})
...
func (s *KeeperSWCYClaimerSuite) TestSWCYClaimer(c *C) {
	...
	for _, claimer := range claimers {
		c.Assert(k.SetSWCYClaimer(ctx, claimer), IsNil)
	}

	for i, claimerChain := range claimerChains {
		tcyClaimer, err := k.GetSWCYClaimer(ctx, claimerChain.address, claimerChain.asset)
		c.Assert(err, IsNil)
		c.Assert(tcyClaimer.Amount.Equal(claimers[i].Amount), Equals, true)
	}
}

func (s *KeeperSWCYClaimerSuite) TestListSWCYClaimersFromL1Address(c *C) {
	ctx, k := setupKeeperForTest(c)
	...
	tcyClaims, err := k.ListSWCYClaimersFromL1Address(ctx, addr)
	c.Assert(err, IsNil)
	...
	tcyClaims, err = k.ListSWCYClaimersFromL1Address(ctx, addr2)
	c.Assert(err, IsNil)
	...
	tcyClaims, err = k.ListSWCYClaimersFromL1Address(ctx, addr3)
	c.Assert(err, IsNil)
