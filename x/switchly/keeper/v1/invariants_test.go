...
func (s *InvariantsSuite) TestAsgardInvariant(c *C) {
	...
	runeBal := k.GetSWITCHBalanceOfModule(ctx, AsgardName)
...
func (s *InvariantsSuite) TestBondInvariant(c *C) {
	...
	network := NewNetwork()
	network.BondRewardSwitch = cosmos.NewUint(2000)
	...
	expSWITCH := common.NewCoin(common.SwitchNative, cosmos.NewUint(3100))
	c.Assert(k.MintToModule(ctx, ModuleName, expSWITCH), IsNil)
	c.Assert(k.SendFromModuleToModule(ctx, ModuleName, BondName, common.NewCoins(expSWITCH)), IsNil)
