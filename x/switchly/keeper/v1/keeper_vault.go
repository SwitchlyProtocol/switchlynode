...
// value in switch, it is considered insecure and no vault is returned
				...
				if item.OutHash.IsEmpty() {
					...
					if item.VaultPubKey.Equals(vault.PubKey) {
						if item.Coin.IsSwitch() {
							...
							totalValue = common.SafeSub(totalValue, pool.AssetValueInSWITCH(item.Coin.Amount))
						}
						...
						if item.Coin.IsSwitch() {
							...
							totalValue = totalValue.Add(pool.AssetValueInSWITCH(item.Coin.Amount))
						}
					}
				}
			}
