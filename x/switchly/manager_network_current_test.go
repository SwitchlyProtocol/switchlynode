...
func (s *NetworkManagerVCURTestSuite) TestUpdateNetwork(c *C) {
	...
	// Vault for getVaultsLiquiditySWITCH.
...
func (s *NetworkManagerVCURTestSuite) TestCalcBlockRewards(c *C) {
	...
	// calcBlockRewards arguments: availablePoolsSWITCH, vaultsLiquiditySWITCH, effectiveSecurityBond, totalEffectiveBond, totalReserve, totalLiquidityFees cosmos.Uint, emissionCurve, blocksPerYear int64

	vaultsLiquiditySWITCH := cosmos.NewUint(1000 * common.One)
	availablePoolsSWITCH := vaultsLiquiditySWITCH.QuoUint64(2) // vaultsLiquiditySWITCH used for availablePoolsSWITCH usually, but *1/2 when testing different values.
	effectiveSecurityBond := cosmos.NewUint(2000 * common.One)
	// Equilibrium state where effectiveSecurityBond is double vaultsLiquiditySWITCH,
	// so expecting equal rewards for vaultsLiquiditySWITCH and the effectiveSecurityBond portion of totalEffectiveBond.
	...
	bondR, poolR, lpShare, _, _, _ := networkMgr.calcBlockRewards(ctx, vaultsLiquiditySWITCH, vaultsLiquiditySWITCH, effectiveSecurityBond, effectiveSecurityBond, totalReserve, totalLiquidityFees, emissionCurve, blocksPerYear, 0, 0, 0)
	...
	bondR, poolR, lpShare, _, _, _ = networkMgr.calcBlockRewards(ctx, vaultsLiquiditySWITCH, vaultsLiquiditySWITCH, effectiveSecurityBond, totalEffectiveBond, totalReserve, totalLiquidityFees, emissionCurve, blocksPerYear, 0, 0, 0)
	...
	c.Check(lpShare.Uint64(), Equals, uint64(3999), Commentf("%d", lpShare.Uint64())) // ~40% for availablePoolsSWITCH, ~40% for effectiveSecurityBond (equilibrium), ~60% for totalEffectiveBond

	// vaultsLiquiditySWITCH more than availablePoolsSWITCH.
	bondR, poolR, lpShare, _, _, _ = networkMgr.calcBlockRewards(ctx, availablePoolsSWITCH, vaultsLiquiditySWITCH, effectiveSecurityBond, effectiveSecurityBond, totalReserve, totalLiquidityFees, emissionCurve, blocksPerYear, 0, 0, 0)
	...
	c.Check(lpShare.Uint64(), Equals, uint64(3330), Commentf("%d", lpShare.Uint64())) // 500 availablePoolsSWITCH (1000 switch value asset+switch liquidity) is getting half the rewards of 2000 effectiveSecurityBond; same yield)
	// With totalEffectiveBond = 1.5 * effectiveSecurityBond:
	bondR, poolR, lpShare, _, _, _ = networkMgr.calcBlockRewards(ctx, availablePoolsSWITCH, vaultsLiquiditySWITCH, effectiveSecurityBond, totalEffectiveBond, totalReserve, totalLiquidityFees, emissionCurve, blocksPerYear, 0, 0, 0)
	...
	c.Check(lpShare.Uint64(), Equals, uint64(2498), Commentf("%d", lpShare.Uint64())) // 500 availablePoolsSWITCH (1000 switch value asset+switch liquidity) is getting a third the rewards of 3000 totalEffectiveBond; same yield)

	// Liquidity fees non-zero.
	bondR, poolR, lpShare, _, _, _ = networkMgr.calcBlockRewards(ctx, vaultsLiquiditySWITCH, vaultsLiquiditySWITCH, effectiveSecurityBond, effectiveSecurityBond, totalReserve, cosmos.NewUint(3000), emissionCurve, blocksPerYear, 0, 0, 0)
	...
	// With totalEffectiveBond = 1.5 * effectiveSecurityBond:
	bondR, poolR, lpShare, _, _, _ = networkMgr.calcBlockRewards(ctx, vaultsLiquiditySWITCH, vaultsLiquiditySWITCH, effectiveSecurityBond, totalEffectiveBond, totalReserve, cosmos.NewUint(3000), emissionCurve, blocksPerYear, 0, 0, 0)
	...
	bondR, poolR, lpShare, _, _, _ = networkMgr.calcBlockRewards(ctx, vaultsLiquiditySWITCH, vaultsLiquiditySWITCH, effectiveSecurityBond, effectiveSecurityBond, cosmos.ZeroUint(), totalLiquidityFees, emissionCurve, blocksPerYear, 0, 0, 0)
	...
	// With totalEffectiveBond = 1.5 * effectiveSecurityBond:
	bondR, poolR, lpShare, _, _, _ = networkMgr.calcBlockRewards(ctx, vaultsLiquiditySWITCH, vaultsLiquiditySWITCH, effectiveSecurityBond, totalEffectiveBond, cosmos.ZeroUint(), totalLiquidityFees, emissionCurve, blocksPerYear, 0, 0, 0)
	c.Check(bondR.Uint64(), Equals, uint64(0), Commentf("%d", bondR.Uint64()))
	effectiveSecurityBondR = bondR.Mul(effectiveSecurityBond).Quo(totalEffectiveBond)
	...
	c.Check(bondR.Uint64(), Equals, uint64(3171), Commentf("%d", bondR.Uint64()))
	c.Check(poolR.Uint64(), Equals, uint64(0), Commentf("%d", poolR.Uint64()))
	c.Check(lpShare.Uint64(), Equals, uint64(0), Commentf("%d", lpShare.Uint64())) // No pools are capable of receiving rewards, so should not transfer any SWITCH to the Pool Module (broken invariant).
	// With totalEffectiveBond = 1.5 * effectiveSecurityBond:
	...
	c.Check(lpShare.Uint64(), Equals, uint64(6667), Commentf("%d", lpShare.Uint64())) // ~67% for availablePoolsSWITCH, ~22% for effectiveSecurityBond (equilibrium), ~33% for totalEffectiveBond
	...
	c.Check(lpShare.Uint64(), Equals, uint64(5002), Commentf("%d", lpShare.Uint64())) // 500 availablePoolsSWITCH (1000 switch value asset+switch liquidity) is getting half the rewards of 3000 totalEffectiveBond; 3/2 higher yield)
	...
	// Provided liquidity equal to effectiveSecurityBond (still pool rewards since less than totalEffectiveSWITCH).
	bondR, poolR, lpShare, _, _, _ = networkMgr.calcBlockRewards(ctx, vaultsLiquiditySWITCH, vaultsLiquiditySWITCH, effectiveSecurityBond, effectiveSecurityBond, totalReserve, totalLiquidityFees, emissionCurve, blocksPerYear, 0, 0, 0)
	c.Check(bondR.Uint64(), Equals, uint64(3171), Commentf("%d", bondR.Uint64()))
	c.Check(poolR.Uint64(), Equals, uint64(0), Commentf("%d", poolR.Uint64()))
	c.Check(lpShare.Uint64(), Equals, uint64(0), Commentf("%d", lpShare.Uint64()))
	...
	effectiveSecurityBond = cosmos.NewUint(2000 * common.One) // Resetting from the half-size comparison.
	c.Check(vaultsLiquiditySWITCH.String(), Equals, cosmos.NewUint(1000*common.One).String())
	c.Check(availablePoolsSWITCH.String(), Equals, cosmos.NewUint(500*common.One).String())

	bondR, poolR, _, devFundDeduct, systemIncomeBurnDeduct, tcyStakeDeduct := networkMgr.calcBlockRewards(ctx, availablePoolsSWITCH, vaultsLiquiditySWITCH, effectiveSecurityBond, totalEffectiveBond, totalReserve, totalLiquidityFees, emissionCurve, blocksPerYear, 0, 0, 0)
	...
	e12PoolYield := poolR.MulUint64(1e12).Quo(availablePoolsSWITCH.MulUint64(2)) // The pool liquidity experiencing yield is the total value, both Asset and SWITCH depths.
	...
	bondR, poolR, _, _, _, _ = networkMgr.calcBlockRewards(ctx, availablePoolsSWITCH, vaultsLiquiditySWITCH, effectiveSecurityBond, totalEffectiveBond, totalReserve, totalLiquidityFees, emissionCurve, blocksPerYear, 0, 0, 0)
	e12BondYield = bondR.MulUint64(1e12).Quo(totalEffectiveBond)
	e12PoolYield = poolR.MulUint64(1e12).Quo(availablePoolsSWITCH.MulUint64(2)) // The pool liquidity experiencing yield is the total value, both Asset and SWITCH depths.
	...
	// Equilbrium yield when vaultsLiquiditySWITCH is 3/2 greater:
	bondR, poolR, _, _, _, _ = networkMgr.calcBlockRewards(ctx, availablePoolsSWITCH, vaultsLiquiditySWITCH.MulUint64(3).QuoUint64(2), effectiveSecurityBond, totalEffectiveBond, totalReserve, totalLiquidityFees, emissionCurve, blocksPerYear, 0, 0, 0)
	e12BondYield = bondR.MulUint64(1e12).Quo(totalEffectiveBond)
	e12PoolYield = poolR.MulUint64(1e12).Quo(availablePoolsSWITCH.MulUint64(2)) // The pool liquidity experiencing yield is the total value, both Asset and SWITCH depths.
	c.Check(e12BondYield.String(), Equals, "7930")
	c.Check(e12PoolYield.String(), Equals, "7920")
	...
	// Equilbrium yield when availablePoolsSWITCH is doubled:
	bondR, poolR, _, _, _, _ = networkMgr.calcBlockRewards(ctx, availablePoolsSWITCH.MulUint64(2), vaultsLiquiditySWITCH, effectiveSecurityBond, totalEffectiveBond, totalReserve, totalLiquidityFees, emissionCurve, blocksPerYear, 0, 0, 0)
	e12BondYield = bondR.MulUint64(1e12).Quo(totalEffectiveBond)
	e12PoolYield = poolR.MulUint64(1e12).Quo(availablePoolsSWITCH.MulUint64(2).MulUint64(2)) // The (doubled) pool liquidity experiencing yield is the total value, both Asset and SWITCH depths.
	c.Check(e12BondYield.String(), Equals, "6343")
	c.Check(e12PoolYield.String(), Equals, "6340")
	c.Check(e12BondYield.QuoUint64(100).String(), Equals, e12PoolYield.QuoUint64(100).String())

	// No change when ignoring L1 Assets when vaultsLiquiditySWITCH is equally increased in order to increase availablePoolsSWITCH:
	bondR, poolR, _, _, _, _ = networkMgr.calcBlockRewards(ctx, availablePoolsSWITCH.MulUint64(2), vaultsLiquiditySWITCH.Add(availablePoolsSWITCH), effectiveSecurityBond, totalEffectiveBond, totalReserve, totalLiquidityFees, emissionCurve, blocksPerYear, 0, 0, 0)
	e12BondYield = bondR.MulUint64(1e12).Quo(totalEffectiveBond)
	e12PoolYield = poolR.MulUint64(1e12).Quo(availablePoolsSWITCH.MulUint64(2).MulUint64(2)) // The (doubled) pool liquidity experiencing yield is the total value, both Asset and SWITCH depths.
	c.Check(e12BondYield.String(), Equals, "6343")
	c.Check(e12PoolYield.String(), Equals, "6340")
	...
	// Equilbrium yield when availablePoolsSWITCH is tripled:
	bondR, poolR, _, _, _, _ = networkMgr.calcBlockRewards(ctx, availablePoolsSWITCH.MulUint64(3), vaultsLiquiditySWITCH, effectiveSecurityBond, totalEffectiveBond, totalReserve, totalLiquidityFees, emissionCurve, blocksPerYear, 0, 0, 0)
	e12BondYield = bondR.MulUint64(1e12).Quo(totalEffectiveBond)
	e12PoolYield = poolR.MulUint64(1e12).Quo(availablePoolsSWITCH.MulUint64(3).MulUint64(2)) // The (tripled) pool liquidity experiencing yield is the total value, both Asset and SWITCH depths.
...
func (*NetworkManagerVCURTestSuite) TestGetAvailablePoolsSWITCH(c *C) {
	ctx, mgr := setupManagerForTest(c)
	helper := NewVaultGenesisSetupTestHelper(mgr.Keeper())
	mgr.K = helper
	networkMgr := newNetworkMgrVCUR(helper, mgr.TxOutStore(), mgr.EventMgr())
	...
	p.Status = PoolAvailable
	c.Assert(helper.SetPool(ctx, p), IsNil)
	pools, totalLiquidity, err := getAvailablePoolsSWITCH(ctx, networkMgr.k)
	c.Assert(err, IsNil)
...
func (s *NetworkManagerVCURTestSuite) TestRagnarokPool(c *C) {
	...
	lps := LiquidityProviders{
		...
		{
			Asset:              common.BTCAsset,
			SwitchAddress:      GetRandomSwitchAddress(),
			AssetAddress:       GetRandomBTCAddress(),
			LastAddHeight:      10,
			Units:              btcPool.LPUnits.QuoUint64(2),
			PendingSwitch:      cosmos.ZeroUint(),
...
func (*NetworkManagerVCURTestSuite) TestPOLLiquidityWithdraw(c *C) {
	...
	lps := LiquidityProviders{
		{
			Asset:              btcPool.Asset,
			SwitchAddress:      GetRandomETHAddress(),
			AssetAddress:       GetRandomBTCAddress(),
			LastAddHeight:      5,
			Units:              btcPool.LPUnits.QuoUint64(2),
			PendingSwitch:      cosmos.ZeroUint(),
	...
	// To withdraw max 1% (100 basis points) of the pool SWITCH depth, asymmetrically withdraw as SWITCH 0.5% of all pool units.
	...
	// To withdraw 0.5% of the pool SWITCH depth, asymmetrically withdraw as SWITCH 0.25% of all pool units.
...
func (*NetworkManagerVCURTestSuite) TestFairMergePOLCycle(c *C) {
	...
	c.Assert(err, IsNil)
	c.Assert(pol.SwitchDeposited.Uint64(), Equals, uint64(0))
	c.Assert(pol.SwitchWithdrawn.Uint64(), Equals, uint64(0))
	...
	// cycle should succeed, still no switch deposited since max is 0
	err = net.POLCycle(ctx, mgr)
	c.Assert(err, IsNil)
	...
	pol, err = mgr.Keeper().GetPOL(ctx)
	c.Assert(err, IsNil)
	c.Assert(pol.SwitchDeposited.String(), Equals, "0")
	c.Assert(pol.SwitchWithdrawn.String(), Equals, "0")
	...
	pol, err = mgr.Keeper().GetPOL(ctx)
	c.Assert(err, IsNil)
	c.Assert(pol.SwitchDeposited.String(), Equals, "200000000")
	c.Assert(pol.SwitchWithdrawn.String(), Equals, "0")
	...
	c.Assert(err, IsNil)
	c.Assert(pol.SwitchDeposited.String(), Equals, "200000000")
	c.Assert(pol.SwitchWithdrawn.String(), Equals, "199395540")
	...
	pol, err = mgr.Keeper().GetPOL(ctx)
	c.Assert(err, IsNil)
	c.Assert(pol.SwitchDeposited.String(), Equals, "400006044")
	c.Assert(pol.SwitchWithdrawn.String(), Equals, "199395540")

	// withdraw entire pol position 1 basis point of switch depth at a time
	mgr.Keeper().SetMimir(ctx, constants.POLTargetSynthPerPoolDepth.String(), 10000)
	...
	c.Assert(err, IsNil)
	c.Assert(pol.SwitchDeposited.String(), Equals, "400006044")
	c.Assert(pol.SwitchWithdrawn.String(), Equals, "199415528")
	// another basis point
	err = net.POLCycle(ctx, mgr)
	c.Assert(err, IsNil)
	pol, err = mgr.Keeper().GetPOL(ctx)
	c.Assert(err, IsNil)
	c.Assert(pol.SwitchDeposited.String(), Equals, "400006044")
	c.Assert(pol.SwitchWithdrawn.String(), Equals, "199435514")
	...
	// any smaller buffer should withdraw one basis point of switch
	...
	c.Assert(err, IsNil)
	c.Assert(pol.SwitchDeposited.String(), Equals, "400006044")
	c.Assert(pol.SwitchWithdrawn.String(), Equals, "199455500")
	...
	c.Assert(err, IsNil)
	c.Assert(pol.SwitchDeposited.String(), Equals, "400006044")
	c.Assert(pol.SwitchWithdrawn.String(), Equals, "398797134")
...
func (s *NetworkManagerVCURTestSuite) TestSpawnDerivedAssets(c *C) {
	...
	mgr.Keeper().SetMimir(ctx, "SwitchlyAnchor-ETH-BUSD-BD1", 1) // enable BUSD pool as a SWITCHLY anchor
	...
	// should not have any affect on SWITCHLY.BNB
	bscPool := NewPool()
	...
	c.Assert(err, IsNil)
	usd, err := mgr.Keeper().GetPool(ctx, common.SWITCHLY)
	c.Assert(err, IsNil)
...
func (s *NetworkManagerVCURTestSuite) TestSpawnDerivedAssetsBasisPoints(c *C) {
	...
	mgr.Keeper().SetMimir(ctx, "SwitchlyAnchor-ETH-BUSD-BD1", 1) // enable ETH.BUSD pool as a SWITCHLY anchor
	ethBusd, err := common.NewAsset("ETH.BUSD-BD1")
	c.Assert(err, IsNil)
...
func (s *NetworkManagerVCURTestSuite) TestDistributeSWCYStake(c *C) {
	ctx, mgr := setupManagerForTest(c)
	mgr.K.SetMimir(ctx, "SWCYStakeDistributionHalt", 0)
	nmgr := newNetworkMgrVCUR(mgr.Keeper(), NewTxStoreDummy(), NewDummyEventMgr())
	ver := GetCurrentVersion()
	constAccessor := constants.GetConstantValues(ver)

	address1 := GetRandomSwitchAddress()
	acc1, err := address1.AccAddress()
	c.Assert(err, IsNil)
	address2 := GetRandomSwitchAddress()
	acc2, err := address2.AccAddress()
	c.Assert(err, IsNil)
	address3 := GetRandomSwitchAddress()
	acc3, err := address3.AccAddress()
	c.Assert(err, IsNil)
	address4 := GetRandomSwitchAddress()
	acc4, err := address4.AccAddress()
	c.Assert(err, IsNil)
	tcyStakeAddress := mgr.Keeper().GetModuleAccAddress(SWCYStakeName)

	// Add SWCYStaker to accounts -> acc1 ~ 75%, acc2 ~ 25%, acc4 = less than MinSWCYForSWCYStakeDistribution
	amountAddr1 := cosmos.NewUint(157_499_999_99950000)
	err = mgr.Keeper().SetSWCYStaker(ctx, SWCYStaker{
		Address: address1,
		Amount:  amountAddr1,
	})
	c.Assert(err, IsNil)
	...
	// Set staking amount less than MinSWCYForSWCYStakeDistribution to address 4
	amt := constAccessor.GetInt64Value(constants.MinSWCYForSWCYStakeDistribution) - 1
	...
	stakingModuleSWCYAmount := amountAddr1.Add(amountAddr2).Add(amountAddr4)
	coin := common.NewCoin(common.SWCY, stakingModuleSWCYAmount)
	err = mgr.Keeper().MintToModule(ctx, ModuleName, coin)
	c.Assert(err, IsNil)
	err = mgr.Keeper().SendFromModuleToModule(ctx, ModuleName, SWCYStakeName, common.NewCoins(coin))
	c.Assert(err, IsNil)

	c.Assert(mgr.Keeper().SWCYStakerExists(ctx, address1), Equals, true)
	c.Assert(mgr.Keeper().SWCYStakerExists(ctx, address2), Equals, true)
	c.Assert(mgr.Keeper().SWCYStakerExists(ctx, address3), Equals, true)
	c.Assert(mgr.Keeper().SWCYStakerExists(ctx, address4), Equals, true)

	// Mint less than MinSWITCHForSWCYStakeDistribution on SWCYStakeName
	tcyStakeFeeAmount := constAccessor.GetInt64Value(constants.MinSWITCHForSWCYStakeDistribution) - 1
	FundModule(c, ctx, mgr.Keeper(), SWCYStakeName, uint64(tcyStakeFeeAmount))

	nmgr.distributeSWCYStake(ctx, mgr)

	// Check balances, accounts should not receive funds from SWCYStake since funds
	// are less than MinSWITCHForSWCYStakeDistribution
	...
	balanceSWCY := mgr.Keeper().GetBalanceOf(ctx, tcyStakeAddress, common.SwitchNative)
	c.Assert(balanceSWCY.Amount.Equal(math.NewInt(tcyStakeFeeAmount)), Equals, true)
	c.Assert(balanceSWCY.Denom, Equals, common.SwitchNative.Native())
	...
	// Mint 210M SWITCH to SWCYStakeName (fund already has MinSWITCHForSWCYStakeDistribution)
	tcyStakeFeeAmount = 210_000_000_00000000 - tcyStakeFeeAmount
	c.Assert(tcyStakeFeeAmount > 0, Equals, true)
	FundModule(c, ctx, mgr.Keeper(), SWCYStakeName, uint64(tcyStakeFeeAmount))
	balanceSWCY = mgr.Keeper().GetBalanceOf(ctx, tcyStakeAddress, common.SwitchNative)
	c.Assert(balanceSWCY.Amount.Equal(math.NewInt(210_000_000_00000000)), Equals, true)
	...
	c.Assert(mgr.Keeper().SWCYStakerExists(ctx, address3), Equals, false)
	c.Assert(mgr.Keeper().SWCYStakerExists(ctx, address4), Equals, true)
	...
	// SWCYStake should not have funds after the distribution
	balanceAcc1 = mgr.Keeper().GetBalanceOf(ctx, acc1, common.SwitchNative)
	...
	balanceClaiming := mgr.Keeper().GetBalanceOfModule(ctx, SWCYClaimingName, common.SwitchNative.Native())
	c.Assert(balanceClaiming.Equal(math.NewUint(99999)), Equals, true)

	balanceSWCY = mgr.Keeper().GetBalanceOf(ctx, tcyStakeAddress, common.SwitchNative)
	c.Assert(balanceSWCY.Amount.IsZero(), Equals, true)

	// Move acc1, acc2 and claiming module SWITCH balances to zero
	...
	err = mgr.Keeper().SendFromModuleToModule(ctx, SWCYClaimingName, ModuleName, common.NewCoins(coin))
	c.Assert(err, IsNil)
	balanceClaiming = mgr.Keeper().GetBalanceOfModule(ctx, SWCYClaimingName, common.SwitchNative.Native())
	...
	// Mint 420M SWITCH to SWCYStakeName
	...
	// 25% to acc2, 25% to acc3 and acc4 should not receive switch.
	// SWCYStake should not have funds after the distribution
	balanceAcc1 = mgr.Keeper().GetBalanceOf(ctx, acc1, common.SwitchNative)
...
func (s *NetworkManagerVCURTestSuite) TestGetSWCYStakeAmountToDistribute(c *C) {
	_, mgr := setupManagerForTest(c)
	nmgr := newNetworkMgrVCUR(mgr.Keeper(), NewTxStoreDummy(), NewDummyEventMgr())
	ver := GetCurrentVersion()
	constAccessor := constants.GetConstantValues(ver)
	minSWITCH := constAccessor.GetInt64Value(constants.MinSWITCHForSWCYStakeDistribution)

	// If funds are less than MinSWITCHForSWCYStakeDistribution it should NOT distribute that amount
	amount := uint64(minSWITCH - 1)
	tcyStakeAmount := cosmos.NewUint(amount)
	result := nmgr.getSWCYStakeAmountToDistribute(tcyStakeAmount, minSWITCH)
	c.Assert(result.IsZero(), Equals, true)

	// If funds are equal to MinSWITCHForSWCYStakeDistribution it should distribute that amount
	amount = uint64(minSWITCH)
	tcyStakeAmount = cosmos.NewUint(amount)
	result = nmgr.getSWCYStakeAmountToDistribute(tcyStakeAmount, minSWITCH)
	c.Assert(result.IsZero(), Equals, false)
	c.Assert(result.Equal(tcyStakeAmount), Equals, true)

	// If funds are equal to 2x MinSWITCHForSWCYStakeDistribution it should distribute that amount
	amount = uint64(minSWITCH * 2)
	tcyStakeAmount = cosmos.NewUint(amount)
	result = nmgr.getSWCYStakeAmountToDistribute(tcyStakeAmount, minSWITCH)
	c.Assert(result.IsZero(), Equals, false)
	c.Assert(result.Equal(tcyStakeAmount), Equals, true)

	// If funds are equal to 2.5x MinSWITCHForSWCYStakeDistribution it should only distribute 2x
	amount = uint64(float64(minSWITCH) * 2.5)
	amoutMul2 := uint64(minSWITCH * 2)
	tcyStakeAmount = cosmos.NewUint(amount)
	result = nmgr.getSWCYStakeAmountToDistribute(tcyStakeAmount, minSWITCH)
	c.Assert(result.IsZero(), Equals, false)
