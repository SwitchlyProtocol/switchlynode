...
func (vm *NetworkMgrVCUR) suspendVirtualPool(ctx cosmos.Context, mgr Manager, derivedAsset common.Asset, suspendReasonErr error) {
	...
	if derivedPool.Status != PoolSuspended {
		...
		telemetry.IncrCounterWithLabels(
			[]string{"switchlynode", "derived_asset", "suspended"},
			float32(1),
...
// equivalent length with their pool depths in SWITCH.
func (vm *NetworkMgrVCUR) GetAvailableAnchorsAndDepths(
	...
	// gather anchor assets with available pools and their switch depths
	availableAnchors := make([]common.Asset, 0)
	switchDepths := make([]cosmos.Uint, 0)
	for _, anchorAsset := range mgr.Keeper().GetAnchors(ctx, asset) {
		...
		switchDepths = append(switchDepths, p.BalanceSwitch)
	}

	return availableAnchors, switchDepths
}
...
func (vm *NetworkMgrVCUR) CalcAnchor(ctx cosmos.Context, mgr Manager, asset common.Asset) (cosmos.Uint, cosmos.Uint, cosmos.Uint) {
	...
	totalSWITCHDepth := cosmos.Sum(slipsDepths)
	...
	return totalSWITCHDepth, price, weightedMeanSlip
}
...
func (vm *NetworkMgrVCUR) spawnDerivedAssets(ctx cosmos.Context, mgr Manager) error {
	...
	layer1Assets := []common.Asset{common.SWITCHLY}
...
func (vm *NetworkMgrVCUR) SpawnDerivedAsset(ctx cosmos.Context, asset common.Asset, mgr Manager) {
	var err error
	layer1Asset := asset
	if layer1Asset.IsDerivedAsset() && !asset.Equals(common.SWITCHLY) {
	...
	if !asset.Equals(common.SWITCHLY) && !layer1Asset.IsGasAsset() {
		return
	}
	...
	if layer1Pool.IsEmpty() && !asset.Equals(common.SWITCHLY) {
		return
	}
	...
	totalSWITCHDepth, price, slippage := vm.CalcAnchor(ctx, mgr, layer1Asset)
	if totalSWITCHDepth.IsZero() {
	...
	if derivedPool.Status != PoolAvailable || derivedPool.StatusSince == 0 {
		...
		telemetry.IncrCounterWithLabels(
			[]string{"switchlynode", "derived_asset", "available"},
			float32(1),
			[]metrics.Label{telemetry.NewLabel("pool", derivedPool.Asset.String())},
		)
	}

	minSWITCHDepth := common.GetSafeShare(cosmos.NewUint(uint64(minDepthPts)), cosmos.NewUint(10000), totalSWITCHDepth)
	switchDepth := common.GetUncappedShare(cosmos.NewUint(uint64(depthBasisPts)), cosmos.NewUint(10000), totalSWITCHDepth)
	// adjust switch depth by median slippage. This is so high volume trading
	...
	switchDepth = common.GetSafeShare(reverseSlip, cosmos.NewUint(uint64(maxAnchorSlip)), switchDepth)
	if switchDepth.LT(minSWITCHDepth) {
		switchDepth = minSWITCHDepth
	}
	assetDepth := switchDepth.Mul(price).QuoUint64(uint64(constants.DollarMulti * common.One))

	// emit an event for midgard
	switchAmt := common.SafeSub(switchDepth, derivedPool.BalanceSwitch)
	...
	if derivedPool.BalanceSwitch.GT(switchDepth) {
		runeAdd = false
		switchAmt = common.SafeSub(derivedPool.BalanceSwitch, switchDepth)
	}

	// Only emit an EventPoolBalanceChanged if there's a balance change.
	if !assetAmt.IsZero() || !switchAmt.IsZero() {
		mod := NewPoolMod(derivedPool.Asset, switchAmt, runeAdd, assetAmt, assetAdd)
		emitPoolBalanceChangedEvent(ctx, mod, "derived pool adjustment", mgr)

		derivedPool.BalanceAsset = assetDepth
		derivedPool.BalanceSwitch = switchDepth
	}
...
func (vm *NetworkMgrVCUR) EndBlock(ctx cosmos.Context, mgr Manager) error {
	...
	if IsPeriodLastBlock(ctx, blocksPerDay) {
		vm.distributeSWCYStake(ctx, mgr)
	}
	return nil
}
	...
	for _, vault := range retiring {
		...
		if (ctx.BlockHeight()-vault.StatusSince)%migrateInterval == 0 {
			for _, coin := range availableCoins {
				// non-native switch assets are no migrated, therefore they are
				// burned in each churn
				if coin.IsNative() {
					continue
				}
				// ERC20 SWITCH will be burned when it reach router contract
				...
				if coin.Asset.Equals(gasAsset) {
					...
					if amt.LTE(dustThreshold) && nth > migrationRounds {
						...
						switchAmt := p.AssetValueInSWITCH(coin.Amount)
						if !switchAmt.IsZero() {
							if err := vm.k.SendFromModuleToModule(ctx, ReserveName, AsgardName, common.NewCoins(common.NewCoin(common.SwitchNative, switchAmt))); err != nil {
								return fmt.Errorf("fail to transfer SWITCH from reserve to asgard,err:%w", err)
							}
						}
						p.BalanceSwitch = p.BalanceSwitch.Add(switchAmt)
						...
						emitPoolBalanceChangedEvent(ctx,
							NewPoolMod(p.Asset, switchAmt, true, coin.Amount, false),
							"burn dust",
							mgr)
						continue
					}
...
// paySaverYield - takes a pool asset and total switch collected in yield to the pool, then pays out savers their proportion of yield based on its size (relative to dual side LPs) and the SynthYieldBasisPoints
func (vm *NetworkMgrVCUR) paySaverYield(ctx cosmos.Context, asset common.Asset, switchAmt cosmos.Uint) error {
	...
	assetAmt := pool.SWITCHValueInAsset(switchAmt)
	...
	tx := common.NewTx(common.BlankTxID, modAddress, asgardAddress, common.NewCoins(coin), nil, "SWITCHLY-SAVERS-YIELD")
...
func (vm *NetworkMgrVCUR) POLCycle(ctx cosmos.Context, mgr Manager) error {
	...
	for idx, pool := range pools {
		...
		// detect if we need to withdraw switch
...
func (vm *NetworkMgrVCUR) addPOLLiquidity(
	...
	switchAmt := common.GetSafeShare(move, cosmos.NewUint(1000_000), pool.BalanceSwitch)
	if switchAmt.IsZero() {
		return nil
	}
	coins := common.NewCoins(common.NewCoin(common.SwitchNative, switchAmt))
	...
	if !switchPool.TotalUnits().IsZero() {
		...
		depositUnits = common.GetSafeShare(switchAmt, switchPoolValue, switchPool.TotalUnits())
	}

	// check balance
	bal := mgr.Keeper().GetSWITCHBalanceOfModule(ctx, ReserveName)
	if switchAmt.GT(bal) {
		return nil
	}
	...
	tx := common.NewTx(common.BlankTxID, polAddress, asgardAddress, coins, nil, "SWITCHLY-POL-ADD")
	msg := NewMsgAddLiquidity(tx, pool.Asset, switchAmt, cosmos.ZeroUint(), polAddress, common.NoAddress, common.NoAddress, cosmos.ZeroUint(), signer)
	_, err = handler(ctx, msg)
	if err != nil {
		// revert the switch back to the reserve
	...
	// deposit was successful, update the switch pool units accordingly
	switchPool.ReserveUnits = switchPool.ReserveUnits.Add(depositUnits)
	mgr.Keeper().SetSwitchPool(ctx, switchPool)

	// rebalance ownership from reserve to poolers if able
	err = reserveExitSwitchPool(ctx, mgr)
	if err != nil {
		return fmt.Errorf("fail to exit switchpool: %s", err)
	}
	return nil
}
...
func (vm *NetworkMgrVCUR) removePOLLiquidity(
	...
	lpSWITCH := common.GetSafeShare(lp.Units, pool.GetPoolUnits(), pool.BalanceSwitch).MulUint64(2)
	basisPts := common.GetSafeShare(switchAmt, lpSWITCH, maxBps)
	...
	// adjust switch amount to reflect basis points of withdraw
	switchAmt = common.GetSafeShare(basisPts, maxBps, lpSWITCH)

	// switch pool tracks the reserve and pooler unit shares of pol
	switchPool, err := mgr.Keeper().GetSwitchPool(ctx)
	if err != nil {
		return fmt.Errorf("fail to get switchpool: %s", err)
	}

	// reserve acquires corresponding switchpool units that will be withdrawn
	switchPoolValue, err := switchPoolValue(ctx, mgr)
	if err != nil {
		return fmt.Errorf("fail to get switch pool value: %s", err)
	}
	reserveSwitchPoolValue := common.GetSafeShare(switchPool.ReserveUnits, switchPool.TotalUnits(), switchPoolValue)
	if reserveSwitchPoolValue.LT(switchAmt) {
		rebalanceSWITCH := common.SafeSub(switchAmt, reserveSwitchPoolValue)
		err = reserveEnterSwitchPool(ctx, mgr, rebalanceSWITCH)
		if err != nil {
			return fmt.Errorf("fail to enter switchpool: %s", err)
		}

		// fetch updated switchpool
		switchPool, err = mgr.Keeper().GetSwitchPool(ctx)
		if err != nil {
			return fmt.Errorf("fail to get switchpool: %s", err)
		}
	...
	withdrawUnits := common.GetSafeShare(switchAmt, switchPoolValue, switchPool.TotalUnits())
	coins := common.NewCoins(common.NewCoin(common.SwitchNative, cosmos.ZeroUint()))
	tx := common.NewTx(common.BlankTxID, polAddress, asgardAddress, coins, nil, "SWITCHLY-POL-REMOVE")
	...
	// withdraw was successful, update the switchpool units accordingly
	switchPool.ReserveUnits = common.SafeSub(switchPool.ReserveUnits, withdrawUnits)
	mgr.Keeper().SetSwitchPool(ctx, switchPool)
...
func (vm *NetworkMgrVCUR) withdrawLPs(ctx cosmos.Context, pool Pool, na NodeAccount, mgr Manager) (done bool) {
	...
	for ; iterator.Valid(); iterator.Next() {
		...
		if lp.Units.IsZero() && lp.PendingAsset.IsZero() && lp.PendingSwitch.IsZero() {
			vm.k.RemoveLiquidityProvider(ctx, lp)
			continue
		}
		...
		if !lp.SwitchAddress.IsEmpty() {
			withdrawAddr = lp.SwitchAddress
			// if liquidity provider only add SWITCH , then asset address will be empty
			if lp.AssetAddress.IsEmpty() {
				withdrawAsset = common.SwitchNative
			}
		} else {
			// if liquidity provider only add Asset, then SWITCH Address will be empty
		...
		ctx.Logger().Info("ragnarok LP", "pool", pool.Asset, "rune_address", lp.SwitchAddress, "asset_address", lp.AssetAddress)
		_, err := handler(ctx, withdrawMsg)
		if err != nil {
			ctx.Logger().Error(
				"fail to withdraw, remove LP",
				"liquidity provider", lp.SwitchAddress,
...
func (vm *NetworkMgrVCUR) withdrawLiquidity(ctx cosmos.Context, pool Pool, na NodeAccount, mgr Manager) error {
	...
	if pool.Status == PoolAvailable {
		// redeem all synth asset from the pool, and send SWITCH to reserve
		...
		// Before updating the Status, get the pool with deducted BalanceSwitch from synth redemption.
	...
	// If any SWITCH remains in the pool (such as if the last withdraw were Asset-address-only),
	// transfer it to the Reserve to prevent broken-invariant Pool Module oversolvency.
	remainingSWITCH := common.NewCoin(common.SwitchNative, pool.BalanceSwitch)
	pool.BalanceSwitch = cosmos.ZeroUint()
	remainingSWITCH.Amount = remainingSWITCH.Amount.Add(pool.PendingInboundSwitch)
	pool.PendingInboundSwitch = cosmos.ZeroUint()
	if !remainingSWITCH.IsEmpty() {
		if err := vm.k.SendFromModuleToModule(ctx, AsgardName, ReserveName, common.NewCoins(remainingSWITCH)); err != nil {
			// Still proceed to suspend the pool, but log the error.
			ctx.Logger().Error("fail to transfer remaining pool ragnarok switch from asgard to reserve", "error", err)
		}
	}
...
func (vm *NetworkMgrVCUR) UpdateNetwork(ctx cosmos.Context, constAccessor constants.ConstantValues, gasManager GasManager, eventMgr EventManager) error {
	...
	totalReserve := vm.k.GetSWITCHBalanceOfModule(ctx, ReserveName)

	// when total reserve is zero , can't pay reward
	if totalReserve.IsZero() {
		return nil
	}
	availablePools, availablePoolsSWITCH, err := getAvailablePoolsSWITCH(ctx, vm.k)
	if err != nil {
		return fmt.Errorf("fail to get available pools and their switch: %w", err)
	}
	vaultsLiquiditySWITCH, err := getVaultsLiquiditySWITCH(ctx, vm.k)
	if err != nil {
		return fmt.Errorf("fail to get vaults liquidity switch: %w", err)
	}

	// If no SWITCH is in Available pools, then don't give out block rewards.
	if availablePoolsSWITCH.IsZero() {
		return nil // If no SWITCH is in available pools, then don't give out block rewards.
	}
	...
	swcyStakeSystemIncomeBps := vm.k.GetConfigInt64(ctx, constants.SWCYStakeSystemIncomeBps)
	blocksPerYear := constAccessor.GetInt64Value(constants.BlocksPerYear)
	bondReward, totalPoolRewards, lpShare, devFundDeduct, systemIncomeBurnDeduct, swcyStakeDeduct := vm.calcBlockRewards(ctx,
		availablePoolsSWITCH, vaultsLiquiditySWITCH, effectiveSecurityBond,
		totalEffectiveBond, totalReserve, totalLiquidityFees, emissionCurve,
		blocksPerYear, devFundSystemIncomeBps, systemIncomeBurnRateBps, swcyStakeSystemIncomeBps)
	...
	if !devFundDeduct.IsZero() {
		...
		coin := common.NewCoin(common.SwitchNative, devFundDeduct)
	...
	if !systemIncomeBurnDeduct.IsZero() {
		coin := common.NewCoin(common.SwitchNative, systemIncomeBurnDeduct)
		// Burn system income
		...
		// Decrement the MaxSWITCHSupply mimir by the amount of SWITCH burnt
		currentMaxSWITCHSupply := cosmos.SafeUintFromInt64(vm.k.GetConfigInt64(ctx, constants.MaxSWITCHSupply))
		currentMaxSWITCHSupply = currentMaxSWITCHSupply.Sub(systemIncomeBurnDeduct)
		vm.k.SetMimir(ctx, constants.MaxSWITCHSupply.String(), int64(currentMaxSWITCHSupply.Uint64()))
	}

	if !swcyStakeDeduct.IsZero() {
		coin := common.NewCoin(common.SwitchNative, swcyStakeDeduct)
		if err := vm.k.SendFromModuleToModule(ctx, ReserveName, SWCYStakeName, common.NewCoins(coin)); err != nil {
			return fmt.Errorf("fail to transfer funds from reserve to swcy fund: %w", err)
		}
	}
	...
	if !totalPoolRewards.IsZero() { // If Pool Rewards to hand out
		...
		for _, pool := range availablePools {
			...
			if totalLiquidityFees.IsZero() {
				amt = common.GetSafeShare(pool.BalanceSwitch, availablePoolsSWITCH, totalPoolRewards)
	...
	if !bondReward.IsZero() {
		coin := common.NewCoin(common.SwitchNative, bondReward)
	...
	network.BondRewardSwitch = network.BondRewardSwitch.Add(bondReward) // Add here for individual Node collection later

	rewardEvt := NewEventRewards(bondReward, evtPools, devFundDeduct, systemIncomeBurnDeduct, swcyStakeDeduct)
...
func (vm *NetworkMgrVCUR) payPoolRewards(ctx cosmos.Context, poolRewards []cosmos.Uint, pools Pools) error {
	for i, reward := range poolRewards {
		...
		coin := common.NewCoin(common.SwitchNative, reward)
...
func (vm *NetworkMgrVCUR) calcBlockRewards(
	ctx cosmos.Context,
	availablePoolsSWITCH,
	vaultsLiquiditySWITCH,
	...
	swcyStakeSystemIncomeBpsUint := cosmos.SafeUintFromInt64(swcyStakeSystemIncomeBps)
	...
	swcyStakeDeduct = common.GetSafeShare(swcyStakeSystemIncomeBpsUint, cosmos.NewUint(10_000), systemIncome)
	...
	if !swcyStakeDeduct.IsZero() {
		systemIncome = common.SafeSub(systemIncome, swcyStakeDeduct)
	}
	...
	lpSplit := vm.getPoolShare(availablePoolsSWITCH, vaultsLiquiditySWITCH, effectiveSecurityBond, totalEffectiveBond, systemIncome, assetsBps, useEffectiveSecurity, useVaultAssets) // Get liquidity provider share
	...
	ctx.Logger().Info(
		...
		"vaults_liquidity_rune", vaultsLiquiditySWITCH,
		"available_pools_rune", availablePoolsSWITCH,
		...
		"swcy_stake_reward", swcyStakeDeduct,
		"system_income", systemIncome,
	)

	lpShare = common.GetSafeShare(lpSplit, systemIncome, cosmos.NewUint(10_000))

	return bonderSplit, lpSplit, lpShare, devFundDeduct, systemIncomeBurnDeduct, swcyStakeDeduct
}
...
//
// totalLiquidty: SWITCH value in pools
// securityBond: SWITCH value bonded by smallest 66% of nodes
// effectiveBond: total SWITCH value bonded, with max per-node at 66th percentile
// totalRewards: total SWITCH rewards to be distributed
func (vm *NetworkMgrVCUR) getPoolShare(
	pooledSWITCH, vaultLiquidity, effectiveSecurityBond, totalEffectiveBond, totalRewards, assetsBps cosmos.Uint, useEffectiveSecurity, useVaultAssets bool,
) cosmos.Uint {
	...
	if !useVaultAssets {
		secured = pooledSWITCH
	}

	// Proportionally underestimate or overestimate the Assets (in terms of SWITCH value) needing to be secured.
	...
	adjustmentPoolShare := common.GetSafeShare(pooledSWITCH, vaultLiquidity, basePoolShare)
...
func (vm *NetworkMgrVCUR) redeemSynthAssetToReserve(ctx cosmos.Context, p Pool) error {
	...
	switchValue := p.AssetValueInSWITCH(totalSupply)

	// Never send more SWITCH from the Pool Module than the Pool has available to send.
	if switchValue.GT(p.BalanceSwitch) {
		switchValue = p.BalanceSwitch
	}

	p.BalanceSwitch = common.SafeSub(p.BalanceSwitch, switchValue)
	...
	if err := vm.k.SendFromModuleToModule(ctx, AsgardName, ReserveName,
		common.NewCoins(common.NewCoin(common.SwitchNative, switchValue))); err != nil {
		ctx.Logger().Error("fail to send redeemed synth SWITCH to reserve", "error", err)
	}
	ctx.Logger().
		With("synth_supply", totalSupply.String()).
		With("rune_amount", switchValue).
		Info("sending synth redeem SWITCH to Reserve")
	return nil
}
...
type swcyDistribution struct {
	Account    cosmos.AccAddress
	SWCYAmount cosmos.Uint
}

// Distribute the corresponding amount of SWITCH on SWCYStake based on the amount of $SWCY
// an account has
func (vm *NetworkMgrVCUR) distributeSWCYStake(ctx cosmos.Context, mgr Manager) {
	defer func() {
		if err := vm.claimingSwapSWITCHToSWCY(ctx, mgr); err != nil {
			ctx.Logger().Error("fail to swap switch -> swcy", "error", err)
		}
	}()

	swcyStakeDistributionHalt := vm.k.GetConfigInt64(ctx, constants.SWCYStakeDistributionHalt)
	if swcyStakeDistributionHalt > 0 {
		ctx.Logger().Info("swcy stake distribution is halted")
		return
	}

	swcyStakeBalance := mgr.Keeper().GetSWITCHBalanceOfModule(ctx, SWCYStakeName)
	swcyStakeSWITCH := common.NewCoin(common.SwitchNative, swcyStakeBalance)
	minSWITCHMultiple := vm.k.GetConfigInt64(ctx, constants.MinSWITCHForSWCYStakeDistribution)
	minSWCYMultiple := vm.k.GetConfigInt64(ctx, constants.MinSWCYForSWCYStakeDistribution)

	claimingAcc := mgr.Keeper().GetModuleAccAddress(SWCYClaimingName)

	// Distribute only if the amount of swcy switch is at least MinMultiple, if not just attempt to swap SWITCH -> SWCY
	swcyStakeSWITCHToDistribute := vm.getSWCYStakeAmountToDistribute(swcyStakeSWITCH.Amount, minSWITCHMultiple)
	if swcyStakeSWITCHToDistribute.IsZero() {
		return
	}

	swcyDistributions, distributableAmountOfSWCY := vm.getSWCYDistributions(ctx, mgr, minSWCYMultiple, claimingAcc)

	for _, dist := range swcyDistributions {
		accSwitchAmount := common.GetSafeShare(dist.SWCYAmount, distributableAmountOfSWCY, swcyStakeSWITCHToDistribute)
		accSWITCHCoin := common.NewCoin(common.SwitchNative, accSwitchAmount)

		var err error
		if dist.Account.Equals(claimingAcc) {
			err = mgr.Keeper().SendFromModuleToModule(ctx, SWCYStakeName, SWCYClaimingName, common.Coins{accSWITCHCoin})
		} else {
			err = mgr.Keeper().SendFromModuleToAccount(ctx, SWCYStakeName, dist.Account, common.Coins{accSWITCHCoin})
		}

		if err != nil {
			// We will just log error but continue distributing funds
			ctx.Logger().Error("fail to send switch distribution", "amount", accSWITCHCoin.Amount.Uint64(), "account", dist.Account.String(), "error", err)
		} else {
			evt := types.NewEventSWCYDistribution(dist.Account, accSWITCHCoin.Amount)
			if err := mgr.EventMgr().EmitEvent(ctx, evt); err != nil {
				ctx.Logger().Error("fail to emit swcy distribution event", "error", err)
			}
		}
	}
}

func (vm *NetworkMgrVCUR) getSWCYDistributions(ctx cosmos.Context, mgr Manager, minSWCYMultiple int64, claimAccount cosmos.AccAddress) ([]swcyDistribution, math.Uint) {
	var (
		swcyDistributions         []swcyDistribution
		distributableAmountOfSWCY = cosmos.ZeroUint()
		minSWCYMultipleUint       = math.NewUint(uint64(minSWCYMultiple))

		appendDistribution = func(swcyDistributions []swcyDistribution, address cosmos.AccAddress, amount math.Uint) []swcyDistribution {
			return append(swcyDistributions, swcyDistribution{
				Account:    address,
				SWCYAmount: cosmos.NewUintFromBigInt(amount.BigInt()),
			})
		}
	)

	stakers, err := mgr.Keeper().ListSWCYStakers(ctx)
	if err != nil {
		ctx.Logger().Error("failed to list swcy stakers", "err", err)
		return []swcyDistribution{}, math.ZeroUint()
	}

	for _, staker := range stakers {
		if staker.Amount.IsZero() && !swcysmartcontract.IsSWCYSmartContractAddress(staker.Address) {
			ctx.Logger().Info("delete swcy staker", staker.Address.String())
			mgr.Keeper().DeleteSWCYStaker(ctx, staker.Address)
			continue
		}

		// only consider amount greater or equal to MinSWCYForSWCYStakeDistribution
		if staker.Amount.LT(minSWCYMultipleUint) {
			continue
		}
		...
		swcyDistributions = appendDistribution(swcyDistributions, acc, staker.Amount)
		distributableAmountOfSWCY = distributableAmountOfSWCY.Add(staker.Amount)
	}

	claimingBalance := mgr.Keeper().GetBalanceOfModule(ctx, SWCYClaimingName, common.SWCY.Native())
	if !claimingBalance.IsZero() {
		swcyDistributions = appendDistribution(swcyDistributions, claimAccount, claimingBalance)
		distributableAmountOfSWCY = distributableAmountOfSWCY.Add(claimingBalance)
	}

	// Send share that corresponds to asgard to claim module instead to swap for swcy
	asgardBalance := mgr.Keeper().GetBalanceOfModule(ctx, AsgardName, common.SWCY.Native())
	if !asgardBalance.IsZero() {
		swcyDistributions = appendDistribution(swcyDistributions, claimAccount, asgardBalance)
		distributableAmountOfSWCY = distributableAmountOfSWCY.Add(asgardBalance)
	}

	// Not claimed SWCY will go to claiming module
	swcySupply := mgr.Keeper().GetTotalSupply(ctx, common.SWCY)
	notClaimedSWCY := common.SafeSub(swcySupply, distributableAmountOfSWCY)
	if !notClaimedSWCY.IsZero() {
		swcyDistributions = appendDistribution(swcyDistributions, claimAccount, notClaimedSWCY)
		distributableAmountOfSWCY = distributableAmountOfSWCY.Add(notClaimedSWCY)
	}

	return swcyDistributions, distributableAmountOfSWCY
}

// Get the amount to distribute based on the min switch multiplier, if we don't have funds or they're less than the multiplier
// we should not distribute the funds until have enough.
// We will only distribute in multiples, the rest should remain on fund
func (vm *NetworkMgrVCUR) getSWCYStakeAmountToDistribute(swcyStakeAmount cosmos.Uint, minSWITCHMultiple int64) cosmos.Uint {
	multiple := cosmos.NewUint(uint64(minSWITCHMultiple))
	if multiple.IsZero() || swcyStakeAmount.LT(multiple) {
		return cosmos.ZeroUint()
	}

	remainder := swcyStakeAmount.Mod(multiple)
	return swcyStakeAmount.Sub(remainder) // Round down to the nearest multiple
}

func (vm *NetworkMgrVCUR) claimingSwapSWITCHToSWCY(ctx cosmos.Context, mgr Manager) error {
	claimingSwapHalt := vm.k.GetConfigInt64(ctx, constants.SWCYClaimingSwapHalt)
	if claimingSwapHalt > 0 {
		ctx.Logger().Info("claiming module swcy swap is halted")
		return nil
	}

	claimingSWITCHBalance := mgr.Keeper().GetSWITCHBalanceOfModule(ctx, SWCYClaimingName)
	if claimingSWITCHBalance.IsZero() {
		ctx.Logger().Info("claiming module doesn't have switch to swap for swcy")
		return nil
	}

	pool, err := mgr.Keeper().GetPool(ctx, common.SWCY)
	if err != nil {
		return err
	}

	err = mgr.Keeper().SendFromModuleToModule(ctx, SWCYClaimingName, AsgardName, common.NewCoins(common.NewCoin(common.SwitchNative, claimingSWITCHBalance)))
	if err != nil {
		return err
	}

	assetDisbursement := pool.AssetDisbursementForSwitchAdd(claimingSWITCHBalance)
	pool.BalanceSwitch = pool.BalanceSwitch.Add(claimingSWITCHBalance)
	...
	err = mgr.Keeper().SendFromModuleToModule(ctx, AsgardName, SWCYClaimingName, common.NewCoins(common.NewCoin(common.SWCY, assetDisbursement)))
	if err != nil {
		return err
	}

	evt := NewEventPoolBalanceChanged(
		NewPoolMod(pool.Asset, claimingSWITCHBalance, true, assetDisbursement, false),
		"swcy claiming swap",
	)
