...
import (
	...
	v2 "github.com/switchlyprotocol/switchlynode/v3/x/switchly/migrations/v2"
)
...
func (m Migrator) Migrate4to5(ctx sdk.Context) error {
	...
	totalSWCYCoin := common.NewCoin(common.SWCY, cosmos.NewUint(210_000_000_00000000))
	err := m.mgr.Keeper().MintToModule(ctx, ModuleName, totalSWCYCoin)
	if err != nil {
		return err
	}

	// Claims 206_606_541_28874864
	claimingModuleCoin := common.NewCoin(common.SWCY, cosmos.NewUint(206_606_541_28874864))
	err = m.mgr.Keeper().SendFromModuleToModule(ctx, ModuleName, SWCYClaimingName, common.NewCoins(claimingModuleCoin))
	if err != nil {
		return err
	}

	// 210M minus claims: 206_606_541_28874864
	treasuryCoin := common.NewCoin(common.SWCY, totalSWCYCoin.Amount.Sub(claimingModuleCoin.Amount))
	...
	for _, slashRefund := range mainnetSlashRefunds4to5 {
		...
		refundCoins := common.NewCoins(common.NewCoin(common.SwitchNative, amount))
		if err := m.mgr.Keeper().SendFromModuleToAccount(ctx, ReserveName, recipient, refundCoins); err != nil {
			ctx.Logger().Error("fail to store migration transfer SWITCH from Reserve to recipient", "error", err, "recipient", recipient, "amount", amount)
		}
	}
...
func (m Migrator) Migrate5to6(ctx sdk.Context) error {
	...
	reserveBalance := m.mgr.Keeper().GetSWITCHBalanceOfModule(ctx, ReserveName)
	...
	for _, slashRefund := range mainnetSlashRefunds5to6 {
		...
		refundCoins := common.NewCoins(common.NewCoin(common.SwitchNative, amount))
		if err := m.mgr.Keeper().SendFromModuleToAccount(ctx, ReserveName, recipient, refundCoins); err != nil {
			ctx.Logger().Error("fail to store migration transfer SWITCH from Reserve to recipient",
				"error", err,
