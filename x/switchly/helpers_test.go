...
func (s *HelperSuite) TestDollarsPerSWITCH(c *C) {
	...
	c.Assert(switchUSDPrice, Equals, float32(8.300317))

	// Now try with a second pool, identical depths.
	mgr.Keeper().SetMimir(ctx, "SwitchlyAnchor-ETH-USDC-0XA0B86991C6218B36C1D19D4A2E9EB0CE3606EB48", 1) // enable USDC pool as a SWITCHLY anchor
...
func (HandlerSuite) TestWillSwapSucceed(c *C) {
	...
	// swap from SWITCH no limit, should succeed
	...
	// swap from SWITCH, no limit, but small swap, should fail
	runeTx.Coins = common.Coins{common.NewCoin(common.SwitchNative, cosmos.NewUint(1))}
	msg = NewMsgSwap(runeTx, common.BTCAsset, GetRandomBTCAddress(), cosmos.ZeroUint(), common.NoAddress, cosmos.ZeroUint(), "", "", nil, MarketSwap, 0, 0, GetRandomBech32Addr())
	c.Assert(willSwapOutputExceedLimitAndFees(ctx, mgr, *msg), Equals, false)

	// swap from SWITCH, limit too high, should fail
	...
	// swap from SWITCH, limit not too high, should succeed
	...
	// swap to SWITCH, no limit, should succeed
	msg = NewMsgSwap(tx, common.SwitchNative, GetRandomSWITCHLYAddress(), cosmos.ZeroUint(), common.NoAddress, cosmos.ZeroUint(), "", "", nil, MarketSwap, 0, 0, GetRandomBech32Addr())
	c.Assert(willSwapOutputExceedLimitAndFees(ctx, mgr, *msg), Equals, true)

	// swap to SWITCH, no limit, but small swap, should fail
	tx.Coins = common.Coins{common.NewCoin(common.BTCAsset, cosmos.NewUint(1))}
	msg = NewMsgSwap(tx, common.SwitchNative, GetRandomSWITCHLYAddress(), cosmos.ZeroUint(), common.NoAddress, cosmos.ZeroUint(), "", "", nil, MarketSwap, 0, 0, GetRandomBech32Addr())
	c.Assert(willSwapOutputExceedLimitAndFees(ctx, mgr, *msg), Equals, false)

	// swap to SWITCH, limit too high, should fail
	tx.Coins = common.Coins{common.NewCoin(common.BTCAsset, cosmos.NewUint(common.One))}
	msg = NewMsgSwap(tx, common.SwitchNative, GetRandomSWITCHLYAddress(), cosmos.NewUint(100_000*common.One), common.NoAddress, cosmos.ZeroUint(), "", "", nil, MarketSwap, 0, 0, GetRandomBech32Addr())
	c.Assert(willSwapOutputExceedLimitAndFees(ctx, mgr, *msg), Equals, false)

	// swap to SWITCH, limit not too high, should succeed
	tx.Coins = common.Coins{common.NewCoin(common.BTCAsset, cosmos.NewUint(common.One))}
	msg = NewMsgSwap(tx, common.SwitchNative, GetRandomSWITCHLYAddress(), cosmos.NewUint(1*common.One), common.NoAddress, cosmos.ZeroUint(), "", "", nil, MarketSwap, 0, 0, GetRandomBech32Addr())
	c.Assert(willSwapOutputExceedLimitAndFees(ctx, mgr, *msg), Equals, true)
}
