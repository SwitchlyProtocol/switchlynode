...
var keySwitchly = cosmos.NewKVStoreKey(StoreKey)
...
func setupManagerForTest(c *C) (cosmos.Context, *Mgrs) {
	...
	k := kv1.NewKeeper(encodingConfig.Codec, bk, ak, uk, keySwitchly)
	...
	mgr := NewManagers(k, encodingConfig.Codec, bk, ak, uk, wk, keySwitchly)
...
func setupKeeperForTest(c *C) (cosmos.Context, keeper.Keeper) {
	...
	k := kv1.NewKVStore(encodingConfig.Codec, bk, ak, uk, keySwitchly, GetCurrentVersion())
...
func (HandlerSuite) TestHandleTxInWithdrawLiquidityMemo(c *C) {
	...
	msg := NewMsgWithdrawLiquidity(tx, lp.SwitchAddress, cosmos.NewUint(uint64(MaxWithdrawBasisPoints)), common.DOGEAsset, common.EmptyAsset, w.activeNodeAccount.NodeAddress)
	c.Assert(err, IsNil)
...
func (HandlerSuite) TestRefund(c *C) {
	...
	// check SWITCHLYNode DONT create a refund transaction when SWITCHLYNode don't have a pool for
	// the asset sent.
...
func (HandlerSuite) TestGetMsgWithdrawFromMemo(c *C) {
	...
	if common.SwitchNative.Equals(common.SwitchNative) {
		tx.FromAddress = GetRandomSWITCHLYAddress()
	}
...
func (HandlerSuite) TestGetMsgUnBondFromMemo(c *C) {
	...
	tx.Memo = "unbond:" + GetRandomSWITCHLYAddress().String() + ":1000"
	obTx := NewObservedTx(tx, w.ctx.BlockHeight(), GetRandomPubKey(), w.ctx.BlockHeight())
	msg, err := processOneTxIn(w.ctx, w.keeper, obTx, w.activeNodeAccount.NodeAddress)
	c.Assert(err, IsNil)
	c.Assert(msg, NotNil)
...
func (HandlerSuite) TestGetMsgLiquidityFromMemo(c *C) {
	w := getHandlerTestWrapper(c, 1, true, false)
	// provide DOGE, however SWITCHLYNode send T-CAN as coin , which is incorrect, should result in an error
	m, err := ParseMemo(GetCurrentVersion(), fmt.Sprintf("add:DOGE.DOGE:%s", GetRandomSwitchAddress()))
	c.Assert(err, IsNil)
	...
	// Asymentic liquidity provision should works fine, only SWITCH
	txin.Tx.Coins = common.Coins{
		common.NewCoin(runeAsset,
			cosmos.NewUint(100*common.One)),
	}

	// provide only switch should be fine
	...
	lokiAsset, _ := common.NewAsset("DOGE.LOKI")
	// Make sure the SWITCH Address and Asset Address set correctly
	txin.Tx.Coins = common.Coins{
		common.NewCoin(runeAsset,
			cosmos.NewUint(100*common.One)),
	...
	switchAddr := GetRandomSwitchAddress()
	lokiAddLiquidityMemo, err := ParseMemo(GetCurrentVersion(), fmt.Sprintf("add:DOGE.LOKI:%s", switchAddr))
	c.Assert(err, IsNil)
	...
	c.Assert(msgAddLiquidity.SwitchAddress, Equals, switchAddr)
...
func (s *HandlerSuite) TestMemoFetchAddress(c *C) {
	...
	name := NewSWITCHName("hello", 50, []SWITCHNameAlias{{Chain: common.SWITCHLYChain, Address: thorAddr}})
