...
func (GasManagerTestSuiteVCUR) TestGetAssetOutboundFee(c *C) {
	...
	// should still return nil error if inSWITCH is false
	...
	// conversion to switch requires a pool, so should return 0 and no error
	fee, err = gasMgr.GetAssetOutboundFee(ctx, common.AVAXAsset, true)
	...
	// conversion to switch should now work
	fee, err = gasMgr.GetAssetOutboundFee(ctx, common.AVAXAsset, true)
	...
	// A trade asset with inSWITCH true should return the fee in SWITCH, not in the trade asset.
	switchFee, err := gasMgr.GetAssetOutboundFee(ctx, tradeAsset, true)
	c.Assert(err, IsNil)
	c.Assert(switchFee.String(), Equals, "2000000")
	c.Assert(switchFee.Equal(tradeAssetFee), Equals, false)
	...
	k.SetMimir(ctx, "SwitchlyAnchor-BSC-BUSD-BD1", 1) // enable BUSD pool as a SWITCHLY anchor
	...
	// set mimirs:
	// target surplus: 100 SWITCH
	// min multiplier: 10_000
	// max multiplier: 30_000
	k.SetMimir(ctx, constants.TargetOutboundFeeSurplusSWITCH.String(), 100_00000000) // 100 $SWITCH
	...
	c.Assert(k.AddToOutboundFeeSpentSwitch(ctx, common.BTCAsset, cosmos.NewUint(100_00000000)), IsNil)
	c.Assert(k.AddToOutboundFeeWithheldSwitch(ctx, common.BTCAsset, cosmos.NewUint(150_00000000)), IsNil)
	fee, err = gasMgr.GetAssetOutboundFee(ctx, common.BTCAsset, false)
	c.Assert(fee.Uint64(), Equals, uint64(1000*50000*2))
	c.Assert(err, IsNil)

	// Add more surplus for BTC, should be at min multiplier
	c.Assert(k.AddToOutboundFeeWithheldSwitch(ctx, common.BTCAsset, cosmos.NewUint(50_00000000)), IsNil)
	fee, err = gasMgr.GetAssetOutboundFee(ctx, common.BTCAsset, false)
	...
	c.Assert(k.AddToOutboundFeeSpentSwitch(ctx, btcUsd, cosmos.NewUint(100_00000000)), IsNil)
	c.Assert(k.AddToOutboundFeeWithheldSwitch(ctx, btcUsd, cosmos.NewUint(150_00000000)), IsNil)
	fee, err = gasMgr.GetAssetOutboundFee(ctx, btcUsd, false)
	c.Assert(fee.Uint64(), Equals, uint64(2*(1000*50000*2)))
	c.Assert(err, IsNil)

	// Add more surplus for BTC.USDC, should be at min multiplier
	c.Assert(k.AddToOutboundFeeWithheldSwitch(ctx, btcUsd, cosmos.NewUint(50_00000000)), IsNil)
	fee, err = gasMgr.GetAssetOutboundFee(ctx, btcUsd, false)
...
func (GasManagerTestSuiteVCUR) TestOutboundFeeMultiplier(c *C) {
	...
	targetSurplus := cosmos.NewUint(100_00000000) // 100 $SWITCH
