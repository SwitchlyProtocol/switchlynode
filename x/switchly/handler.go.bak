...
// NewInternalHandler returns a handler for "switchly" internal type messages.
func NewInternalHandler(mgr Manager) cosmos.Handler {
	return func(ctx cosmos.Context, msg cosmos.Msg) (*cosmos.Result, error) {
		...
		if !ok {
			errMsg := fmt.Sprintf("Unrecognized switchly Msg type: %v", sdk.MsgTypeURL(msg))
...
func getInternalHandlerMapping(mgr Manager) map[string]MsgHandler {
	...
	m[sdk.MsgTypeURL(&MsgManageSWITCHName{})] = NewManageSWITCHNameHandler(mgr)
	...
	m[sdk.MsgTypeURL(&MsgSwitchPoolDeposit{})] = NewSwitchPoolDepositHandler(mgr)
	m[sdk.MsgTypeURL(&MsgSwitchPoolWithdraw{})] = NewSwitchPoolWithdrawHandler(mgr)
	m[sdk.MsgTypeURL(&MsgWasmExec{})] = NewWasmExecHandler(mgr)
	m[sdk.MsgTypeURL(&MsgSwitch{})] = NewSwitchHandler(mgr)
	m[sdk.MsgTypeURL(&MsgSWCYClaim{})] = NewSWCYClaimHandler(mgr)
	m[sdk.MsgTypeURL(&MsgSWCYStake{})] = NewSWCYStakeHandler(mgr)
	m[sdk.MsgTypeURL(&MsgSWCYUnstake{})] = NewSWCYUnstakeHandler(mgr)
	return m
}
...
func getMsgAddLiquidityFromMemo(ctx cosmos.Context, memo AddLiquidityMemo, tx ObservedTx, signer cosmos.AccAddress) (cosmos.Msg, error) {
	// Extract the SWITCH amount and the asset amount from the transaction. At least one of them must be
	// nonzero. If SWITCHLYNode saw two types of coins, one of them must be the asset coin.
	switchCoin := tx.Tx.Coins.GetCoin(common.SwitchNative)
	assetCoin := tx.Tx.Coins.GetCoin(memo.GetAsset())

	var switchAddr common.Address
	var assetAddr common.Address
	if tx.Tx.Chain.Equals(common.SWITCHLYChain) {
		switchAddr = tx.Tx.FromAddress
		assetAddr = memo.GetDestination()
	} else {
		switchAddr = memo.GetDestination()
		assetAddr = tx.Tx.FromAddress
	}
	// in case we are providing native switch and another native asset
	if memo.GetAsset().Chain.Equals(common.SWITCHLYChain) {
		assetAddr = switchAddr
	}

	return NewMsgAddLiquidity(tx.Tx, memo.GetAsset(), switchCoin.Amount, assetCoin.Amount, switchAddr, assetAddr, memo.AffiliateAddress, memo.AffiliateBasisPoints, signer), nil
}

func getMsgDonateFromMemo(memo DonateMemo, tx ObservedTx, signer cosmos.AccAddress) (cosmos.Msg, error) {
	switchCoin := tx.Tx.Coins.GetCoin(common.SwitchNative)
	assetCoin := tx.Tx.Coins.GetCoin(memo.GetAsset())
	return NewMsgDonate(tx.Tx, memo.GetAsset(), switchCoin.Amount, assetCoin.Amount, signer), nil
}
...
func getMsgManageSWITCHNameFromMemo(memo ManageSWITCHNameMemo, tx ObservedTx, signer cosmos.AccAddress) (cosmos.Msg, error) {
	if len(tx.Tx.Coins) == 0 {
		return nil, fmt.Errorf("transaction must have switch in it")
	}
	return NewMsgManageSWITCHName(memo.Name, memo.Chain, memo.Address, tx.Tx.Coins[0], memo.Expire, memo.PreferredAsset, memo.Owner, signer), nil
}
...
func processOneTxIn(ctx cosmos.Context, keeper keeper.Keeper, tx ObservedTx, signer cosmos.AccAddress) (cosmos.Msg, error) {
	...
	memo, err := ParseMemoWithSWITCHNames(ctx, keeper, tx.Tx.Memo)
	if err != nil {
		ctx.Logger().Error("fail to parse memo", "error", err)
		return nil, err
	}

	// SWITCHLYNode should not have one tx across chain, if it is cross chain it should be separate tx
	...
	case ManageSWITCHNameMemo:
		newMsg, err = getMsgManageSWITCHNameFromMemo(m, tx, signer)
	case LoanOpenMemo:
	...
	case SwitchPoolDepositMemo:
		newMsg = NewMsgSwitchPoolDeposit(signer, tx.Tx)
	case SwitchPoolWithdrawMemo:
		newMsg = NewMsgSwitchPoolWithdraw(signer, tx.Tx, m.GetBasisPts(), m.GetAffiliateAddress(), m.GetAffiliateBasisPoints())
	case ExecMemo:
		coin := tx.Tx.Coins[0]
	...
	case SWCYClaimMemo:
		newMsg = NewMsgSWCYClaim(m.Address, tx.Tx.FromAddress, signer)
	case SWCYStakeMemo:
		newMsg = NewMsgSWCYStake(tx.Tx, signer)
	case SWCYUnstakeMemo:
		newMsg = NewMsgSWCYUnstake(tx.Tx, m.BasisPoints, signer)
	case MaintMemo:
...
func fuzzyAssetMatch(ctx cosmos.Context, keeper keeper.Keeper, origAsset common.Asset) common.Asset {
	...
	// Only check BalanceSwitch after checking the error so that no panic if there were an error.
