...
func (s *HandlerDepositSuite) TestValidate(c *C) {
	...
	msg := NewMsgDeposit(coins, fmt.Sprintf("ADD:DOGE.DOGE:%s", GetRandomSwitchAddress()), addr)
...
func (s *HandlerDepositSuite) TestAddSwap(c *C) {
	...
	ctx, mgr := setupManagerForTest(c)
	handler := NewDepositHandler(mgr)
	affAddr := GetRandomSWITCHLYAddress()
	tx := common.NewTx(
		GetRandomTxHash(),
	...
	msg1 := NewMsgSwap(tx, common.BTCAsset, GetRandomBTCAddress(), cosmos.ZeroUint(), GetRandomSWITCHLYAddress(), cosmos.NewUint(20000), "", "", nil, MarketSwap, 0, 0, GetRandomBech32Addr())
	...
	// NONE SWITCH , synth asset should be handled correctly
...
func (s *HandlerDepositSuite) TestTargetModule(c *C) {
	...
	testCases := []struct {
		...
		{
			name:       "switchlyname coins should go to reserve",
	...
	for _, tc := range testCases {
		...
		balBefore := mgr.Keeper().GetSWITCHBalanceOfModule(ctx, tc.moduleName)
		result, err := handler.Run(ctx, msg)
		balAfter := mgr.Keeper().GetSWITCHBalanceOfModule(ctx, tc.moduleName)
