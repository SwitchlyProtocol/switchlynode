...
func (s *HandlerOutboundTxSuite) TestOuboundTxHandlerSendExtraFundShouldBeSlashed(c *C) {
	...
	newReserve := helper.keeper.GetSWITCHBalanceOfModule(helper.ctx, ReserveName)
	c.Assert(err, IsNil)
...
func (s *HandlerOutboundTxSuite) TestOutboundTxHandlerInvalidObservedTxVoterShouldSlash(c *C) {
	...
	// expected 0.5 slashed SWITCH be added to reserve
...
func (s *HandlerOutboundTxSuite) TestOutboundTxHandlerETHChainSpendTooMuchGasPerSWITCHLYNodeInstructionShouldNotSlash(c *C) {
	helper := newOutboundTxHandlerTestHelper(c)
	handler := NewOutboundTxHandler(helper.mgr)
	pool := NewPool()
	pool.Asset = common.ETHAsset
