...
func (h CommonOutboundTxHandler) handle(ctx cosmos.Context, tx ObservedTx, inTxID common.TxID) (*cosmos.Result, error) {
	...
	// every Signing Transaction Period , SWITCHLYNode will check whether a
	...
	for height := latestHeight; height >= earliestHeight; height-- {
		...
		for i, txOutItem := range txOut.TxArray {
			...
			// txes, SWITCHLYNode have to correlate outbound tx back to the
			// inbound, and also txitem , thus SWITCHLYNode could record both
			// outbound tx hash correctly given every tx item will only have
			// one coin in it , SWITCHLYNode could use that to identify which tx it
			// is
			...
			if txOutItem.InHash.Equals(inTxID) &&
				...
				if txOutItem.Chain.Equals(common.ETHChain) {
					...
					// If switchlynode instruct bifrost to spend more than MaxETHGas , then it should not be slashed.
