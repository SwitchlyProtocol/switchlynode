...
func (s *SlashingVCURSuite) TestSlashVault(c *C) {
	...
	asgardBeforeSlash := mgr.Keeper().GetSWITCHBalanceOfModule(ctx, AsgardName)
	bondBeforeSlash := mgr.Keeper().GetSWITCHBalanceOfModule(ctx, BondName)
	reserveBeforeSlash := mgr.Keeper().GetSWITCHBalanceOfModule(ctx, ReserveName)
	...
	asgardAfterSlash := mgr.Keeper().GetSWITCHBalanceOfModule(ctx, AsgardName)
	bondAfterSlash := mgr.Keeper().GetSWITCHBalanceOfModule(ctx, BondName)
	reserveAfterSlash := mgr.Keeper().GetSWITCHBalanceOfModule(ctx, ReserveName)
	...
	c.Assert(asgardAfterSlash.Sub(asgardBeforeSlash).Uint64(), Equals, poolAfterSlash.Sub(poolBeforeSlash).Uint64(), Commentf("%d", "pool/asgard switch mismatch"))
...
func (s *SlashingVCURSuite) TestUpdatePoolFromSlash(c *C) {
	...
	creditSWITCH := cosmos.NewUint(500 * common.One)
	stolenAsset := common.NewCoin(common.BTCAsset, deductAsset)
	slasher.updatePoolFromSlash(ctx, pool, stolenAsset, creditSWITCH, mgr)
...
func (s *SlashingVCURSuite) TestNetworkShouldNotSlashMorethanVaultAmount(c *C) {
	...
	// Even though the total switch value to slash is 3000, the node only has 1000 SWITCH bonded, so only slash and credit that much to the pool's switch side
