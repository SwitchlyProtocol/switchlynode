...
func (s *HandlerAddLiquiditySuite) TestAddLiquidityHandler(c *C) {
	...
	k := &MockAddLiquidityKeeper{
		...
		polAddress: GetRandomSwitchAddress(),
	}
	mgr.K = k
	// happy path
	...
	msg.SwitchAmount = cosmos.ZeroUint()
	...
	c.Assert(err, IsNil)
	c.Check(pol.SwitchDeposited.Uint64(), Equals, uint64(0))
}
...
func (s *HandlerAddLiquiditySuite) TestAddLiquidityHandler_NoPool_ShouldCreateNewPool(c *C) {
	...
	mgr.constAccessor = constants.NewDummyConstants(map[constants.ConstantName]int64{
		constants.MaximumLiquiditySWITCH: 600_000_00000000,
	...
	c.Assert(postLiquidityPool.BalanceSwitch.String(), Equals, preLiquidityPool.BalanceSwitch.Add(msg.SwitchAmount).String())
}
...
func (s *HandlerAddLiquiditySuite) TestAddLiquidityHandlerValidation(c *C) {
	...
	testCases := []struct {
		...
		},
		{
			...
			msg:            NewMsgAddLiquidity(GetRandomTx(), common.ETHAsset, cosmos.NewUint(common.One*5000), cosmos.NewUint(common.One*5000), GetRandomSwitchAddress(), GetRandomETHAddress(), common.NoAddress, cosmos.ZeroUint(), activeNodeAccount.NodeAddress),
			expectedResult: errAddLiquiditySWITCHMoreThanBond,
		},
		{
			name:           "switch address with wrong chain should fail",
			msg:            NewMsgAddLiquidity(GetRandomTx(), common.ETHAsset, cosmos.NewUint(common.One*5), cosmos.NewUint(common.One*5), GetRandomETHAddress(), GetRandomSwitchAddress(), common.NoAddress, cosmos.ZeroUint(), GetRandomValidatorNode(NodeActive).NodeAddress),
			expectedResult: errAddLiquidityFailValidation,
		},
		{
			name:           "asset address with wrong network should fail",
			msg:            NewMsgAddLiquidity(GetRandomTx(), common.ETHAsset, cosmos.NewUint(common.One*5), cosmos.NewUint(common.One*5), GetRandomSwitchAddress(), mainnetBTCAddr, common.NoAddress, cosmos.ZeroUint(), GetRandomValidatorNode(NodeActive).NodeAddress),
...
func (s *HandlerAddLiquiditySuite) TestCalculateLPUnits(c *C) {
	inputs := []struct {
		...
		poolSWITCH       cosmos.Uint
		poolAsset      cosmos.Uint
		addSWITCH        cosmos.Uint
		...
		{
			name:           "first-add-zero-switch",
			oldLPUnits:     cosmos.ZeroUint(),
			poolSWITCH:       cosmos.ZeroUint(),
			poolAsset:      cosmos.ZeroUint(),
			addSWITCH:        cosmos.ZeroUint(),
			addAsset:       cosmos.NewUint(100 * common.One),
			poolUnits:      cosmos.ZeroUint(),
			liquidityUnits: cosmos.ZeroUint(),
			expectedErr:    errors.New("total SWITCH in the pool is zero"),
		},
		{
			...
			poolSWITCH:       cosmos.ZeroUint(),
			poolAsset:      cosmos.ZeroUint(),
			addSWITCH:        cosmos.NewUint(100 * common.One),
		...
		},
		{
			...
			poolSWITCH:       cosmos.NewUint(500 * common.One),
			poolAsset:      cosmos.NewUint(500 * common.One),
			addSWITCH:        cosmos.NewUint(345 * common.One),
		...
		},
		{
			...
			poolSWITCH:       cosmos.NewUint(500 * common.One),
			poolAsset:      cosmos.NewUint(500 * common.One),
			addSWITCH:        cosmos.NewUint(500 * common.One),
			addAsset:       cosmos.ZeroUint(),
	...
	for _, item := range inputs {
		...
		poolUnits, liquidityUnits, err := calculatePoolUnits(item.oldLPUnits, item.poolSWITCH, item.poolAsset, item.addSWITCH, item.addAsset)
...
func (s *HandlerAddLiquiditySuite) TestAddLiquidityV1(c *C) {
	...
	err = h.addLiquidity(ctx, common.Asset{}, cosmos.NewUint(100*common.One), cosmos.NewUint(100*common.One), switchAddress, assetAddress, txID, false, constAccessor)
	c.Assert(err, NotNil)
	c.Assert(ps.SetPool(ctx, Pool{
		BalanceSwitch:        cosmos.ZeroUint(),
		BalanceAsset:         cosmos.NewUint(100 * common.One),
	...
	err = h.addLiquidity(ctx, common.ETHAsset, cosmos.NewUint(100*common.One), cosmos.NewUint(100*common.One), switchAddress, assetAddress, txID, false, constAccessor)
	c.Assert(err, IsNil)
	su, err := ps.GetLiquidityProvider(ctx, common.ETHAsset, switchAddress)
	c.Assert(err, IsNil)
	...
	// add asymmetically
	err = h.addLiquidity(ctx, common.ETHAsset, cosmos.NewUint(100*common.One), cosmos.ZeroUint(), switchAddress, assetAddress, txID, false, constAccessor)
	c.Assert(err, IsNil)
	err = h.addLiquidity(ctx, common.ETHAsset, cosmos.ZeroUint(), cosmos.NewUint(100*common.One), switchAddress, assetAddress, txID, false, constAccessor)
	c.Assert(err, IsNil)

	err = h.addLiquidity(ctx, notExistLiquidityProviderAsset, cosmos.NewUint(100*common.One), cosmos.NewUint(100*common.One), switchAddress, assetAddress, txID, false, constAccessor)
	c.Assert(err, NotNil)
	c.Assert(ps.SetPool(ctx, Pool{
		BalanceSwitch:        cosmos.NewUint(100 * common.One),
		BalanceAsset:         cosmos.NewUint(100 * common.One),
	...
	err = h.addLiquidity(ctx, common.ETHAsset, cosmos.NewUint(common.One), cosmos.NewUint(common.One), switchAddress, assetAddress, txID, false, constAccessor)
	c.Assert(err, IsNil)
	...
	// add switch
	err = h.addLiquidity(ctx, common.BTCAsset, cosmos.NewUint(100*common.One), cosmos.ZeroUint(), switchAddress, btcAddress, txID, true, constAccessor)
	c.Assert(err, IsNil)
	_, err = ps.GetLiquidityProvider(ctx, common.BTCAsset, switchAddress)
	c.Assert(err, IsNil)
	// c.Check(su.Units.IsZero(), Equals, true)
	// add btc
	err = h.addLiquidity(ctx, common.BTCAsset, cosmos.ZeroUint(), cosmos.NewUint(100*common.One), switchAddress, btcAddress, txID, false, constAccessor)
	c.Assert(err, IsNil)
	su, err = ps.GetLiquidityProvider(ctx, common.BTCAsset, switchAddress)
	c.Assert(err, IsNil)
...
func (HandlerAddLiquiditySuite) TestSWITCHOnlyFairMergeProvidedLiquidity(c *C) {
	...
	err := h.addLiquidity(ctx, common.BTCAsset, cosmos.NewUint(100*common.One), cosmos.ZeroUint(), switchAddr, common.NoAddress, txID, false, constAccessor)
	c.Assert(err, IsNil)

	su, err := k.GetLiquidityProvider(ctx, common.BTCAsset, switchAddr)
	c.Assert(err, IsNil)
...
func (s *HandlerAddLiquiditySuite) TestAddLiquidityPOL(c *C) {
	...
	ctx, mgr := setupManagerForTest(c)
	activeNodeAccount := GetRandomValidatorNode(NodeActive)
	polAddr := GetRandomSwitchAddress()
	pool := NewPool()
	pool.Asset = common.ETHAsset
	pool.Status = PoolAvailable
	...
	pol, err := mgr.Keeper().GetPOL(ctx)
	c.Assert(err, IsNil)
	c.Check(pol.SwitchDeposited.Uint64(), Equals, uint64(10000000000))
}
