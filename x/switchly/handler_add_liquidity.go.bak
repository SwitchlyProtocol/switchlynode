...
func (h AddLiquidityHandler) validate(ctx cosmos.Context, msg MsgAddLiquidity) error {
	...
	// and so there is no need to check at this stage for whether the addition is of SWITCH or Asset or with needsSwap.
	if !msg.Asset.Equals(gasAsset) && !msg.Asset.IsSWCY() && !msg.Asset.IsRUJI() {
	...
	// * stable pools (as determined by SWITCHLY anchor pool settings)
	if msg.Asset.IsSyntheticAsset() {
		// Check if the asset is in the anchor pools
		isAnchorAsset := false
		for _, asset := range h.mgr.Keeper().GetAnchors(ctx, common.SWITCHLY) {
		...
		if !msg.Asset.GetLayer1Asset().IsGasAsset() && !isAnchorAsset {
			return fmt.Errorf("asset must be a gas asset or SWITCHLY anchor pool")
		}
		...
		if !msg.SwitchAmount.IsZero() {
			return fmt.Errorf("cannot deposit switch into a vault")
		}
	}

	if !msg.SwitchAddress.IsEmpty() && !msg.SwitchAddress.IsChain(common.SWITCHLYChain) {
	...
	if !msg.AssetAddress.IsEmpty() {
		...
		// a position with pending SWITCH cannot be completed with Asset,
		...
		if msg.SwitchAddress.Equals(polAddress) {
	...
	totalLiquiditySWITCH, err := h.getTotalLiquiditySWITCH(ctx)
	if err != nil {
		return ErrInternal(err, "fail to get total liquidity SWITCH")
	}

	// total liquidity SWITCH after current add liquidity
	totalLiquiditySWITCH = totalLiquiditySWITCH.Add(msg.SwitchAmount)
	totalLiquiditySWITCH = totalLiquiditySWITCH.Add(pool.AssetValueInSWITCH(msg.AssetAmount))
	maximumLiquiditySWITCH := h.mgr.Keeper().GetConfigInt64(ctx, constants.MaximumLiquiditySWITCH)
	if maximumLiquiditySWITCH > 0 {
		if totalLiquiditySWITCH.GT(cosmos.NewUint(uint64(maximumLiquiditySWITCH))) {
			return errAddLiquiditySWITCHOverLimit
		}
	}

	coins := common.NewCoins(
		common.NewCoin(common.SwitchNative, msg.SwitchAmount),
...
func (h AddLiquidityHandler) handle(ctx cosmos.Context, msg MsgAddLiquidity) error {
	...
	if pool.IsEmpty() {
		ctx.Logger().Info("pool doesn't exist yet, creating a new one...", "symbol", msg.Asset.String(), "creator", msg.SwitchAddress)
	...
	if !msg.Asset.IsSyntheticAsset() && !msg.Tx.ID.IsBlank() {
		...
		if !msg.SwitchAddress.IsEmpty() && msg.SwitchAmount.IsZero() {
			stage = true
		}
	}
	...
	affiliateSWITCH := common.GetSafeShare(msg.AffiliateBasisPoints, cosmos.NewUint(10000), msg.SwitchAmount)
	affiliateAsset := common.GetSafeShare(msg.AffiliateBasisPoints, cosmos.NewUint(10000), msg.AssetAmount)
	userSWITCH := common.SafeSub(msg.SwitchAmount, affiliateSWITCH)
...
func (h AddLiquidityHandler) validateAddLiquidityMessage(ctx cosmos.Context, keeper keeper.Keeper, asset common.Asset, requestTxHash common.TxID, switchAddr, assetAddr common.Address) error {
	if asset.IsEmpty() {
	...
	if switchAddr.IsEmpty() && assetAddr.IsEmpty() {
		return errors.New("switch address and asset address is empty")
	}
	...
	if pool.Status == PoolStaged && (switchAddr.IsEmpty() || assetAddr.IsEmpty()) {
...
// r = switch provided;
// a = asset provided
// R = switch Balance (before)
// A = asset Balance (before)
// P = Pool Units (before)
// units / (P + units) = (1/2) * ((r / (R + r)) + (a / (A + a)))
// units = P * (r*A + a*R + 2*r*a) / (r*A + a*R + 2*R*A)
func calculatePoolUnits(oldPoolUnits, poolSWITCH, poolAsset, addSWITCH, addAsset cosmos.Uint) (cosmos.Uint, cosmos.Uint, error) {
	if addSWITCH.Add(poolSWITCH).IsZero() {
		return cosmos.ZeroUint(), cosmos.ZeroUint(), errors.New("total SWITCH in the pool is zero")
	}
	if addAsset.Add(poolAsset).IsZero() {
		return cosmos.ZeroUint(), cosmos.ZeroUint(), errors.New("total asset in the pool is zero")
	}

	if poolSWITCH.IsZero() || poolAsset.IsZero() || oldPoolUnits.IsZero() {
		sUnits := addSWITCH
		if addSWITCH.IsZero() {
			if !poolAsset.IsZero() {
				// Where possible, keep a SWITCH scale for new units.
				sUnits = addAsset.Mul(poolSWITCH).Quo(poolAsset)
			} else {
				sUnits = addAsset
			}
		}
	...
	R := cosmos.NewDecFromBigInt(poolSWITCH.BigInt())
	A := cosmos.NewDecFromBigInt(poolAsset.BigInt())
	r := cosmos.NewDecFromBigInt(addSWITCH.BigInt())
...
func (h AddLiquidityHandler) addLiquidity(ctx cosmos.Context,
	asset common.Asset,
	addSwitchAmount, addAssetAmount cosmos.Uint,
	switchAddr, assetAddr common.Address,
	requestTxHash common.TxID,
	stage bool,
	constAccessor constants.ConstantValues,
) (err error) {
	ctx.Logger().Info("liquidity provision", "asset", asset, "switch amount", addSwitchAmount, "asset amount", addAssetAmount)
	if err = h.validateAddLiquidityMessage(ctx, h.mgr.Keeper(), asset, requestTxHash, switchAddr, assetAddr); err != nil {
	...
	if su.Units.IsZero() {
		if su.PendingTxID.IsEmpty() {
			if su.SwitchAddress.IsEmpty() {
				su.SwitchAddress = switchAddr
			}
		...
		if asset.IsSyntheticAsset() {
			// new SU, by default, places the switchly address to the switch address,
			// but here we want it to be on the asset address only
			su.AssetAddress = assetAddr
			su.SwitchAddress = common.NoAddress // no switch to add/withdraw
		} else {
			// ensure input addresses match LP position addresses
			if !switchAddr.Equals(su.SwitchAddress) {
				return errAddLiquidityMismatchAddr
			}
	...
	if addSwitchAmount.IsZero() {
		runeTxID = su.PendingTxID
	} else {
		assetTxID = su.PendingTxID
	}

	pendingSwitchAmt := su.PendingSwitch.Add(addSwitchAmount)
	pendingAssetAmt := su.PendingAsset.Add(addAssetAmount)

	// if we have an asset address and no asset amount, put the switch pending
	if stage && pendingAssetAmt.IsZero() {
		pool.PendingInboundSwitch = pool.PendingInboundSwitch.Add(addSwitchAmount)
		su.PendingSwitch = pendingSwitchAmt
		su.PendingTxID = requestTxHash
		h.mgr.Keeper().SetLiquidityProvider(ctx, su)
		if err = h.mgr.Keeper().SetPool(ctx, pool); err != nil {
			ctx.Logger().Error("fail to save pool pending inbound switch", "error", err)
		}

		// add pending liquidity event
		evt := NewEventPendingLiquidity(pool.Asset, AddPendingLiquidity, su.SwitchAddress, addSwitchAmount, su.AssetAddress, cosmos.ZeroUint(), requestTxHash, common.TxID(""))
	...
	// if we have a switch address and no switch asset, put the asset in pending
	if stage && pendingSwitchAmt.IsZero() {
		...
		evt := NewEventPendingLiquidity(pool.Asset, AddPendingLiquidity, su.SwitchAddress, cosmos.ZeroUint(), su.AssetAddress, addAssetAmount, common.TxID(""), requestTxHash)
		if err = h.mgr.EventMgr().EmitEvent(ctx, evt); err != nil {
			return ErrInternal(err, "fail to emit partial add liquidity event")
		}
		return nil
	...
	pool.PendingInboundSwitch = common.SafeSub(pool.PendingInboundSwitch, su.PendingSwitch)
	pool.PendingInboundAsset = common.SafeSub(pool.PendingInboundAsset, su.PendingAsset)
	su.PendingAsset = cosmos.ZeroUint()
	su.PendingSwitch = cosmos.ZeroUint()
	su.PendingTxID = ""

	ctx.Logger().Info("pre add liquidity", "pool", pool.Asset, "switch", pool.BalanceSwitch, "asset", pool.BalanceAsset, "LP units", pool.LPUnits, "synth units", pool.SynthUnits)
	ctx.Logger().Info("adding liquidity", "switch", addSwitchAmount, "asset", addAssetAmount)

	balanceSWITCH := pool.BalanceSwitch
	balanceAsset := pool.BalanceAsset

	oldPoolUnits := pool.GetPoolUnits()
	var newPoolUnits, liquidityUnits cosmos.Uint
	if asset.IsSyntheticAsset() {
		pendingSwitchAmt = cosmos.ZeroUint() // sanity check
		newPoolUnits, liquidityUnits = calculateVaultUnits(oldPoolUnits, balanceAsset, pendingAssetAmt)
	} else {
		newPoolUnits, liquidityUnits, err = calculatePoolUnits(oldPoolUnits, balanceSWITCH, balanceAsset, pendingSwitchAmt, pendingAssetAmt)
		if err != nil {
	...
	poolSWITCH := balanceSWITCH.Add(pendingSwitchAmt)
	poolAsset := balanceAsset.Add(pendingAssetAmt)
	pool.LPUnits = pool.LPUnits.Add(liquidityUnits)
	pool.BalanceSwitch = poolSWITCH
	pool.BalanceAsset = poolAsset
	ctx.Logger().Info("post add liquidity", "pool", pool.Asset, "switch", pool.BalanceSwitch, "asset", pool.BalanceAsset, "LP units", pool.LPUnits, "synth units", pool.SynthUnits, "add liquidity units", liquidityUnits)
	if (pool.BalanceSwitch.IsZero() && !asset.IsSyntheticAsset()) || pool.BalanceAsset.IsZero() {
		return ErrInternal(err, "pool cannot have zero switch or asset balance")
	}
	...
	if pool.Status == PoolAvailable {
		if su.AssetDepositValue.IsZero() && su.SwitchDepositValue.IsZero() {
			su.SwitchDepositValue = common.GetSafeShare(su.Units, pool.GetPoolUnits(), pool.BalanceSwitch)
			su.AssetDepositValue = common.GetSafeShare(su.Units, pool.GetPoolUnits(), pool.BalanceAsset)
		} else {
			su.SwitchDepositValue = su.SwitchDepositValue.Add(common.GetSafeShare(liquidityUnits, pool.GetPoolUnits(), pool.BalanceSwitch))
			su.AssetDepositValue = su.AssetDepositValue.Add(common.GetSafeShare(liquidityUnits, pool.GetPoolUnits(), pool.BalanceAsset))
		}
	}
	h.mgr.Keeper().SetLiquidityProvider(ctx, su)

	evt := NewEventAddLiquidity(asset, liquidityUnits, su.SwitchAddress, pendingSwitchAmt, pendingAssetAmt, runeTxID, assetTxID, su.AssetAddress)
	if err = h.mgr.EventMgr().EmitEvent(ctx, evt); err != nil {
		return ErrInternal(err, "fail to emit add liquidity event")
	}

	// if its the POL is adding, track switch added
	polAddress, err := h.mgr.Keeper().GetModuleAddress(ReserveName)
	if err != nil {
		return err
	}

	if polAddress.Equals(su.SwitchAddress) {
		var pol ProtocolOwnedLiquidity
		pol, err = h.mgr.Keeper().GetPOL(ctx)
		if err != nil {
			return err
		}
		pol.SwitchDeposited = pol.SwitchDeposited.Add(pendingSwitchAmt)

		if err = h.mgr.Keeper().SetPOL(ctx, pol); err != nil {
			return err
		}

		ctx.Logger().Info("POL deposit", "pool", pool.Asset, "switch", pendingSwitchAmt)
		telemetry.IncrCounterWithLabels(
			[]string{"switchlynode", "pol", "pool", "rune_deposited"},
			telem(pendingSwitchAmt),
...
func (h AddLiquidityHandler) getTotalLiquiditySWITCH(ctx cosmos.Context) (cosmos.Uint, error) {
