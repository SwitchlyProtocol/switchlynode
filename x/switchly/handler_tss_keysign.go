...
// when a signer fail to join tss keysign , switchly need to slash the node account
...
func (h TssKeysignHandler) handleV3_0_0(ctx cosmos.Context, msg MsgTssKeysignFail) (*cosmos.Result, error) {
	...
	for _, item := range vault.GetMembership() {
		...
		if err != nil {
			return nil, wrapError(ctx, err, "fail to get switchly address for "+item.String())
		}
	...
	telemetry.IncrCounterWithLabels(
		[]string{"switchlynode", "tss", "keysign", "failure"},
		float32(1),
