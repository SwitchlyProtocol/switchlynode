...
func (s AdvSwapQueueVCURSuite) TestScoreMsgs(c *C) {
	...
	// 50 ETH is worth more than 100 SWITCH
...
func (s AdvSwapQueueVCURSuite) TestEndBlock(c *C) {
	...
	affilAddr := GetRandomSWITCHLYAddress()
	...
	c.Assert(err, IsNil)
	c.Assert(items, HasLen, 2) // two outbounds are switch, which doesn't show up in the outbound items list
