...
func (pm *PoolMgrVCUR) EndBlock(ctx cosmos.Context, mgr Manager) error {
	...
	if ctx.BlockHeight()%poolCycle == 0 && !mgr.Keeper().RagnarokInProgress(ctx) {
		...
		minSwitchPoolDepth, err := mgr.Keeper().GetMimir(ctx, constants.MinSWITCHPoolDepth.String())
		if minSwitchPoolDepth < 0 || err != nil {
			minSwitchPoolDepth = mgr.GetConstants().GetInt64Value(constants.MinSWITCHPoolDepth)
		}
		...
		if err := pm.cyclePools(ctx, maxAvailablePools, minSwitchPoolDepth, stagedPoolCost, mgr); err != nil {
...
// Staged pools are charged a fee and those with with zero switch depth and
// non-zero asset depth are removed along with their liquidity providers, and
// remaining assets are abandoned.
// The valid Staged pool with the highest switch depth is promoted to Available.
// If there are more than the maximum available pools, the Available pool with
// with the lowest switch depth is demoted to Staged
func (pm *PoolMgrVCUR) cyclePools(ctx cosmos.Context, maxAvailablePools, minSwitchPoolDepth, stagedPoolCost int64, mgr Manager) error {
	...
	minSWITCHDepth := cosmos.NewUint(uint64(minSwitchPoolDepth))
	...
	validPool := func(pool Pool) bool {
		if pool.BalanceAsset.IsZero() || pool.BalanceSwitch.IsZero() || pool.BalanceSwitch.LT(minSWITCHDepth) {
			return false
		}
		return true
	}
	...
	for ; iterator.Valid(); iterator.Next() {
		...
		switch pool.Status {
		case PoolAvailable:
			// any available pools that have no asset, no switch, or less than
			// min switch, moves back to staged status
			if validPool(pool) &&
		...
		case PoolStaged:
			...
			if !fee.IsZero() {
				...
				if err := mgr.Keeper().AddPoolFeeToReserve(ctx, fee); err != nil {
					ctx.Logger().Error("fail to add switch to reserve", "from pool", pool.Asset, "err", err)
				}
			...
			if pool.BalanceSwitch.IsZero() && !pool.BalanceAsset.IsZero() {
				...
				// Transfer any pending SWITCH to the Reserve so it isn't left in the Pool Module after pool deletion
				if !pool.PendingInboundSwitch.IsZero() {
					if err := mgr.Keeper().AddPoolFeeToReserve(ctx, pool.PendingInboundSwitch); err != nil {
						ctx.Logger().Error("fail to transfer pending inbound switch to reserve during pool burning", "from pool", pool.Asset, "err", err)
					}
				}
...
func (pm *PoolMgrVCUR) checkSaversUtilization(ctx cosmos.Context, mgr Manager) error {
	...
	tx := common.NewTx(
		common.BlankTxID, latestLp.AssetAddress, latestLp.AssetAddress, coins, nil, "SWITCHLY-SAVER-EJECT",
	)
...
func (pm *PoolMgrVCUR) commitPendingLiquidity(ctx cosmos.Context, pool Pool, mgr Manager) error {
	...
	for ; iterator.Valid(); iterator.Next() {
		...
		if lp.AssetAddress.IsEmpty() || lp.SwitchAddress.IsEmpty() {
			continue
		}
		...
		tx := common.Tx{
			ID:        common.BlankTxID,
			ToAddress: common.NoAddress,
			Memo:      "SWITCHLY-PENDING-LIQUIDITY-ADD",
		}

		if !lp.PendingSwitch.IsZero() {
		...
		if !lp.PendingAsset.IsZero() {
			tx.FromAddress = lp.SwitchAddress
			tx.Chain = common.SWITCHLYChain
			tx.Coins = common.NewCoins(common.NewCoin(common.SwitchNative, runeAdd))
		}

		msg := NewMsgAddLiquidity(tx, lp.Asset, runeAdd, assetAdd, lp.SwitchAddress, lp.AssetAddress, common.NoAddress, cosmos.ZeroUint(), signer)
		_, err := handler(ctx, msg)
		if err != nil {
			ctx.Logger().Error("failed to commit pending liquidity", "asset", lp.Asset, "switchly address", lp.SwitchAddress, "asset address", lp.AssetAddress, "pending switch", lp.PendingSwitch, "pending asset", lp.PendingAsset, "error", err)
		...
		count += 1
		cleanedSWITCH = cleanedSWITCH.Add(lp.PendingSwitch)
		cleanedAsset = cleanedSWITCH.Add(lp.PendingAsset)
	}
	ctx.Logger().Info("cleaned pending liquidity", "pool", pool.Asset, "count", count, "cleared switch", cleanedSWITCH.String(), "cleared asset", cleanedAsset.String())

	// add telemetry
	telemetry.IncrCounterWithLabels(
		[]string{"switchlynode", "pool", "pending", "clear", "count"},
		float32(count),
		[]metrics.Label{telemetry.NewLabel("pool", pool.Asset.String())},
	)
	telemetry.IncrCounterWithLabels(
		[]string{"switchlynode", "pool", "pending", "clear", "switch"},
		telem(cleanedSWITCH),
		[]metrics.Label{telemetry.NewLabel("pool", pool.Asset.String())},
	)
	telemetry.IncrCounterWithLabels(
		[]string{"switchlynode", "pool", "pending", "clear", "asset"},
		telem(cleanedAsset),
		[]metrics.Label{telemetry.NewLabel("pool", pool.Asset.String())},
	)
