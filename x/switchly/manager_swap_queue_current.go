	...
	for i := int64(0); i < vm.getTodoNum(int64(len(swaps)), minSwapsPerBlock, maxSwapsPerBlock); i++ {
		...
		if handleErr != nil {
			...
			if pick.msg.IsStreaming() {
				...
				// Should not refund a failed preferred asset swap, the SWITCH is still in the AffiliateCollector module
				...
				if strings.HasPrefix(pick.msg.Tx.Memo, PreferredAssetSwapMemoPrefix) && pick.msg.Tx.FromAddress.Equals(affColAddress) {
					...
					switchAmt := pick.msg.Tx.Coins[0].Amount
					memo := pick.msg.Tx.Memo
					if err := vm.cleanupFailedPreferredAssetSwap(ctx, mgr, memo, switchAmt); err != nil {
		...
		if pick.msg.IsStreaming() {
			...
			if swp.Valid() == nil && swp.IsDone() {
				...
				memo, err := ParseMemoWithSWITCHNames(ctx, vm.k, pick.msg.Tx.Memo)
				if err != nil {
					return err
				}
...
func (vm *SwapQueueVCUR) cleanupFailedPreferredAssetSwap(ctx cosmos.Context, mgr Manager, memo string, switchAmt cosmos.Uint) error {
	ctx.Logger().Info("preferred asset swap failed, send switch back to affiliate collector", "switchAmt", switchAmt.String(), "memo", memo)
	// get the preferred asset swap's switchlyname
	name, ok := strings.CutPrefix(memo, fmt.Sprintf("%s-", PreferredAssetSwapMemoPrefix))
	if !ok {
		return fmt.Errorf("failed to get switchlyname from memo: %s", memo)
	}
	if tn, err := vm.k.GetSWITCHName(ctx, name); err == nil {
		affCol, err := mgr.Keeper().GetAffiliateCollector(ctx, tn.Owner)
		if err != nil {
			return fmt.Errorf("failed to get affiliate collector record: %w", err)
		} else {
			affCol.SwitchAmount = affCol.SwitchAmount.Add(switchAmt)
			mgr.Keeper().SetAffiliateCollector(ctx, affCol)
		}
	} else {
		return fmt.Errorf("failed to get switchlyname: %w", err)
	}

	// send switch back to affiliate collector
	if err := mgr.Keeper().SendFromModuleToModule(ctx, AsgardName, AffiliateCollectorName, common.NewCoins(common.NewCoin(common.SwitchNative, switchAmt))); err != nil {
		return fmt.Errorf("failed to send switch back to affiliate collector: %w", err)
	}
...
func (vm *SwapQueueVCUR) scoreMsgs(ctx cosmos.Context, items swapItems, synthVirtualDepthMult int64) (swapItems, error) {
	...
	for i, item := range items {
		...
		nonSWITCHAsset := sourceAsset
		if nonSWITCHAsset.IsSwitch() {
			nonSWITCHAsset = targetAsset
		}
		pool := pools[nonSWITCHAsset]
		...
		if nonSWITCHAsset.IsSyntheticAsset() {
		...
		// double swap , thus need to convert source coin to SWITCH and calculate fee and slip again
		switchCoin := common.NewCoin(common.SwitchNative, pool.AssetValueInSWITCH(item.msg.Tx.Coins[0].Amount))
		nonSWITCHAsset = targetAsset
		pool = pools[nonSWITCHAsset]
		...
		vm.getLiquidityFeeAndSlip(ctx, pool, switchCoin, &items[i], virtualDepthMult)
	}

	return items, nil
}

// getLiquidityFeeAndSlip calculate liquidity fee and slip, fee is in SWITCH
func (vm *SwapQueueVCUR) getLiquidityFeeAndSlip(ctx cosmos.Context, pool Pool, sourceCoin common.Coin, item *swapItem, virtualDepthMult int64) {
	...
	if sourceCoin.IsSwitch() {
		fee = pool.AssetValueInSWITCH(fee)
	}
