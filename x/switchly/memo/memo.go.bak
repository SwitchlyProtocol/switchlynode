...
var stringToTxTypeMap = map[string]TxType{
	...
	"pool+":       TxSwitchPoolDeposit,
	"pool-":       TxSwitchPoolWithdraw,
	"x":           TxExec,
	"exec":        TxExec,
	"switch":      TxSwitch,
	"tcy":         TxSWCYClaim,
...
func StringToTxType(s string) (TxType, error) {
	// SWITCHLYNode can support Abbreviated MEMOs , usually it is only one character
...
type Memo interface {
	...
	GetAffiliateSWITCHName() *types.SWITCHName
...
func (m MemoBase) GetAffiliateSWITCHName() *types.SWITCHName { return nil }
...
func ParseMemoWithSWITCHNames(ctx cosmos.Context, keeper keeper.Keeper, memo string) (mem Memo, err error) {
	defer func() {
		if r := recover(); r != nil {
			mem = EmptyMemo
			err = fmt.Errorf("panicked parsing memo(%s), err: %s", memo, r)
...
func FetchAddress(ctx cosmos.Context, keeper keeper.Keeper, name string, chain common.Chain) (common.Address, error) {
	...
	if keeper.SWITCHNameExists(ctx, parts[0]) {
		var switchlyname types.SWITCHName
		switchlyname, err = keeper.GetSWITCHName(ctx, parts[0])
		if err != nil {
			return common.NoAddress, err
		}
		return switchlyname.GetAlias(chain), nil
	}
