...
type SwapMemo struct {
	...
	AffiliateSWITCHName   *types.SWITCHName // TODO: remove on hardfork
...
func (m SwapMemo) GetAffiliateSWITCHName() *types.SWITCHName { return m.AffiliateSWITCHName }
...
func NewSwapMemo(asset common.Asset, dest common.Address, slip cosmos.Uint, affAddr common.Address, affPts cosmos.Uint, dexAgg, dexTargetAddress string, dexTargetLimit cosmos.Uint, swapType types.SwapType, quan, interval uint64, tn types.SWITCHName, refundAddress common.Address, affiliates []string, affiliatesFeeBps []cosmos.Uint) SwapMemo {
	...
	if !tn.Owner.Empty() {
		swapMemo.AffiliateSWITCHName = &tn
	}
	return swapMemo
}
...
func (p *parser) ParseSwapMemoV3_0_0() (SwapMemo, error) {
	...
	// Parse multiple affiliate switchlynames + fee bps
	...
	if len(affiliates) != len(affFeeBps) {
		return SwapMemo{}, fmt.Errorf("affiliate switchlynames and affiliate fee bps count mismatch")
	}
	...
	// Set affiliate switchlyname when there is only one, this is used in handler_swap to
	// update affiliate collector record (affiliate fee swaps will only have one
	// affiliate)
	tn := p.getSWITCHName(4, false, types.NewSWITCHName("", 0, nil), -1)
...
func ParseSwapMemoV1(ctx cosmos.Context, keeper keeper.Keeper, asset common.Asset, parts []string) (SwapMemo, error) {
	...
	return NewSwapMemo(asset, destination, slip, affAddr, cosmos.NewUint(affPts), "", "", cosmos.ZeroUint(), swapType, 0, 0, types.NewSWITCHName("", 0, nil), "", nil, nil), nil
}
