...
func (p *parser) parse() (mem Memo, err error) {
	...
	case TxSwitchPoolDeposit:
		return p.ParseSwitchPoolDepositMemo()
	case TxSwitchPoolWithdraw:
		return p.ParseSwitchPoolWithdrawMemo()
	...
	case TxSWITCHName:
		return p.ParseManageSWITCHNameMemo()
	case TxLoanOpen:
	...
	case TxSWCYClaim:
		return p.ParseSWCYClaimMemo()
	case TxSWCYStake:
		return p.ParseSWCYStakeMemo()
	case TxSWCYUnstake:
		return p.ParseSWCYUnstakeMemo()
	case TxMaint:
...
func (p *parser) getSWITCHName(idx int, required bool, def types.SWITCHName, subIndex int) types.SWITCHName {
	p.incRequired(required)
	name := p.get(idx)
	if subIndex >= 0 {
	...
	if p.keeper.SWITCHNameExists(p.ctx, name) {
		tn, err := p.keeper.GetSWITCHName(p.ctx, name)
		if err != nil {
			if required || p.get(idx) != "" {
				p.addErr(fmt.Errorf("cannot parse '%s' as a SWITCHName: %w", p.get(idx), err))
			}
		}
		return tn
	}
