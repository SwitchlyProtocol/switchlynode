...
func (s *HandlerLoanSuite) TestLoanValidate(c *C) {
	...
	// reduce the supply of switch
	bal := mgr.Keeper().GetSWITCHBalanceOfModule(ctx, ModuleName)
	...
	mgr.Keeper().SetMimir(ctx, "MaxSWITCHSupply", int64(max.Uint64()))
	...
	msg = NewMsgLoanOpen(owner, common.ETHAsset, cosmos.NewUint(100), GetRandomSwitchAddress(), common.DOGEAsset, cosmos.ZeroUint(), common.NoAddress, cosmos.ZeroUint(), "", "", cosmos.ZeroUint(), GetRandomBech32Addr(), txid)
	c.Assert(handler.validate(ctx, *msg), NotNil)
}
...
func (s *HandlerLoanSuite) TestLoanOpenHandleToSWITCHLY(c *C) {
	ctx, mgr := setupManagerForTest(c)
	ctx = ctx.WithBlockHeight(128)
	mockTxOut := MockTxOutDummy{
		blockOut: NewTxOut(ctx.BlockHeight()),
	...
	// happy path
	txid, _ := common.NewTxID("29FC8D032CF17380AA1DC86F85A479CA9433E85887A9317C5D70D87EF56EAFAA")
	receiver := GetRandomSWITCHLYAddress()
	signer, _ := cosmos.AccAddressFromBech32("tswitch1jrcpkdj6xd7cqv5yqkxq7qkzq48cun4m6w9dgk")
	msg := NewMsgLoanOpen(owner, common.BTCAsset, cosmos.NewUint(1e8), receiver, common.SWITCHLY, cosmos.ZeroUint(), common.NoAddress, cosmos.ZeroUint(), "", "", cosmos.ZeroUint(), signer, txid)
	c.Assert(handler.handle(ctx.WithValue(constants.CtxLoanTxID, txid), *msg), IsNil)
	c.Assert(mgr.SwapQ().EndBlock(ctx, mgr), IsNil)
	...
	c.Check(outs[0].Coin.Asset.Equals(common.SWITCHLY), Equals, true)
