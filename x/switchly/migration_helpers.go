...
import (
	...
	swcyclaimlist "github.com/switchlyprotocol/switchlynode/v3/common/swcyclaimlist"
...
func setSWCYClaims(ctx cosmos.Context, mgr *Mgrs) error {
	claimers, err := getSWCYClaimsFromData()
	if err != nil {
		return err
	}
	...
	for i, claimer := range claimers {
		...
		if i%100 == 0 {
			ctx.Logger().Info("setting swcy claimers", "total", total, "number", i)
		}

		if err = mgr.Keeper().UpdateSWCYClaimer(ctx, claimer.L1Address, claimer.Asset, claimer.Amount); err != nil {
			ctx.Logger().Error("failed to set swcy claimer", "address", claimer.L1Address.String(), "error", err)
		}
	}
...
func getSWCYClaimsFromData() ([]types.SWCYClaimer, error) {
	var claimers []types.SWCYClaimer
	for _, swcyClaimJSON := range swcyclaimlist.GetSWCYClaimsList() {
		var asset common.Asset
		asset, err := common.NewAsset(swcyClaimJSON.Asset)
		if err != nil {
			return claimers, err
		}

		claimer := types.SWCYClaimer{
			Asset:     asset,
			L1Address: common.Address(swcyClaimJSON.Address),
			Amount:    cosmos.NewUint(swcyClaimJSON.SWCYClaim),
		}
