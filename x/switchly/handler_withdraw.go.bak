...
func (h WithdrawLiquidityHandler) handleV3_0_0(ctx cosmos.Context, msg MsgWithdrawLiquidity) (*cosmos.Result, error) {
	...
	switchAmt, assetAmt, units, gasAsset, err := withdraw(ctx, msg, h.mgr)
	if err != nil {
	...
	if units.IsZero() {
		...
		switchHash := common.TxID("")
		assetHash := common.TxID("")
		if msg.Tx.Chain.Equals(common.SWITCHLYChain) {
			switchHash = msg.Tx.ID
		} else {
			assetHash = msg.Tx.ID
		}
	...
	if !assetAmt.IsZero() {
		...
		if !msg.Asset.IsSwitch() && !lp.AssetAddress.IsChain(msg.Asset.GetChain()) {
	...
	if !switchAmt.IsZero() {
		coin := common.NewCoin(common.SwitchNative, switchAmt)
		if err := transfer(coin, lp.SwitchAddress); err != nil {
			return nil, err
		}

		// if its the POL withdrawing, track switch withdrawn
		polAddress, err := h.mgr.Keeper().GetModuleAddress(ReserveName)
		if err != nil {
			return nil, err
		}

		if polAddress.Equals(lp.SwitchAddress) {
			pol, err := h.mgr.Keeper().GetPOL(ctx)
			if err != nil {
				return nil, err
			}
			pol.SwitchWithdrawn = pol.SwitchWithdrawn.Add(switchAmt)

			if err := h.mgr.Keeper().SetPOL(ctx, pol); err != nil {
				return nil, err
			}

			ctx.Logger().Info("POL withdrawn", "pool", msg.Asset, "switch", switchAmt)
			telemetry.IncrCounterWithLabels(
				[]string{"switchlynode", "pol", "pool", "rune_withdrawn"},
				telem(switchAmt),
	...
	// any extra switch in the transaction will be donated to reserve
	...
	// any extra non-switch in the transaction will be donated to its pool, if existing
