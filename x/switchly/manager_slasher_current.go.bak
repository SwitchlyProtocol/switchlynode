...
func (s *SlasherVCUR) LackSigning(ctx cosmos.Context, mgr Manager) error {
	...
	for i, toi := range txs.TxArray {
		...
		if toi.OutHash.IsEmpty() {
			...
			memo, _ := ParseMemoWithSWITCHNames(ctx, s.keeper, toi.Memo) // ignore err
			if memo.IsInternal() {
			...
			if !toi.Coin.IsSwitch() && !s.keeper.PoolExist(ctx, toi.Coin.Asset) {
				ctx.Logger().Error("fail to add outbound to queue", "error", "coin is not switch and does not have an associated pool")
				continue
			}
...
// SlashVault switchly keep monitoring the outbound tx from asgard pool
// usually the txout is triggered by switchly itself by
// adding an item into the txout array, refer to TxOutItem for the detail, the
// TxOutItem contains a specific coin and amount.  if somehow switchly
...
func (s *SlasherVCUR) SlashVault(ctx cosmos.Context, vaultPK common.PubKey, coins common.Coins, mgr Manager) error {
	...
	for _, coin := range coins {
		...
		// stolenSWITCHValue is the value in SWITCH of the missing funds
		stolenSWITCHValue := pool.AssetValueInSWITCH(stolenAssetValue)

		if stolenSWITCHValue.IsZero() {
			continue
		}

		penaltyPts := mgr.Keeper().GetConfigInt64(ctx, constants.SlashPenalty)
		// total slash amount is penaltyPts the SWITCH value of the missing funds
		totalSWITCHToSlash := common.GetUncappedShare(cosmos.NewUint(uint64(penaltyPts)), cosmos.NewUint(10_000), stolenSWITCHValue)
		totalSWITCHSlashed := cosmos.ZeroUint()
		pauseOnSlashThreshold := mgr.Keeper().GetConfigInt64(ctx, constants.PauseOnSlashThreshold)
		if pauseOnSlashThreshold > 0 && totalSWITCHToSlash.GTE(cosmos.NewUint(uint64(pauseOnSlashThreshold))) {
		...
		for _, member := range membership {
			...
			runeSlashed := s.slashAndUpdateNodeAccount(ctx, na, coin, vault, totalBond, totalSWITCHToSlash)
			totalSWITCHSlashed = totalSWITCHSlashed.Add(runeSlashed)
		}

		//  2/3 of the total slashed SWITCH value to asgard
		//  1/3 of the total slashed SWITCH value to reserve
		runeToAsgard := stolenSWITCHValue

		// stolenSWITCHValue is the total value in SWITCH of stolen coins, but totalSWITCHSlashed is
		// the total amount able to be slashed from Nodes, credit the pool only totalSWITCHSlashed
		if totalSWITCHSlashed.LT(stolenSWITCHValue) {
			// this should theoretically never happen
			ctx.Logger().Info("total slashed bond amount is less than SWITCH value", "slashed_bond", totalSWITCHSlashed.String(), "rune_value", stolenSWITCHValue.String())
			runeToAsgard = totalSWITCHSlashed
		}
		runeToReserve := common.SafeSub(totalSWITCHSlashed, runeToAsgard)
...
// portion of the total bond of the offending Vault's membership. Return the amount of SWITCH slashed
func (s SlasherVCUR) slashAndUpdateNodeAccount(ctx cosmos.Context, na types.NodeAccount, coin common.Coin, vault types.Vault, totalBond, totalSlashAmountInSWITCH cosmos.Uint) cosmos.Uint {
	slashAmountSWITCH := common.GetSafeShare(na.Bond, totalBond, totalSlashAmountInSWITCH)
	if slashAmountSWITCH.GT(na.Bond) {
		ctx.Logger().Info("slash amount is larger than bond", "slash amount", slashAmountSWITCH, "bond", na.Bond)
		slashAmountSWITCH = na.Bond
	}

	ctx.Logger().Info("slash node account", "node address", na.NodeAddress.String(), "amount", slashAmountSWITCH.String(), "total slash amount", totalSlashAmountInSWITCH)
	na.Bond = common.SafeSub(na.Bond, slashAmountSWITCH)

	bondEvent := NewEventBond(slashAmountSWITCH, BondCost, common.Tx{}, &na, nil)
	...
	slashAmountSWITCHFloat, _ := new(big.Float).SetInt(slashAmountSWITCH.BigInt()).Float32()
	telemetry.IncrCounterWithLabels(
		[]string{"switchlynode", "bond_slash"},
		slashAmountSWITCHFloat,
		append(
			metricLabels,
			telemetry.NewLabel("address", na.NodeAddress.String()),
