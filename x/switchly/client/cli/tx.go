...
func GetCmdObserveTxIns() *cobra.Command {
	...
	cmd.Flags().String("switchlynode-api", "", "switchlynode api endpoint")
...
func observeTxs(outbound bool) func(cmd *cobra.Command, args []string) error {
	return func(cmd *cobra.Command, args []string) error {
		...
		thorNodeAPI, err := cmd.Flags().GetString("switchlynode-api")
		if err != nil {
			return err
		}
		...
		if txids != "" && thorNodeAPI == "" {
			return fmt.Errorf("--txids requires --switchlynode-api")
		}
...
func findLackingObservation(txid, address, switchlynodeAPI string) (*common.ObservedTx, error) {
	// get tx details from switchlynode API
	url := fmt.Sprintf("%s/switchly/tx/details/%s", switchlynodeAPI, txid)
