...
func (h SwapHandler) validateV3_0_0(ctx cosmos.Context, msg MsgSwap) error {
	...
	memo, _ := ParseMemoWithSWITCHNames(ctx, h.mgr.Keeper(), msg.Tx.Memo)
	...
	if target.IsSyntheticAsset() {
		...
		if len(msg.Tx.Coins) > 0 {
			// calculate switch value on incoming swap, and add to total liquidity.
			switchVal := sourceCoin.Amount
			if !sourceCoin.IsSwitch() {
				pool, err := h.mgr.Keeper().GetPool(ctx, sourceCoin.Asset.GetLayer1Asset())
				if err != nil {
					return ErrInternal(err, "fail to get pool")
				}
				switchVal = pool.AssetValueInSWITCH(sourceCoin.Amount)
			}
			totalLiquiditySWITCH = totalLiquiditySWITCH.Add(switchVal)
		}
		maximumLiquiditySWITCH, err := h.mgr.Keeper().GetMimir(ctx, constants.MaximumLiquiditySWITCH.String())
		if maximumLiquiditySWITCH < 0 || err != nil {
			maximumLiquiditySWITCH = h.mgr.GetConstants().GetInt64Value(constants.MaximumLiquiditySWITCH)
		}
		...
		targetAmount, switchAmount := cosmos.ZeroUint(), cosmos.ZeroUint()
		swapper, err := GetSwapper(h.mgr.GetVersion())
		if err == nil {
			if sourceCoin.IsSwitch() {
				switchAmount = sourceCoin.Amount
			} else {
				// asset --> switch swap
				...
				if err != nil {
					...
					switchAmount = swapper.CalcAssetEmission(sourcePool.BalanceAsset, sourceCoin.Amount, sourcePool.BalanceSwitch)
				}
			}
			// switch --> synth swap
			...
			if err != nil {
				...
				targetAmount = swapper.CalcAssetEmission(targetPool.BalanceSwitch, switchAmount, targetPool.BalanceAsset)
			}
		}
...
func (h SwapHandler) handleV3_0_0(ctx cosmos.Context, msg MsgSwap) (*cosmos.Result, error) {
	...
	var affSwitchlyname *SWITCHName
	var affCol AffiliateFeeCollector

	mem, parseMemoErr := ParseMemoWithSWITCHNames(ctx, h.mgr.Keeper(), msg.Tx.Memo)
	if parseMemoErr == nil {
		affSwitchlyname = mem.GetAffiliateSWITCHName()
	}

	if affSwitchlyname != nil && msg.Destination.Equals(affColAddress) && !msg.AffiliateAddress.IsEmpty() && msg.TargetAsset.IsSwitch() {
		// Add accrued SWITCH for this affiliate
		affCol, err = h.mgr.Keeper().GetAffiliateCollector(ctx, affSwitchlyname.Owner)
		if err != nil {
			ctx.Logger().Error("failed to retrieve AffiliateCollector for switchlyname owner", "address", affSwitchlyname.Owner.String(), "error", err)
		} else {
			...
			if err != nil {
				...
				addSwitchAmt := common.SafeSub(emit, transactionFee)
				affCol.SwitchAmount = affCol.SwitchAmount.Add(addSwitchAmt)
	...
	case TxLoanRepayment:
		...
		msg, err := getMsgLoanRepaymentFromMemo(m, msg.Tx.FromAddress, common.NewCoin(common.SWITCHLY, emit), msg.Signer, msg.Tx.ID)
		if err != nil {
			return nil, err
		}
...
func (h SwapHandler) getTotalLiquiditySWITCH(ctx cosmos.Context) (cosmos.Uint, error) {
