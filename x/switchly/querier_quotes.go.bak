...
func quoteParseAddress(ctx cosmos.Context, mgr *Mgrs, addrString string, chain common.Chain) (common.Address, error) {
	...
	// attempt to lookup a switchlyname address
	name, err := mgr.Keeper().GetSWITCHName(ctx, addrString)
	if err != nil {
	...
	return common.NoAddress, fmt.Errorf("no switchlyname alias for chain %s", chain)
}
...
func quoteHandleAffiliate(ctx cosmos.Context, mgr *Mgrs, affiliateParam, affiliateBpsParam []string, amount sdkmath.Uint) (affiliate common.Address, memo string, bps, newAmount, affiliateAmt sdkmath.Uint, err error) {
	...
	memo = "" // do not resolve switchlyname for the memo
...
func quoteConvertAsset(ctx cosmos.Context, mgr *Mgrs, fromAsset common.Asset, amount sdkmath.Uint, toAsset common.Asset) (sdkmath.Uint, error) {
	...
	if !fromAsset.IsSwitch() {
		...
		amount = fromPool.AssetValueInSWITCH(amount)
	}
	...
	if !toAsset.IsSwitch() {
		...
		amount = toPool.SWITCHValueInAsset(amount)
	}
...
func quoteSimulateSwap(ctx cosmos.Context, mgr *Mgrs, amount sdkmath.Uint, msg *MsgSwap, streamingQuantity uint64) (
	...
	if !msg.TargetAsset.IsSwitch() {
		...
		liquidityFee = targetPool.SWITCHValueInAsset(liquidityFee)
	}
...
// affiliate swap to SWITCH. In this case, we give a 2x buffer on the native_tx_fee so the
...
func calculateMinSwapAmount(ctx cosmos.Context, mgr *Mgrs, fromAsset, toAsset common.Asset, affiliateBps cosmos.Uint) (cosmos.Uint, error) {
	...
	if fromAsset.GetChain().IsSWITCHLYChain() && toAsset.GetChain().IsSWITCHLYChain() {
		// If this is a purely SWITCHLYChain swap, no need to give a 4x buffer since outbound fees do not change
	...
	if affiliateBps.GT(cosmos.ZeroUint()) {
		nativeTxFeeSWITCH, err := mgr.GasMgr().GetAssetOutboundFee(ctx, common.SwitchNative, true)
		if err != nil {
			return cosmos.ZeroUint(), fmt.Errorf("fail to get native tx fee for switch: %w", err)
		}
		affSwapAmountSWITCH := nativeTxFeeSWITCH.Mul(cosmos.NewUint(2))
		mainSwapAmountSWITCH := affSwapAmountSWITCH.Mul(cosmos.NewUint(10_000)).Quo(affiliateBps)

		mainSwapAmount, err := quoteConvertAsset(ctx, mgr, common.SwitchNative, mainSwapAmountSWITCH, fromAsset)
		if err != nil {
...
func (qs queryServer) queryQuoteSwap(ctx cosmos.Context, req *types.QueryQuoteSwapRequest) (*types.QueryQuoteSwapResponse, error) {
	...
	if fromAsset.IsTradeAsset() {
		...
		if err != nil {
			return nil, fmt.Errorf("failed to get switchly address: %w", err)
		}
...
func (qs queryServer) queryQuoteSaverDeposit(ctx cosmos.Context, req *types.QueryQuoteSaverDepositRequest) (*types.QueryQuoteSaverDepositResponse, error) {
	...
	swapMsg := types.QueryQuoteSwapRequest{
		...
		Destination: string(GetRandomSWITCHLYAddress()),
	}
...
func (qs queryServer) queryQuoteLoanOpen(ctx cosmos.Context, req *types.QueryQuoteLoanOpenRequest) (*types.QueryQuoteLoanOpenResponse, error) {
	...
	// Affiliate fee in SWITCH
	affiliateSwitchAmt := sdkmath.ZeroUint()
	...
	if affiliate != common.NoAddress && !affiliateBps.IsZero() {
		...
		_, affiliateSwitchAmt, _, err = quoteSimulateSwap(ctx, qs.mgr, affiliateAmt, affiliateSwap, 1)
		if err == nil {
	...
	// sum liquidity fees in switch from all swap events
	outboundFee := sdkmath.ZeroUint()
	liquidityFee := sdkmath.ZeroUint()
	affiliateFee := affiliateSwitchAmt
	...
	for i := len(events) - 1; i >= 0; i-- {
		...
		// sum liquidity fee in switch for all swap events
		case "swap":
	...
	// convert fees to target asset if it is not switch
	if !targetAsset.Equals(common.SwitchNative) {
		targetPool, err := qs.mgr.Keeper().GetPool(ctx, targetAsset)
		if err != nil {
			return nil, fmt.Errorf("failed to get pool: %w", err)
		}
		affiliateFee = targetPool.SWITCHValueInAsset(affiliateSwitchAmt)
		liquidityFee = targetPool.SWITCHValueInAsset(liquidityFee)
	}
...
func quoteSimulateCloseLoan(ctx cosmos.Context, mgr *Mgrs, msg *MsgLoanRepayment) (
	...
	for i := len(events) - 1; i >= 0; i-- {
		...
		case "swap":
			...
			streamingSwapBlocks += swapQuantity.BigInt().Int64()
			switch {
			case coin.Asset.Equals(common.SWITCHLY):
				repaymentEmit = coin
	...
	// calculate emit values in switch
	torPool, err := mgr.Keeper().GetPool(ctx, common.SWITCHLY)
	if err != nil {
		return nil, fmt.Errorf("failed to get pool: %w", err)
	}
	repaymentEmitSwitch := torPool.SWITCHValueInAsset(repaymentEmit.Amount)
	outPool, err := mgr.Keeper().GetPool(ctx, outboundEmit.Asset)
	if err != nil {
		return nil, fmt.Errorf("failed to get pool: %w", err)
	}
	outboundEmitSwitch := outPool.SWITCHValueInAsset(outboundEmit.Amount)

	// slippage calculation is weighted to repayment and outbound amounts
	outboundSlip := sdkmath.ZeroUint()
	if !outboundEmitSwitch.IsZero() {
		outboundSlip = outboundLiquidityFee.MulUint64(10000).Quo(outboundEmitSwitch.Add(outboundLiquidityFee))
	}
	repaymentSlip := repaymentLiquidityFee.MulUint64(10000).Quo(repaymentEmitSwitch.Add(repaymentLiquidityFee))
	slippageBps := repaymentSlip.Mul(repaymentEmitSwitch).Add(outboundSlip.Mul(outboundEmitSwitch)).Quo(repaymentEmitSwitch.Add(outboundEmitSwitch))
	...
	if !msg.CollateralAsset.Equals(common.SwitchNative) {
		...
		affiliateFee = loanPool.SWITCHValueInAsset(affiliateFee)
		liquidityFee = loanPool.SWITCHValueInAsset(liquidityFee)
	}
...
func (qs queryServer) queryQuoteLoanClose(ctx cosmos.Context, req *types.QueryQuoteLoanCloseRequest) (*types.QueryQuoteLoanCloseResponse, error) {
	...
	poolThorAsset, err := qs.mgr.Keeper().GetPool(ctx, common.SWITCHLY)
	if err != nil {
		return nil, fmt.Errorf("failed to get pool: %w", err)
	}

	pendingDebt := loan.DebtIssued.Sub(loan.DebtRepaid)
	totalPendingDebtInSWITCH := poolThorAsset.AssetValueInSWITCH(pendingDebt)
	totalPendingDebtInRepaymentAsset := totalPendingDebtInSWITCH

	if !asset.IsSwitch() {
		totalPendingDebtInRepaymentAsset = poolRepayment.SWITCHValueInAsset(totalPendingDebtInSWITCH)
	}
	...
	amountInSwitchlyToRepay := pendingDebt.Mul(repayBps).Quo(sdkmath.NewUint(10_000))
	...
	for thresholdBasisPoint.LTE(sdkmath.NewUint(1500)) { // Arbitrary cap for the threshold of 1500 BPS to avoid harmful requests.
		...
		if exptectedDebtRepaid.GTE(amountInSwitchlyToRepay) {
			break
		}
