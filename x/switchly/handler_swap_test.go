...
func (k *TestSwapHandleKeeper) GetModuleAddress(_ string) (common.Address, error) {
	return GetRandomSWITCHLYAddress(), nil
}
...
func (s *HandlerSwapSuite) TestValidation(c *C) {
	...
	msg := NewMsgSwap(tx, common.DOGEAsset.GetSyntheticAsset(), GetRandomSWITCHLYAddress(), cosmos.ZeroUint(), common.NoAddress, cosmos.ZeroUint(), "", "", nil, MarketSwap, 0, 0, observerAddr)
	err := handler.validate(ctx, *msg)
	c.Assert(err, IsNil)
...
func (s *HandlerSwapSuite) TestValidationWithStreamingSwap(c *C) {
	...
	// happy path
	msg := NewMsgSwap(tx, common.DOGEAsset.GetSyntheticAsset(), GetRandomSWITCHLYAddress(), cosmos.ZeroUint(), common.NoAddress, cosmos.ZeroUint(), "", "", nil, MarketSwap, 10, 20, observerAddr)
	err := handler.validate(ctx, *msg)
	c.Assert(err, IsNil)
...
func (s *HandlerSwapSuite) TestHandle(c *C) {
	...
	poolTCAN.BalanceSwitch = cosmos.NewUint(2349500000)
...
func (s *HandlerSwapSuite) TestSwapSynthERC20(c *C) {
	...
	observerAddr, err := GetRandomSWITCHLYAddress().AccAddress()
	c.Assert(err, IsNil)
	msgSwapFromTxIn, err := getMsgSwapFromMemo(m.(SwapMemo), txIn, observerAddr)
	c.Assert(err, IsNil)
...
func (s *HandlerSwapSuite) TestDoubleSwap(c *C) {
	...
	c.Assert(err, IsNil)
	c.Assert(items, HasLen, 1)
	// double swap , SWITCH not enough to pay for transaction fee
...
func (s *HandlerSwapSuite) TestSwapOutDexIntegration(c *C) {
	...
	m, err := ParseMemoWithSWITCHNames(ctx, mgr.Keeper(), swapMemo)
	c.Assert(err, IsNil)

	txIn := NewObservedTx(
		common.NewTx(GetRandomTxHash(), GetRandomSWITCHLYAddress(), GetRandomSWITCHLYAddress(),
			common.Coins{
