...
// QuerySWITCHProviderResponse
// QuerySwitchProvidersResponse
// QuerySaverResponse
// QueryStreamingSwapResponse
// QueryStreamingSwapsResponse
// SwapperClout
// QuerySwapQueueResponse
// QuerySwitchlynameResponse
...
func (s *QuerierSuite) SetUpTest(c *C) {
	...
	_, _, err := kb.NewMnemonic(username, ckeys.English, cmd.SwitchlyHDPath, password, hd.Secp256k1)
	c.Assert(err, IsNil)
...
func (s *QuerierSuite) TestQueryNodeAccounts(c *C) {
	...
	network.BondRewardSwitch = cosmos.NewUint(common.One * 1000)
	c.Assert(keeper.SetNetwork(ctx, network), IsNil)
	keeper.SetMimir(ctx, "MinimumBondInSWITCH", common.One*1000)
...
func (s *QuerierSuite) TestQueryLiquidityProviders(c *C) {
	...
	s.k.SetLiquidityProvider(s.ctx, LiquidityProvider{
		...
		SwitchAddress:      GetRandomETHAddress(),
		AssetAddress:       GetRandomSwitchAddress(),
		LastAddHeight:      1024,
		LastWithdrawHeight: 0,
		Units:              cosmos.NewUint(10),
	})
...
func (s *QuerierSuite) TestQueryNodeAccount(c *C) {
	...
	s.k.SetMimir(s.ctx, "MinimumBondInSWITCH", common.One*1000)
...
func (s *QuerierSuite) TestQueryNetwork(c *C) {
	...
	c.Assert(openapiNetworkResponse.BondRewardSwitch, Equals, queryNetworkResp.BondRewardSwitch)
	c.Assert(openapiNetworkResponse.EffectiveSecurityBond, Equals, queryNetworkResp.EffectiveSecurityBond)
	c.Assert(openapiNetworkResponse.GasSpentSwitch, Equals, queryNetworkResp.GasSpentSwitch)
	c.Assert(openapiNetworkResponse.GasWithheldSwitch, Equals, queryNetworkResp.GasWithheldSwitch)
	c.Assert(openapiNetworkResponse.NativeOutboundFeeSwitch, Equals, queryNetworkResp.NativeOutboundFeeSwitch)
	c.Assert(openapiNetworkResponse.NativeTxFeeSwitch, Equals, queryNetworkResp.NativeTxFeeSwitch)
	c.Assert(*openapiNetworkResponse.OutboundFeeMultiplier, Equals, queryNetworkResp.OutboundFeeMultiplier)
	c.Assert(openapiNetworkResponse.SwitchPriceInSwitchly, Equals, queryNetworkResp.SwitchPriceInSwitchly)
	c.Assert(openapiNetworkResponse.SwitchlynameFeePerBlockSwitch, Equals, queryNetworkResp.SwitchlynameFeePerBlockSwitch)
	c.Assert(openapiNetworkResponse.SwitchlynameRegisterFeeSwitch, Equals, queryNetworkResp.SwitchlynameRegisterFeeSwitch)
	c.Assert(openapiNetworkResponse.SwitchlyPriceInSwitch, Equals, queryNetworkResp.SwitchlyPriceInSwitch)
...
func (s *QuerierSuite) TestQuerySwap(c *C) {
	...
	var addressBTC, addressSWITCHLY string
	var values []string

	if common.CurrentChainNetwork == common.MockNet {
		addressBTC = "bcrt1qg2px54as9vgzaarkr0zy95hacg3lg4kqz4rrwf"
		addressSWITCHLY = "tswitch1em3zz5pamuftha9nafr7rsa8u860pjeycw6g3c"
		values = []string{
		...
		addressSWITCHLY = "thor14j7zjhnazs85macymj4g0xugr8jhdwg07rh2yy"
		values = []string{
	...
	memo := fmt.Sprintf("=:r:%s/%s:97000000000/1/1:foo:50", addressSWITCHLY, addressBTC)
...
func (s *QuerierSuite) TestQueryCodes(c *C) {
	...
	c.Assert(code.Origin, Equals, "https://switchly.org")
...
func (s *QuerierSuite) TestNetwork(c *C) {
	...
	s.k.SetMimir(s.ctx, "SwitchlyAnchor-ETH-BUSD-BD1", 1) // enable BUSD pool as a SWITCHLY anchor
	...
	switchPriceInSwitchly := s.k.DollarsPerSWITCH(s.ctx)
	c.Assert(switchPriceInSwitchly.String(), Equals, "911224600")

	torPriceInSWITCH := s.k.SWITCHPerDollar(s.ctx)
	c.Assert(torPriceInSWITCH.String(), Equals, "10974243")

	resp, err := s.queryServer.Network(s.ctx, &types.QueryNetworkRequest{})
	c.Assert(err, IsNil)
	c.Assert(resp.SwitchlyPriceInSwitch, Equals, torPriceInSWITCH.String())
	c.Assert(resp.SwitchPriceInSwitchly, Equals, switchPriceInSwitchly.String())
	c.Assert(resp.SwitchlyPriceHalted, Equals, false)
	// there is no previous block, so lastSwitchlyHeight is not set

	s.k.SetMimir(s.ctx, "HALTETHTRADING", 1)

	c.Assert(s.k.DollarsPerSWITCH(s.ctx).String(), Equals, "0")
	c.Assert(s.k.SWITCHPerDollar(s.ctx).String(), Equals, "0")

	resp, err = s.queryServer.Network(s.ctx, &types.QueryNetworkRequest{})
	c.Assert(err, IsNil)
	c.Assert(resp.SwitchlyPriceInSwitch, Equals, torPriceInSWITCH.String())
	c.Assert(resp.SwitchPriceInSwitchly, Equals, switchPriceInSwitchly.String())
	c.Assert(resp.SwitchlyPriceHalted, Equals, true)
