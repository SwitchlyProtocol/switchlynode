...
// GasMgrVCUR implement GasManager interface which will store the gas related events happened in switchly to memory
...
// - inSWITCH: whether the fee should be returned in SWITCH. If false the fee is returned in
// asset units.
func (gm *GasMgrVCUR) GetAssetOutboundFee(ctx cosmos.Context, asset common.Asset, inSWITCH bool) (cosmos.Uint, error) {
	switchlyOutboundFee := gm.keeper.GetOutboundTxFee(ctx)

	// If the asset is native SWITCH, return the default native outbound fee.
	if asset.IsSwitch() {
		return switchlyOutboundFee, nil
	}

	// Asset is on SwitchlyProtocol, but not SWITCH, convert the fee to asset value.
	if asset.IsNative() {
		if inSWITCH {
			return switchlyOutboundFee, nil
		}
		return gm.getSWITCHInAssetValue(ctx, switchlyOutboundFee, asset), nil
	}
	...
	switchUSDPrice := gm.keeper.DollarsPerSWITCH(ctx)
	minAsset := cosmos.ZeroUint()
	if !switchUSDPrice.IsZero() {
		// since MinOutboundUSD is in USD value , thus need to figure out how much SWITCH
		// here use GetShare instead GetSafeShare it is because minOutboundUSD can set to more than $1
		minOutboundInSWITCH := common.GetUncappedShare(cosmos.NewUint(uint64(minOutboundUSD)),
			switchUSDPrice,
			cosmos.NewUint(common.One))

		minAsset = gasPool.SWITCHValueInAsset(minOutboundInSWITCH)
	}

	outboundFeeWithheldSwitch, err := gm.keeper.GetOutboundFeeWithheldSwitch(ctx, asset)
	if err != nil {
		ctx.Logger().Error("fail to get outbound fee withheld switch", "outbound asset", asset, "error", err)
		outboundFeeWithheldSwitch = cosmos.ZeroUint()
	}
	outboundFeeSpentSwitch, err := gm.keeper.GetOutboundFeeSpentSwitch(ctx, asset)
	if err != nil {
		ctx.Logger().Error("fail to get outbound fee spent switch", "outbound asset", asset, "error", err)
		outboundFeeSpentSwitch = cosmos.ZeroUint()
	}

	targetOutboundFeeSurplus := gm.keeper.GetConfigInt64(ctx, constants.TargetOutboundFeeSurplusSWITCH)
	...
	feeMultiplierBps := gm.CalcOutboundFeeMultiplier(ctx, cosmos.NewUint(uint64(targetOutboundFeeSurplus)), outboundFeeSpentSwitch, outboundFeeWithheldSwitch, cosmos.NewUint(uint64(maxMultiplierBasisPoints)), cosmos.NewUint(uint64(minMultiplierBasisPoints)))
	...
	// If feeAsset = gas asset && inSWITCH = false, we are in the correct units, return
	if asset.Equals(asset.GetChain().GetGasAsset()) && !inSWITCH {
		return fee, nil
	}

	if gasPool.BalanceAsset.IsZero() || gasPool.BalanceSwitch.IsZero() {
		ctx.Logger().Error("fail to calculate fee as gas pool balance is zero, returning 0 fee", "pool", gasPool.Asset.String(), "switch", gasPool.BalanceSwitch.String(), "asset", gasPool.BalanceAsset.String())
		return cosmos.ZeroUint(), nil
	}

	// Convert gas asset fee to switch, if inSWITCH = true, return
	fee = gasPool.AssetValueInSWITCH(fee)
	if inSWITCH {
		return fee, nil
	}

	// convert switch value into non-gas asset value
	assetPool, err := gm.keeper.GetPool(ctx, asset)
	if err != nil {
		return cosmos.ZeroUint(), err
	}
	if assetPool.BalanceAsset.IsZero() || assetPool.BalanceSwitch.IsZero() {
		ctx.Logger().Error("fail to calculate fee as asset pool balance is zero, returning 0 fee", "pool", assetPool.Asset.String(), "switch", assetPool.BalanceSwitch.String(), "asset", assetPool.BalanceAsset.String())
		return cosmos.ZeroUint(), nil
	}

	return assetPool.SWITCHValueInAsset(fee), nil
}

// CalcOutboundFeeMultiplier returns the current outbound fee multiplier based on current and target outbound fee surplus
func (gm *GasMgrVCUR) CalcOutboundFeeMultiplier(ctx cosmos.Context, targetSurplusSwitch, gasSpentSWITCH, gasWithheldSWITCH, maxMultiplier, minMultiplier cosmos.Uint) cosmos.Uint {
	// Sanity check
	if targetSurplusSwitch.Equal(cosmos.ZeroUint()) {
	...
	surplusSWITCH := common.SafeSub(gasWithheldSWITCH, gasSpentSWITCH)

	// How many BPs to reduce the multiplier
	multiplierReducedBps := common.GetSafeShare(surplusSWITCH, targetSurplusSwitch, common.SafeSub(maxMultiplier, minMultiplier))
	return common.SafeSub(maxMultiplier, multiplierReducedBps)
}

// getSWITCHInAssetValue convert the transaction fee to asset value , when the given asset is synthetic , it will need to get
// the layer1 asset first , and then use the pool to convert
func (gm *GasMgrVCUR) getSWITCHInAssetValue(ctx cosmos.Context, transactionFee cosmos.Uint, asset common.Asset) cosmos.Uint {
	...
	return pool.SWITCHValueInAsset(transactionFee)
}
...
// ProcessGas to subsidise the gas asset pools with SWITCH for the gas they have spent
func (gm *GasMgrVCUR) ProcessGas(ctx cosmos.Context, keeper keeper.Keeper) {
	...
	reserveSWITCH := keeper.GetSWITCHBalanceOfModule(ctx, ReserveName)
	poolCache := map[common.Asset]Pool{}
	for i := range gm.outAssetGas {
		feeSpentSWITCH := cosmos.ZeroUint()
		for _, coin := range gm.outAssetGas[i].gas {
			...
			// TODO:  Use SWITCHReimbursementForAssetWithdrawal and, within this range, do cached pool updates before a single later SetPool?
			// Currently this uses a constant AssetValueInSWITCH ratio without ensuring a constant depths-product,
			// as a result of which asset-associated reimbursement order does not matter.
			switchGas := pool.AssetValueInSWITCH(coin.Amount) // Convert to SWITCH (gas will never be SWITCH)
			if switchGas.IsZero() {
				continue
			}
			// Keep track of whether the Reserve SWITCH will be enough for all reimbursements.
			reserveSWITCH = common.SafeSub(reserveSWITCH, switchGas)
			if reserveSWITCH.IsZero() {
				// since we don't have enough in the reserve to cover the gas used,
				// no further switch is added to gas pools, sorry LPs!
				switchGas = cosmos.ZeroUint()
			}
			...
			feeSpentSWITCH = feeSpentSWITCH.Add(switchGas)
		}
		// Add SWITCH spent on gas by the reserve
		if err := keeper.AddToOutboundFeeSpentSwitch(ctx, gm.outAssetGas[i].outAsset, feeSpentSWITCH); err != nil {
			ctx.Logger().Error("fail to add to outbound fee spent switch", "outbound asset", gm.outAssetGas[i].outAsset, "error", err)
		}
	}
	...
	for i := range gm.gasEvent.Pools {
		...
		if !gm.gasEvent.Pools[i].SwitchAmt.IsZero() {
			coin := common.NewCoin(common.SwitchNative, gm.gasEvent.Pools[i].SwitchAmt)
			if err := keeper.SendFromModuleToModule(ctx, ReserveName, AsgardName, common.NewCoins(coin)); err != nil {
				...
				pool.BalanceSwitch = pool.BalanceSwitch.Add(gm.gasEvent.Pools[i].SwitchAmt)
			}
		}
