...
func getWithdrawTestKeeper2(c *C, ctx cosmos.Context, k keeper.Keeper, switchAddress common.Address) keeper.Keeper {
...
func (s WithdrawSuite) TestValidateWithdraw(c *C) {
	...
	switchAddress, err := common.NewAddress("0x90f2b1ae50e6018230e90a33f98c7844a0ab635a")
	if err != nil {
...
func (s WithdrawSuite) TestCalculateUnsake(c *C) {
	inputs := []struct {
		...
		expectedWithdrawSWITCH  cosmos.Uint
	...
	}{
		{
			...
			expectedWithdrawSWITCH:  cosmos.ZeroUint(),
...

		{
			...
			expectedWithdrawAsset: cosmos.ZeroUint(),
			expectedUnitLeft:      cosmos.ZeroUint(),
			expectedErr:           errors.New("pool switch balance can't be zero"),
		},
		...
		},
		{
			...
			poolSWITCH:              cosmos.NewUint(700 * common.One),
			...
			expectedWithdrawSWITCH:  cosmos.NewUint(200 * common.One),
			expectedErr:           nil,
		},
		{
			name:                  "withdraw-2",
			poolUnit:              cosmos.NewUint(100),
			poolSWITCH:              cosmos.NewUint(15 * common.One),
			...
			expectedWithdrawSWITCH:  cosmos.NewUint(150000000),
			expectedErr:           nil,
		},
	}
	...
	for _, item := range inputs {
		c.Logf("name:%s", item.name)
		withDrawSWITCH, withDrawAsset, unitAfter, err := calculateWithdraw(ctx, keeper, common.EmptyAsset, item.poolUnit, item.poolSWITCH, item.poolAsset, item.lpUnit, item.percentage, common.EmptyAsset, common.NoAddress)
		...
		c.Logf("expected switch:%s,switch:%s", item.expectedWithdrawSWITCH, withDrawSWITCH)
		c.Check(item.expectedWithdrawSWITCH.Uint64(), Equals, withDrawSWITCH.Uint64(), Commentf("Expected %d, got %d", item.expectedWithdrawSWITCH.Uint64(), withDrawSWITCH.Uint64()))
...
func (WithdrawSuite) TestWithdraw(c *C) {
	...
	ps2 := getWithdrawTestKeeper(c, ctx, mgr.Keeper(), switchAddress)
	...
	testCases := []struct {
		...
		},
		{
			...
			ps:            ps2,
			switchAmount:    cosmos.NewUint(50 * common.One),
	...
	for _, tc := range testCases {
		...
		if tc.expectedError != nil {
			...
			c.Check(r.Uint64(), Equals, tc.switchAmount.Uint64())
			c.Check(asset.Uint64(), Equals, tc.assetAmount.Uint64())
			continue
		}
		c.Assert(err, IsNil)
		c.Assert(r.Uint64(), Equals, tc.switchAmount.Uint64(), Commentf("%d != %d", r.Uint64(), tc.switchAmount.Uint64()))
...
func (WithdrawSuite) TestWithdrawAsym(c *C) {
	...
	testCases := []struct {
		...
		{
			name: "all-good-asymmetric-switch",
			msg: MsgWithdrawLiquidity{
				WithdrawAddress: switchAddress,
				BasisPoints:     cosmos.NewUint(10000),
				Asset:           common.ETHAsset,
			...
			switchAmount:    cosmos.NewUint(6250000000),
			assetAmount:   cosmos.ZeroUint(),
			expectedError: nil,
		},
		{
	...
	for _, tc := range testCases {
		c.Logf("name:%s", tc.name)
		ctx, mgr := setupManagerForTest(c)
		ps := getWithdrawTestKeeper2(c, ctx, mgr.Keeper(), switchAddress)
		mgr.K = ps
...
func (WithdrawSuite) TestWithdrawPendingSwitchOrAsset(c *C) {
	...
	switchAmt, assetAmt, unitsLeft, gas, err := withdraw(ctx, msg, mgr)
	c.Assert(err, IsNil)
	c.Assert(switchAmt.Equal(cosmos.NewUint(1024)), Equals, true)
	...
	msg1 := MsgWithdrawLiquidity{
		WithdrawAddress: lp1.SwitchAddress,
		BasisPoints:     cosmos.NewUint(10000),
		Asset:           common.ETHAsset,
		Tx:              common.Tx{ID: "28B40BF105A112389A339A64BD1A042E6140DC9082C679586C6CF493A9FDE3FE"},
		WithdrawalAsset: common.ETHAsset,
	...
	switchAmt, assetAmt, unitsLeft, gas, err = withdraw(ctx, msg1, mgr)
	c.Assert(err, IsNil)
	c.Assert(assetAmt.Equal(cosmos.NewUint(1024)), Equals, true)
	c.Assert(switchAmt.IsZero(), Equals, true)
	c.Assert(unitsLeft.IsZero(), Equals, true)
	c.Assert(gas.IsZero(), Equals, true)
}
...
func (s *WithdrawSuite) TestWithdrawPendingLiquidityShouldRoundToPoolDecimals(c *C) {
	...
	lpAddr := GetRandomSWITCHLYAddress()
	...
	switchAmt, assetAmt, unitsClaimed, _, err := withdraw(newctx, msg2, mgr)
	c.Assert(err, IsNil)
...
func getWithdrawTestKeeper(c *C, ctx cosmos.Context, k keeper.Keeper, switchAddress common.Address) keeper.Keeper {
	store := NewWithdrawTestKeeper(k)
	pool := Pool{
		BalanceSwitch: cosmos.NewUint(100 * common.One),
		BalanceAsset:  cosmos.NewUint(100 * common.One),
	...
	lp := LiquidityProvider{
		...
		PendingAsset:       cosmos.ZeroUint(),
		PendingTxID:        "",
		SwitchDepositValue:   cosmos.NewUint(100 * common.One),
...
func (WithdrawSuite) TestWithdrawSynth(c *C) {
	...
	lp := LiquidityProvider{
		Asset:              asset,
		SwitchAddress:        common.NoAddress,
	...
	c.Check(switchAmt.IsZero(), Equals, true)
