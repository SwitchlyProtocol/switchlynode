...
func (MsgSwapSuite) TestMsgSwap(c *C) {
	...
	m = NewMsgSwap(tx, common.ETHAsset, GetRandomSWITCHLYAddress(), cosmos.ZeroUint(), common.NoAddress, cosmos.ZeroUint(), "", "", nil, 0, 0, 0, addr)
	c.Assert(m.ValidateBasic(), NotNil)
	m = NewMsgSwap(tx, common.ETHAsset.GetSyntheticAsset(), GetRandomSWITCHLYAddress(), cosmos.ZeroUint(), common.NoAddress, cosmos.ZeroUint(), "", "", nil, 0, 0, 0, addr)
	c.Assert(m.ValidateBasic(), IsNil)
	...
	m = NewMsgSwap(tx, common.ETHAsset, GetRandomETHAddress(), cosmos.ZeroUint(), GetRandomSWITCHLYAddress(), cosmos.NewUint(1024), "", "", nil, 0, 0, 0, addr)
	c.Assert(m.ValidateBasic(), NotNil)
	...
	for name, tc := range map[string]struct {
		...
		"swap SWITCH for BNB with trade target": {
			source:        common.SwitchNative,
			dest:          common.BNBBEP20Asset,
			destAddr:      bnbAddr,
		...
		"swap BNB for SWITCH with trade target": {
			source:        common.BNBBEP20Asset,
			dest:          common.SwitchNative,
			destAddr:      thorAddr,
			tradeTarget:   cosmos.NewUint(100),
		...
		"swap SWITCH for SWITCH with trade target": {
			source:        common.SwitchNative,
			dest:          common.SwitchNative,
			destAddr:      thorAddr,
			tradeTarget:   cosmos.NewUint(100),
