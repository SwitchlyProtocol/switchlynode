...
func NewMsgAddLiquidity(tx common.Tx, asset common.Asset, r, amount cosmos.Uint, switchAddr, assetAddr, affAddr common.Address, affPts cosmos.Uint, signer cosmos.AccAddress) *MsgAddLiquidity {
...
func (m *MsgAddLiquidity) ValidateBasic() error {
	...
	// There is no dedicate pool for SWITCH, because every pool will have SWITCH, that's by design
	if m.Asset.IsSwitch() {
		return cosmos.ErrUnknownRequest("asset cannot be switch")
	}
	// test scenario we get two coins, but none are switch, invalid liquidity provider
	if len(m.Tx.Coins) == 2 && (m.AssetAmount.IsZero() || m.SwitchAmount.IsZero()) {
	...
	if m.SwitchAddress.IsEmpty() && m.AssetAddress.IsEmpty() {
		return cosmos.ErrUnknownRequest("switch address and asset address cannot be empty")
	}
