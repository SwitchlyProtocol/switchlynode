...
func (MsgTradeAccountSuite) TestWithdrawal(c *C) {
	...
	m = NewMsgTradeAccountWithdrawal(asset, cosmos.ZeroUint(), GetRandomSWITCHLYAddress(), signer, dummyTx)
	c.Check(m.ValidateBasic(), NotNil)
}
...
func (s *MsgTradeAccountSuite) TestMsgTradeAccountWithdraw(c *C) {
	...
	m = NewMsgTradeAccountWithdrawal(tradeAsset, cosmos.ZeroUint(), GetRandomSWITCHLYAddress(), signer, dummyTx)
	c.Check(m.ValidateBasic(), NotNil)
}
