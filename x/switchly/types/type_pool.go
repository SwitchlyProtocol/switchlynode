...
func (m Pool) String() string {
	sb := strings.Builder{}
	sb.WriteString(fmt.Sprintln("switch-balance: " + m.BalanceSwitch.String()))
	...
	sb.WriteString(fmt.Sprintln("pending-inbound-switch: " + m.PendingInboundSwitch.String()))
...
// AssetValueInSWITCH convert a specific amount of asset amt into its switch value
func (m Pool) AssetValueInSWITCH(amt cosmos.Uint) cosmos.Uint {
...
// SWITCHReimbursementForAssetWithdrawal returns the equivalent amount of switch for a
...
// preserved.
func (m Pool) SWITCHReimbursementForAssetWithdrawal(amt cosmos.Uint) cosmos.Uint {
	if m.BalanceSwitch.IsZero() || m.BalanceAsset.IsZero() {
		return cosmos.ZeroUint()
	}
	...
	if denom.IsZero() {
		...
		// the equivalent switch value approaches infinity. Return 0 in the limiting
		// case.
		return cosmos.ZeroUint()
	}
...
// SWITCHValueInAsset convert a specific amount of switch amt into its asset value
func (m Pool) SWITCHValueInAsset(amt cosmos.Uint) cosmos.Uint {
	if m.BalanceSwitch.IsZero() || m.BalanceAsset.IsZero() {
		return cosmos.ZeroUint()
	}
...
// SWITCHDisbursementForAssetAdd returns the equivalent amount of switch for a
...
// preserved.
func (m Pool) SWITCHDisbursementForAssetAdd(amt cosmos.Uint) cosmos.Uint {
	if m.BalanceSwitch.IsZero() || m.BalanceAsset.IsZero() {
		return cosmos.ZeroUint()
	}
...
// AssetDisbursementForSwitchAdd returns the equivalent amount of asset for a
// given amount of switch added to the pool, taking slip into account. When this
// amount is withdrawn from the pool, the constant product of depths rule is
// preserved.
func (m Pool) AssetDisbursementForSwitchAdd(amt cosmos.Uint) cosmos.Uint {
	if m.BalanceSwitch.IsZero() || m.BalanceAsset.IsZero() {
		return cosmos.ZeroUint()
	}
...
func (m Pool) GetLUVI() cosmos.Uint {
	bigInt := &big.Int{}
	balSWITCH := m.BalanceSwitch.MulUint64(1e12).BigInt()
	balAsset := m.BalanceAsset.MulUint64(1e12).BigInt()
	num := bigInt.Mul(balSWITCH, balAsset)
	num = bigInt.Sqrt(num)
