...
func (s *NetworkSuite) TestCalcNodeRewards(c *C) {
	network1 := NewNetwork()
	c.Check(network1.BondRewardSwitch.Uint64(), Equals, cosmos.ZeroUint().Uint64())

	vault := Network{
		TotalBondUnits:   cosmos.NewUint(100),
		BondRewardSwitch: cosmos.NewUint(3000),
	}
	...
	vault = Network{
		...
		BondRewardSwitch: cosmos.NewUint(275.357 * common.One),
	}
	...
	vault = Network{
		TotalBondUnits:   cosmos.NewUint(7357),
		BondRewardSwitch: cosmos.ZeroUint(),
	}
	reward = vault.CalcNodeRewards(cosmos.NewUint(78))
