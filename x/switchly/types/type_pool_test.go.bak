...
func (PoolTestSuite) TestPool(c *C) {
	...
	c.Check(p.AssetValueInSWITCH(cosmos.NewUint(25*common.One)).Equal(cosmos.NewUint(50*common.One)), Equals, true)
	c.Check(p.SWITCHValueInAsset(cosmos.NewUint(50*common.One)).Equal(cosmos.NewUint(25*common.One)), Equals, true)
	...
	c.Check(p1.AssetValueInSWITCH(cosmos.NewUint(100)).Uint64(), Equals, cosmos.ZeroUint().Uint64())
	c.Check(p1.SWITCHValueInAsset(cosmos.NewUint(100)).Uint64(), Equals, cosmos.ZeroUint().Uint64())
	p1.BalanceSwitch = cosmos.NewUint(100 * common.One)
...
func (PoolTestSuite) TestPools(c *C) {
	...
	btc.BalanceSwitch = cosmos.NewUint(10)
	...
	c.Check(pool.BalanceSwitch.Uint64(), Equals, uint64(10))

	pool.BalanceSwitch = cosmos.NewUint(20)
	pools = pools.Set(pool)
	pool, ok = pools.Get(common.BTCAsset)
	c.Check(ok, Equals, true)
	c.Check(pool.Asset.Equals(common.BTCAsset), Equals, true)
	c.Check(pool.BalanceSwitch.Uint64(), Equals, uint64(20))
}
...
func (PoolTestSuite) TestCalcUnits(c *C) {
	...
	pool.BalanceSwitch = cosmos.NewUint(100)
	pool.LPUnits = cosmos.NewUint(100)
	...
	pool.BalanceSwitch = cosmos.NewUint(1_000)
	synthSupply = cosmos.NewUint(100)
	units = pool.CalcUnits(synthSupply)
...
func (PoolTestSuite) TestReimbursementAndDisbursement(c *C) {
	...
	p.BalanceSwitch = cosmos.NewUint(100 * common.One)
	p.BalanceAsset = cosmos.NewUint(50 * common.One)
	c.Check(p.SWITCHDisbursementForAssetAdd(cosmos.NewUint(150*common.One)).Equal(cosmos.NewUint(75*common.One)), Equals, true)
	c.Check(p.AssetDisbursementForSwitchAdd(cosmos.NewUint(900*common.One)).Equal(cosmos.NewUint(45*common.One)), Equals, true)

	p.BalanceSwitch = cosmos.NewUint(0)
	c.Check(p.SWITCHDisbursementForAssetAdd(cosmos.NewUint(1*common.One)).Equal(cosmos.NewUint(0*common.One)), Equals, true)
	c.Check(p.AssetDisbursementForSwitchAdd(cosmos.NewUint(1*common.One)).Equal(cosmos.NewUint(0*common.One)), Equals, true)
...
func (PoolTestSuite) TestLUVI(c *C) {
	p := NewPool()
	p.BalanceSwitch = cosmos.NewUint(100)
