...
func HasSuperMajority(signers, total int) bool {
	if signers > total {
		return false // will not have majority if SWITCHLYNode have more signers than node accounts. This shouldn't be possible
	}
	if signers <= 0 {
...
func HasMinority(signers, total int) bool {
	if signers > total {
		return false // if SWITCHLYNode have more signers than node accounts. This shouldn't be possible
	}
	if signers <= 0 {
		return false // edge case
	}
