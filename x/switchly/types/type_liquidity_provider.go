...
func (m *LiquidityProvider) Valid() error {
	...
	if m.AssetAddress.IsEmpty() && m.SwitchAddress.IsEmpty() {
		return errors.New("asset address and switch address cannot be empty")
	}
	return nil
}
...
func (lp LiquidityProvider) GetSwitchRedeemValue(pool Pool, synthSupply cosmos.Uint) (error, cosmos.Uint) {
	...
	poolSWITCHDepth := pool.BalanceSwitch.BigInt()
	num := bigInt.Mul(lpUnits, poolSWITCHDepth)
...
func (lp LiquidityProvider) GetLuviDepositValue(pool Pool) (error, cosmos.Uint) {
	...
	bigInt := &big.Int{}
	runeDeposit := lp.SwitchDepositValue.MulUint64(1e8).BigInt()
...
func (lp LiquidityProvider) GetLuviRedeemValue(switchRedeemValue, assetRedeemValue cosmos.Uint) (error, cosmos.Uint) {
	bigInt := &big.Int{}
	switchValue := switchRedeemValue.MulUint64(1e8).BigInt()
	assetValue := assetRedeemValue.MulUint64(1e8).BigInt()
	num := bigInt.Mul(switchValue, assetValue)
	num = bigInt.Sqrt(num)
	denom := lp.Units.BigInt()
	if len(denom.Bits()) == 0 {
		return nil, cosmos.ZeroUint()
