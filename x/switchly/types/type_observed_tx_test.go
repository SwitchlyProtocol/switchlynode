...
func (s TypeObservedTxSuite) TestVoter(c *C) {
	...
	voter.Add(obTx1, acc1) // check SWITCHLYNode don't duplicate the same signer
	c.Assert(voter.Txs, HasLen, 1)
	...
	voter.Add(obTx1Finalised, acc1) // check SWITCHLYNode don't duplicate the same signer
	c.Assert(voter.Txs, HasLen, 3)
