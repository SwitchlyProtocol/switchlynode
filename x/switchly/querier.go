...
import (
	...
	"github.com/switchlyprotocol/switchlynode/v3/common/wasmpermissions"
...
func (qs queryServer) querySWITCHName(ctx cosmos.Context, req *types.QuerySwitchlynameRequest) (*types.QuerySwitchlynameResponse, error) {
	name, err := qs.mgr.Keeper().GetSWITCHName(ctx, req.Name)
	if err != nil {
		return nil, ErrInternal(err, "fail to fetch SWITCHName")
	}

	affSWITCH := cosmos.ZeroUint()
	affCol, err := qs.mgr.Keeper().GetAffiliateCollector(ctx, name.Owner)
	if err == nil {
		affSWITCH = affCol.SwitchAmount
	}

	// convert to openapi types
	aliases := []*types.SwitchlynameAlias{}
	for _, alias := range name.Aliases {
		aliases = append(aliases, &types.SwitchlynameAlias{
	...
	resp := types.QuerySwitchlynameResponse{
		Name:                              name.Name,
		ExpireBlockHeight:                 name.ExpireBlockHeight,
		Owner:                             name.Owner.String(),
		PreferredAsset:                    name.PreferredAsset.String(),
		Aliases:                           aliases,
		AffiliateCollectorSwitch:          affSWITCH.String(),
		PreferredAssetSwapThresholdSwitch: threshold.String(),
	}
...
func (qs queryServer) querySwitchPool(ctx cosmos.Context, _ *types.QuerySwitchPoolRequest) (*types.QuerySwitchPoolResponse, error) {
	// gather pol data
	...
	// gather switchpool data
	switchPool, err := qs.mgr.Keeper().GetSwitchPool(ctx)
	if err != nil {
		return nil, fmt.Errorf("fail to get SWITCH pool: %w", err)
	}

	// calculate pending units
	switchPoolValue, err := switchPoolValue(ctx, qs.mgr)
	if err != nil {
		return nil, fmt.Errorf("fail to get switch pool value: %w", err)
	}
	pendingSWITCH := qs.mgr.Keeper().GetSWITCHBalanceOfModule(ctx, SwitchPoolName)
	pendingUnits := common.GetSafeShare(pendingSWITCH, switchPoolValue, switchPool.TotalUnits())

	// calculate provider shares
	providerValue := common.GetSafeShare(switchPool.PoolUnits, switchPool.TotalUnits(), switchPoolValue)
	providerPnl := sdkmath.NewIntFromBigInt(providerValue.BigInt()).Sub(switchPool.CurrentDeposit())

	// calculate reserve shares
	reserveValue := common.GetSafeShare(switchPool.ReserveUnits, switchPool.TotalUnits(), switchPoolValue)
	...
	result := types.QuerySwitchPoolResponse{
		Pol: &types.POL{
			SwitchDeposited: pol.SwitchDeposited.String(),
			SwitchWithdrawn: pol.SwitchWithdrawn.String(),
			Value:           polValue.String(),
			Pnl:             pnl.String(),
			CurrentDeposit:  pol.CurrentDeposit().String(),
		},
		Providers: &types.SwitchPoolProviders{
			Units:          switchPool.PoolUnits.String(),
			PendingUnits:   pendingUnits.String(),
			PendingSwitch:  pendingSWITCH.String(),
			Value:          providerValue.String(),
			Pnl:            providerPnl.String(),
			CurrentDeposit: switchPool.CurrentDeposit().String(),
		},
		Reserve: &types.SwitchPoolReserve{
			Units:          switchPool.ReserveUnits.String(),
...
// querySWITCHProvider
func (qs queryServer) querySWITCHProvider(ctx cosmos.Context, req *types.QuerySWITCHProviderRequest) (*types.QuerySWITCHProviderResponse, error) {
	...
	rp, err := qs.mgr.Keeper().GetSWITCHProvider(ctx, addr)
	if err != nil {
		return nil, fmt.Errorf("unable to GetSWITCHProvider: %s", err)
	}

	// get switchpool value to determine current value and pnl
	switchPool, err := qs.mgr.Keeper().GetSwitchPool(ctx)
	if err != nil {
		return nil, fmt.Errorf("fail to get SWITCH pool: %w", err)
	}
	...
	providerValue := common.GetSafeShare(rp.Units, switchPool.TotalUnits(), switchPoolValue)
	...
	result := types.QuerySWITCHProviderResponse{
		SwitchAddress:      rp.SwitchAddress.String(),
...
// querySwitchProviders
func (qs queryServer) querySwitchProviders(ctx cosmos.Context, _ *types.QuerySWITCHProvidersRequest) (*types.QuerySWITCHProvidersResponse, error) {
	// get switchpool value to determine current value and pnl
	switchPool, err := qs.mgr.Keeper().GetSwitchPool(ctx)
	if err != nil {
		return nil, fmt.Errorf("fail to get SWITCH pool: %w", err)
	...
	var switchProviders []*types.QuerySWITCHProviderResponse
	iterator := qs.mgr.Keeper().GetSWITCHProviderIterator(ctx)
	defer iterator.Close()
	for ; iterator.Valid(); iterator.Next() {
		var rp types.SWITCHProvider
		...
		switchProviders = append(switchProviders, &types.QuerySWITCHProviderResponse{
			SwitchAddress:      rp.SwitchAddress.String(),
			Units:              rp.Units.String(),
			Value:              providerValue.String(),
			Pnl:                providerPnl.String(),
	...
	return &types.QuerySWITCHProvidersResponse{Providers: switchProviders}, nil
}
...
func (qs queryServer) queryNetwork(ctx cosmos.Context, _ *types.QueryNetworkRequest) (*types.QueryNetworkResponse, error) {
	...
	_, availablePoolsSWITCH, err := getAvailablePoolsSWITCH(ctx, qs.mgr.Keeper())
	if err != nil {
		return nil, fmt.Errorf("fail to get available pools switch: %w", err)
	}
	vaultsLiquiditySWITCH, err := getVaultsLiquiditySWITCH(ctx, qs.mgr.Keeper())
	if err != nil {
		return nil, fmt.Errorf("fail to get vaults liquidity switch: %w", err)
	}
	effectiveSecurityBond := getEffectiveSecurityBond(nodeAccounts)

	targetOutboundFeeSurplus := qs.mgr.Keeper().GetConfigInt64(ctx, constants.TargetOutboundFeeSurplusSWITCH)
	...
	outboundFeeMultiplier := qs.mgr.gasMgr.CalcOutboundFeeMultiplier(ctx, cosmos.NewUint(uint64(targetOutboundFeeSurplus)), cosmos.NewUint(data.OutboundGasSpentSwitch), cosmos.NewUint(data.OutboundGasWithheldSwitch), cosmos.NewUint(uint64(maxMultiplierBasisPoints)), cosmos.NewUint(uint64(minMultiplierBasisPoints)))

	assets := qs.mgr.Keeper().GetAnchors(ctx, common.SWITCHLY)
	...
	result := types.QueryNetworkResponse{
		...
		BondRewardSwitch:              data.BondRewardSwitch.String(),
		TotalBondUnits:                data.TotalBondUnits.String(),
		AvailablePoolsSwitch:          availablePoolsSWITCH.String(),
		VaultsLiquiditySwitch:         vaultsLiquiditySWITCH.String(),
		EffectiveSecurityBond:         effectiveSecurityBond.String(),
		TotalReserve:                  qs.mgr.Keeper().GetSWITCHBalanceOfModule(ctx, ReserveName).String(),
		VaultsMigrating:               vaultsMigrating,
		GasSpentSwitch:                cosmos.NewUint(data.OutboundGasSpentSwitch).String(),
		GasWithheldSwitch:             cosmos.NewUint(data.OutboundGasWithheldSwitch).String(),
		OutboundFeeMultiplier:         outboundFeeMultiplier.String(),
		NativeTxFeeSwitch:             qs.mgr.Keeper().GetNativeTxFee(ctx).String(),
		NativeOutboundFeeSwitch:       qs.mgr.Keeper().GetOutboundTxFee(ctx).String(),
		SwitchlynameRegisterFeeSwitch: qs.mgr.Keeper().GetSWITCHNameRegisterFee(ctx).String(),
		SwitchlynameFeePerBlockSwitch: qs.mgr.Keeper().GetSWITCHNamePerBlockFee(ctx).String(),
		SwitchPriceInSwitchly:         switchPerDollarIgnoreHalt(ctx, qs.mgr.Keeper()).String(),
		SwitchlyPriceInSwitch:         dollarsPerSWITCHIgnoreHalt(ctx, qs.mgr.Keeper()).String(),
		SwitchlyPriceHalted:           median.IsZero(),
	}
...
func (qs queryServer) queryInboundAddresses(ctx cosmos.Context, _ *types.QueryInboundAddressesRequest) (*types.QueryInboundAddressesResponse, error) {
	...
	for _, chain := range chains {
		// tx send to switchly doesn't need an address , thus here skip it
		...
		// because SWITCHLYNode is using 1e8, while GWei in ETH is in 1e9, thus the minimum SWITCHLYNode can represent is 10Gwei
...
func (qs queryServer) queryNode(ctx cosmos.Context, req *types.QueryNodeRequest) (*types.QueryNodeResponse, error) {
	...
	if nodeAcc.Status == NodeActive && !nodeAcc.Bond.IsZero() {
		...
		reward, err := getNodeCurrentRewards(ctx, qs.mgr, nodeAcc, lastChurnHeight, network.BondRewardSwitch, totalEffectiveBond, bondHardCap)
		if err != nil {
...
func (qs queryServer) queryNodes(ctx cosmos.Context, _ *types.QueryNodesRequest) (*types.QueryNodesResponse, error) {
	...
	for i, na := range nodeAccounts {
		...
		if na.Status == NodeActive {
			reward, err := getNodeCurrentRewards(ctx, qs.mgr, na, lastChurnHeight, network.BondRewardSwitch, totalEffectiveBond, bondHardCap)
			if err != nil {
				return nil, fmt.Errorf("fail to get current node rewards: %w", err)
			}
...
func (qs queryServer) queryLiquidityProviders(ctx cosmos.Context, req *types.QueryLiquidityProvidersRequest) (*types.QueryLiquidityProvidersResponse, error) {
	...
	for ; iterator.Valid(); iterator.Next() {
		...
		lps = append(lps, &types.QueryLiquidityProviderResponse{
			...
			Asset:              lp.Asset.GetLayer1Asset().String(),
			SwitchAddress:      lp.SwitchAddress.String(),
			AssetAddress:       lp.AssetAddress.String(),
			LastAddHeight:      lp.LastAddHeight,
			LastWithdrawHeight: lp.LastWithdrawHeight,
			Units:              lp.Units.String(),
			PendingSwitch:      lp.PendingSwitch.String(),
			PendingAsset:       lp.PendingAsset.String(),
			PendingTxId:        lp.PendingTxID.String(),
			SwitchDepositValue: lp.SwitchDepositValue.String(),
			AssetDepositValue:  lp.AssetDepositValue.String(),
		})
	}
...
func (qs queryServer) queryLiquidityProvider(ctx cosmos.Context, req *types.QueryLiquidityProviderRequest) (*types.QueryLiquidityProviderResponse, error) {
	...
	_, switchRedeemValue := lp.GetSwitchRedeemValue(pool, synthSupply)
	_, assetRedeemValue := lp.GetAssetRedeemValue(pool, synthSupply)
	_, luviDepositValue := lp.GetLuviDepositValue(pool)
	_, luviRedeemValue := lp.GetLuviRedeemValue(switchRedeemValue, assetRedeemValue)
	...
	liqp := types.QueryLiquidityProviderResponse{
		...
		SwitchDepositValue: lp.SwitchDepositValue.String(),
		AssetDepositValue:  lp.AssetDepositValue.String(),
		SwitchRedeemValue:  switchRedeemValue.String(),
...
func (qs queryServer) queryPool(ctx cosmos.Context, req *types.QueryPoolRequest) (*types.QueryPoolResponse, error) {
	...
	// from negative MaxSWITCHSupply, so dropping the errors for both
	...
	switchDepth, _, _ := qs.mgr.NetworkMgr().CalcAnchor(ctx, qs.mgr, asset)
	dpool, _ := qs.mgr.Keeper().GetPool(ctx, asset.GetDerivedAsset())
	dbps := common.GetUncappedShare(dpool.BalanceSwitch, switchDepth, cosmos.NewUint(constants.MaxBasisPts))
	...
	p := types.QueryPoolResponse{
		...
		PendingInboundSwitch: pool.PendingInboundSwitch.String(),
		BalanceAsset:         pool.BalanceAsset.String(),
		BalanceSwitch:        pool.BalanceSwitch.String(),
	...
	if !pool.BalanceAsset.IsZero() && !pool.BalanceSwitch.IsZero() {
		dollarsPerSWITCH := dollarsPerSWITCHIgnoreHalt(ctx, qs.mgr.Keeper())
		p.AssetSwitchPrice = dollarsPerSWITCH.Mul(pool.BalanceSwitch).Quo(pool.BalanceAsset).String()
	}
...
func (qs queryServer) queryPools(ctx cosmos.Context, _ *types.QueryPoolsRequest) (*types.QueryPoolsResponse, error) {
	...
	for ; iterator.Valid(); iterator.Next() {
		...
		// Ignore derived assets (except SWITCHLY)
		...
		switchDepth, _, _ := qs.mgr.NetworkMgr().CalcAnchor(ctx, qs.mgr, pool.Asset)
...
func (qs queryServer) queryDerivedPool(ctx cosmos.Context, req *types.QueryDerivedPoolRequest) (*types.QueryDerivedPoolResponse, error) {
	...
	// sum switch depth of anchor pools
	switchDepth := sdkmath.ZeroUint()
	for _, anchor := range qs.mgr.Keeper().GetAnchors(ctx, asset) {
		aPool, _ := qs.mgr.Keeper().GetPool(ctx, anchor)
		switchDepth = switchDepth.Add(aPool.BalanceSwitch)
	}
	...
	p := types.QueryDerivedPoolResponse{
		...
		BalanceSwitch: dpool.BalanceSwitch.String(),
	}
	p.DerivedDepthBps = dbps.String()
...
func checkPending(ctx cosmos.Context, keeper keeper.Keeper, voter ObservedTxVoter) (isSwap, isPending, pending bool, streamingSwap StreamingSwap) {
	...
	memo, err := ParseMemoWithSWITCHNames(ctx, keeper, voter.Tx.Tx.Memo)
	if err != nil {
...
func (qs queryServer) queryTxStages(ctx cosmos.Context, req *types.QueryTxStagesRequest) (*types.QueryTxStagesResponse, error) {
	...
	// when no TxIn voter don't check TxOut voter, as TxOut SWITCHLYChain observation or not matters little to the user once signed and broadcast
	...
	for height := startHeight; height <= ctx.BlockHeight(); height++ {
		...
		for _, tx := range txs.TxArray {
			if tx.OutHash.IsEmpty() {
				memo, _ := ParseMemoWithSWITCHNames(ctx, qs.mgr.Keeper(), tx.Memo)
				if memo.IsInternal() {
					query.Internal++
...
func (qs queryServer) queryMimirNodeValues(ctx cosmos.Context, req *types.QueryMimirNodeValuesRequest) (*types.QueryMimirNodeValuesResponse, error) {
	...
	if err != nil {
		ctx.Logger().Error("fail to parse switchly address", "error", err)
		return nil, fmt.Errorf("fail to parse switchly address: %w", err)
	}
...
func (qs queryServer) queryOutboundFees(ctx cosmos.Context, asset string) (*types.QueryOutboundFeesResponse, error) {
	...
	if asset != "" {
		...
		// (even if for instance a Derived Asset to show its SWITCHLYChain outbound fee).
		...
		// By default display the outbound fees of SWITCH and all external-chain Layer 1 assets.
		...
		for ; iterator.Valid(); iterator.Next() {
			...
			if pool.Asset.IsNative() {
				...
				// of SwitchlyChain Assets other than SWITCH.
				continue
			}
			if pool.BalanceAsset.IsZero() || pool.BalanceSwitch.IsZero() {
	...
	targetSurplusSwitch := cosmos.NewUint(uint64(qs.mgr.Keeper().GetConfigInt64(ctx, constants.TargetOutboundFeeSurplusSWITCH)))
	...
	// Due to the nature of pool iteration by key, this is expected to have SWITCH at the top and then be in alphabetical order.
	...
	for i := range assets {
		...
		var outboundFeeWithheldSwitchString, outboundFeeSpentSwitchString, surplusSWITCHString, dynamicMultiplierBasisPointsString string
		if !assets[i].IsNative() {
			outboundFeeWithheldSwitch, err := qs.mgr.Keeper().GetOutboundFeeWithheldSwitch(ctx, assets[i])
			if err != nil {
				ctx.Logger().Error("fail to get outbound fee withheld switch", "outbound asset", assets[i], "error", err)
				return nil, fmt.Errorf("fail to get outbound fee withheld switch for asset (%s): %w", assets[i], err)
			}
			outboundFeeWithheldSwitchString = outboundFeeWithheldSwitch.String()

			outboundFeeSpentSwitch, err := qs.mgr.Keeper().GetOutboundFeeSpentSwitch(ctx, assets[i])
			if err != nil {
				ctx.Logger().Error("fail to get outbound fee spent switch", "outbound asset", assets[i], "error", err)
				return nil, fmt.Errorf("fail to get outbound fee spent switch for asset (%s): %w", assets[i], err)
			}
			outboundFeeSpentSwitchString = outboundFeeSpentSwitch.String()

			surplusSWITCHString = common.SafeSub(outboundFeeWithheldSwitch, outboundFeeSpentSwitch).String()

			dynamicMultiplierBasisPointsString = qs.mgr.GasMgr().CalcOutboundFeeMultiplier(ctx, targetSurplusSwitch, outboundFeeSpentSwitch, outboundFeeWithheldSwitch, maxMultiplier, minMultiplier).String()
		}
		...
		result = append(result, &types.QueryOutboundFeeResponse{
			...
			OutboundFee:                  outboundFee.String(),
			FeeWithheldSwitch:            outboundFeeWithheldSwitchString,
			FeeSpentSwitch:               outboundFeeSpentSwitchString,
			SurplusSwitch:                surplusSWITCHString,
...
// switchPerDollarIgnoreHalt mirrors keeper.SWITCHPerDollar, ignoring halts by using
// dollarsPerSWITCHIgnoreHalt to return the last known price instead of "0"
func switchPerDollarIgnoreHalt(ctx cosmos.Context, k keeper.Keeper) cosmos.Uint {
	switchPerDollar := dollarsPerSWITCHIgnoreHalt(ctx, k)

	one := cosmos.NewUint(common.One)

	return common.GetUncappedShare(one, switchPerDollar, one)
}

// dollarsPerSWITCHIgnoreHalt mirrors keeper.DollarsPerSWITCH, but ignoring halts if all
// anchor chains are unavailable with them. This is used for the SWITCHLY price on pools to
// ensure a best effort price is returned whenever possible instead of zero.
func dollarsPerSWITCHIgnoreHalt(ctx cosmos.Context, k keeper.Keeper) cosmos.Uint {
	// check for mimir override
	dollarsPerSWITCH, err := k.GetMimir(ctx, "DollarsPerSWITCH")
	if err == nil && dollarsPerSWITCH > 0 {
		return cosmos.NewUint(uint64(dollarsPerSWITCH))
	}

	usdAssets := k.GetAnchors(ctx, common.SWITCHLY)
	...
	for _, asset := range usdAssets {
		...
		// value := common.GetUncappedShare(pool.BalanceAsset, pool.BalanceSwitch, cosmos.NewUint(common.One))
		value := pool.SWITCHValueInAsset(cosmos.NewUint(constants.DollarMulti * common.One))
...
// querySWCYStakers
func (qs queryServer) querySWCYStakers(ctx cosmos.Context, req *types.QuerySWCYStakersRequest) (*types.QuerySWCYStakersResponse, error) {
	var stakers []*types.QuerySWCYStakerResponse
	swcyStakers, err := qs.mgr.Keeper().ListSWCYStakers(ctx)
	if err != nil {
		return &types.QuerySWCYStakersResponse{}, err
	}
	for _, staker := range swcyStakers {
		stakers = append(stakers, &types.QuerySWCYStakerResponse{
			Address: staker.Address.String(),
			Amount:  staker.Amount.String(),
		})
	}
	return &types.QuerySWCYStakersResponse{SwcyStakers: stakers}, nil
}

// querySWCYStaker
func (qs queryServer) querySWCYStaker(ctx cosmos.Context, req *types.QuerySWCYStakerRequest) (*types.QuerySWCYStakerResponse, error) {
	addr, err := common.NewAddress(req.Address)
	if err != nil {
		ctx.Logger().Error("fail to get parse address", "error", err)
		return nil, fmt.Errorf("fail to parse address: %w", err)
	}
	staker, err := qs.mgr.Keeper().GetSWCYStaker(ctx, addr)
	if err != nil {
		ctx.Logger().Error("fail to get swcy staker", "error", err)
		return nil, fmt.Errorf("fail to swcy staker: %w", err)
	}

	stakerRes := types.QuerySWCYStakerResponse{
		Address: staker.Address.String(),
		Amount:  staker.Amount.String(),
	}
...
// querySWCYClaimers
func (qs queryServer) querySWCYClaimers(ctx cosmos.Context, req *types.QuerySWCYClaimersRequest) (*types.QuerySWCYClaimersResponse, error) {
	var claimers []*types.QuerySWCYClaimer
	iterator := qs.mgr.Keeper().GetSWCYClaimerIterator(ctx)
	defer iterator.Close()
	for ; iterator.Valid(); iterator.Next() {
		var claimer SWCYClaimer
		qs.mgr.Keeper().Cdc().MustUnmarshal(iterator.Value(), &claimer)

		claimers = append(claimers, &types.QuerySWCYClaimer{
			Asset:     claimer.Asset.String(),
			L1Address: claimer.L1Address.String(),
			Amount:    claimer.Amount.String(),
		})
	}
	return &types.QuerySWCYClaimersResponse{SwcyClaimers: claimers}, nil
}

// querySWCYClaimer
func (qs queryServer) querySWCYClaimer(ctx cosmos.Context, req *types.QuerySWCYClaimerRequest) (*types.QuerySWCYClaimerResponse, error) {
	addr, err := common.NewAddress(req.Address)
	if err != nil {
		ctx.Logger().Error("fail to get parse address", "error", err)
		return nil, fmt.Errorf("fail to parse address: %w", err)
	}
	addressClaims, err := qs.mgr.Keeper().ListSWCYClaimersFromL1Address(ctx, addr)
	if err != nil {
		ctx.Logger().Error("fail to get swcy claimer", "error", err)
		return nil, fmt.Errorf("fail to swcy claimer: %w", err)
	}

	var claimsRes []*types.QuerySWCYClaimer
	for _, claim := range addressClaims {
		claimsRes = append(claimsRes, &types.QuerySWCYClaimer{
	...
	return &types.QuerySWCYClaimerResponse{SwcyClaimer: claimsRes}, nil
}
