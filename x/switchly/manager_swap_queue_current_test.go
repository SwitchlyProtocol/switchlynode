...
func (s SwapQueueVCURSuite) TestScoreMsgs(c *C) {
	...
	msgs := []*MsgSwap{
		...
		}, common.SwitchNative, GetRandomGAIAAddress(), cosmos.ZeroUint(), common.NoAddress, cosmos.ZeroUint(),
			"", "", nil,
			MarketSwap,
			0, 0, GetRandomBech32Addr()),
		NewMsgSwap(common.Tx{
		...
		}, common.SwitchNative, GetRandomSWITCHLYAddress(), cosmos.ZeroUint(), common.NoAddress, cosmos.ZeroUint(),
			"", "", nil,
			MarketSwap,
			0, 0, GetRandomBech32Addr()),
	...
	// 50 ATOM is worth more than 100 SWITCH
...
func (s SwapQueueVCURSuite) TestStreamingSwapSelection(c *C) {
	...
	tx := common.NewTx(
		...
		ethAddr,
		ethAddr,
		common.NewCoins(common.NewCoin(common.SwitchNative, cosmos.NewUint(common.One*100))),
		common.Gas{common.NewCoin(common.ETHAsset, cosmos.NewUint(common.One))},
		"",
	)

	// happy path
	msg := NewMsgSwap(tx, common.ETHAsset.GetSyntheticAsset(), GetRandomSWITCHLYAddress(), cosmos.ZeroUint(), common.NoAddress, cosmos.ZeroUint(), "", "", nil, MarketSwap, 10, 20, GetRandomBech32Addr())
