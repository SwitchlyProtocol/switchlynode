...
func processErrataTxAttestation(
	...
	memo, _ := ParseMemoWithSWITCHNames(ctx, k, tx.Memo)
	...
	if switchCoin.Amount.GT(pool.BalanceSwitch) {
		switchCoin.Amount = pool.BalanceSwitch
	}
	if assetCoin.Amount.GT(pool.BalanceAsset) {
		assetCoin.Amount = pool.BalanceAsset
	}
	pool.BalanceSwitch = common.SafeSub(pool.BalanceSwitch, switchCoin.Amount)
	...
	// send errata event
	mods := PoolMods{
		NewPoolMod(pool.Asset, switchCoin.Amount, false, assetCoin.Amount, false),
	}
...
func processErrataOutboundTx(ctx cosmos.Context, k keeper.Keeper, eventMgr EventManager, er *common.ErrataTx) error {
	...
	m, err := ParseMemoWithSWITCHNames(ctx, k, tx.Memo)
	if err != nil {
	...
	if !vaultPubKey.IsEmpty() {
		...
		if compensate {
			for _, coin := range tx.Coins {
				if coin.IsSwitch() {
					// it is using native switch, so outbound can't be SWITCH
					continue
				}
				// trunk-ignore(golangci-lint/govet): shadow
				...
				switchValue := p.AssetValueInSWITCH(coin.Amount)
				p.BalanceSwitch = p.BalanceSwitch.Add(switchValue)
				...
				if err := k.SendFromModuleToModule(ctx, ReserveName, AsgardName, common.Coins{
					common.NewCoin(common.SwitchNative, switchValue),
				}); err != nil {
				...
				// send errata event
				mods := PoolMods{
					NewPoolMod(p.Asset, switchValue, true, coin.Amount, false),
				}
