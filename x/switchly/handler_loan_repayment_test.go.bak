...
func (s *HandlerLoanRepaymentSuite) TestLoanValidate(c *C) {
	...
	// happy path
	msg := NewMsgLoanRepayment(owner, common.ETHAsset, cosmos.OneUint(), owner, common.NewCoin(common.SWITCHLY, cosmos.NewUint(10*common.One)), signer, txid)
...
func (s *HandlerLoanRepaymentSuite) TestLoanRepaymentHandleWithSWITCHLY(c *C) {
	ctx, mgr := setupManagerForTest(c)
	...
	// mint SWITCHLY to burn later
	coin := common.NewCoin(common.SWITCHLY, cosmos.NewUint(1000*common.One))
	...
	msg := NewMsgLoanRepayment(owner, common.BTCAsset, cosmos.OneUint(), owner, common.NewCoin(common.SWITCHLY, cosmos.NewUint(10*common.One)), signer, txid)
...
func (s *HandlerLoanRepaymentSuite) TestLoanRepaymentHandleWithSwap(c *C) {
	...
	c.Assert(mgr.SwapQ().EndBlock(ctx, mgr), IsNil) // swap into SWITCHLY
