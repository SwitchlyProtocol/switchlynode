...
func (MigrationHelpersTestSuite) TestGetSWCYClaimsFromData(c *C) {
	claimer, err := getSWCYClaimsFromData()
	c.Assert(err, IsNil)
...
func (MigrationHelpersTestSuite) TestSetSWCYClaims(c *C) {
	ctx, mgr := setupManagerForTest(c)

	address, err := common.NewAddress("0x00112c24ebee9c96d177a3aa2ff55dcb93a53c80")
	c.Assert(err, IsNil)
	_, err = mgr.Keeper().GetSWCYClaimer(ctx, address, common.AVAXAsset)
	c.Assert(err.Error(), Equals, fmt.Sprintf("SWCYClaimer doesn't exist: %s", address.String()))

	address2, err := common.NewAddress("bc1qj937cw6xap470hg427p2kxl85u3ac3ps84hye3")
	c.Assert(err, IsNil)
	_, err = mgr.Keeper().GetSWCYClaimer(ctx, address2, common.BTCAsset)
	c.Assert(err.Error(), Equals, fmt.Sprintf("SWCYClaimer doesn't exist: %s", address2.String()))

	address3, err := common.NewAddress("tswitch1v0982t4mwv64gytjesmfa2mkxsmz24j7lm93su")
	c.Assert(err, IsNil)
	_, err = mgr.Keeper().GetSWCYClaimer(ctx, address3, common.DOGEAsset)
	c.Assert(err.Error(), Equals, fmt.Sprintf("SWCYClaimer doesn't exist: %s", address3.String()))

	c.Assert(setSWCYClaims(ctx, mgr), IsNil)

	claimer, err := mgr.Keeper().GetSWCYClaimer(ctx, address, common.AVAXAsset)
	c.Assert(err, IsNil)
	...
	claimer, err = mgr.Keeper().GetSWCYClaimer(ctx, address2, common.BTCAsset)
	c.Assert(err, IsNil)
	...
	claimer, err = mgr.Keeper().GetSWCYClaimer(ctx, address3, common.DOGEAsset)
	c.Assert(err, IsNil)
