...
// validateMessage is trying to validate the legitimacy of the incoming message and decide whether SWITCHLYNode can handle it
...
func (s *SwapperVCUR) Swap(ctx cosmos.Context,
	...
	for _, evt := range swapEvents {
		...
		telemetry.IncrCounterWithLabels(
			[]string{"switchlynode", "swap", "count"},
			float32(1),
			[]metrics.Label{telemetry.NewLabel("pool", evt.Pool.String())},
		)
		telemetry.IncrCounterWithLabels(
			[]string{"switchlynode", "swap", "slip"},
			telem(evt.SwapSlip),
			[]metrics.Label{telemetry.NewLabel("pool", evt.Pool.String())},
		)
		telemetry.IncrCounterWithLabels(
			[]string{"switchlynode", "swap", "liquidity_fee"},
			telem(evt.LiquidityFeeInSwitch),
			[]metrics.Label{telemetry.NewLabel("pool", evt.Pool.String())},
		)
	}
...
func (s *SwapperVCUR) swapOne(ctx cosmos.Context,
	...
	// the non-SWITCH asset is used to determine which min slip to use
	...
	// do SWITCHLYNode have enough balance to swap?
	if emitAssets.GTE(Y) {
		return cosmos.ZeroUint(), evt, errSwapFailNotEnoughBalance
	}

	ctx.Logger().Info("pre swap", "pool", pool.Asset, "switch", pool.BalanceSwitch, "asset", pool.BalanceAsset, "lp units", pool.LPUnits, "synth units", pool.SynthUnits)

	// Burning of input synth or derived pool input (Asset or SWITCH).
	...
	// Minting of output synth or derived pool output (Asset or SWITCH).
	if (target.IsSyntheticAsset() || pool.Asset.IsDerivedAsset()) &&
		!emitAssets.IsZero() {
		// If the source isn't SWITCH, the target should be SWITCH.
	...
	// Only alter BalanceAsset when the non-SWITCH asset isn't a synth.
	if source.IsSwitch() {
	...
	// Now that pool depths have been adjusted to post-swap, determine LiquidityFeeInSwitch.
	if target.IsSwitch() {
		// Because the output asset is SWITCH, liquidity Fee is already in SWITCH.
		swapEvt.LiquidityFeeInSwitch = swapEvt.LiquidityFee
	} else {
		// Momentarily deduct the liquidity fee for SWITCHDisbursementForAssetAdd.
		pool.BalanceAsset = common.SafeSub(pool.BalanceAsset, swapEvt.LiquidityFee)
		swapEvt.LiquidityFeeInSwitch = pool.SWITCHDisbursementForAssetAdd(swapEvt.LiquidityFee)
		// Restore the BalanceAsset which is constant-depths-product swapped for the LiquidityFeeInSwitch.
		pool.BalanceAsset = pool.BalanceAsset.Add(swapEvt.LiquidityFee)
	}

	if !pool.Asset.IsDerivedAsset() {
		// Deduct LiquidityFeeInSwitch from the pool's SWITCH depth and send it to the Reserve Module to be system income.
		// (So that the liquidity fee isn't used for later swaps in the same block.)
		if !swapEvt.LiquidityFeeInSwitch.IsZero() {
			pool.BalanceSwitch = common.SafeSub(pool.BalanceSwitch, swapEvt.LiquidityFeeInSwitch)
			liqFeeCoin := common.NewCoin(common.SwitchNative, swapEvt.LiquidityFeeInSwitch)

			targetModule := ReserveName
			if pool.Asset.IsSWCY() {
				targetModule = SWCYClaimingName
			}

			if err := mgr.Keeper().SendFromModuleToModule(ctx, AsgardName, targetModule, common.NewCoins(liqFeeCoin)); err != nil {
				ctx.Logger().Error("fail to move liquidity fee SWITCH during swap", "module", targetModule, "error", err)
				return cosmos.ZeroUint(), evt, err
			}

			// Only add to liquidity fees if not SWCY
			if !pool.Asset.IsSWCY() {
				if err := keeper.AddToLiquidityFees(ctx, pool.Asset, swapEvt.LiquidityFeeInSwitch); err != nil {
	...
	ctx.Logger().Info("post swap", "pool", pool.Asset, "switch", pool.BalanceSwitch, "asset", pool.BalanceAsset, "lp units", pool.LPUnits, "synth units", pool.SynthUnits, "emit asset", emitAssets)
	...
	// apply swapper clout
	availableClout := swapEvt.LiquidityFeeInSwitch
