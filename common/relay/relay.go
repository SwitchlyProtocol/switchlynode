...
func (n *NodeRelay) fetchUUID() error {
	// GET UUID PREFIX
	resp, err := http.Get(fmt.Sprintf("%s/uuid_prefix", config.GetSwitchlynode().NodeRelayURL))
	if err != nil {
		return err
	}
...
func (n *NodeRelay) Broadcast() (string, error) {
	...
	resp, err := http.Post(fmt.Sprintf("%s/msg", config.GetSwitchlynode().NodeRelayURL), "application/json", responseBody)
	// Handle Error
	if err != nil {
		return "", err
	}
