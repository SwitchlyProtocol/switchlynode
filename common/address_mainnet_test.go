...
func (s *AddressSuite) TestAddress(c *C) {
	...
	// switchly tests
	addr, err := NewAddress("swtc1nd50908twkglcdw0cpx6zgnhw96k02fwyvdrsk")
	c.Assert(err, IsNil)
	...
	addr, err = NewAddress("tswtc1nd50908twkglcdw0cpx6zgnhw96k02fwqmunfn")
	c.Assert(err, IsNil)
	c.Check(addr.IsChain(SWITCHLYChain), Equals, true)
	c.Check(addr.IsChain(GAIAChain), Equals, false)
	c.Check(addr.IsChain(ETHChain), Equals, false)
...
func (s *AddressSuite) TestAddress_Mainnet(c *C) {
	...
	addr, err = NewAddress("sswtc1nd50908twkglcdw0cpx6zgnhw96k02fws434xg")
	c.Assert(err, IsNil)
	c.Check(addr.IsChain(SWITCHLYChain), Equals, true)
	c.Check(addr.GetNetwork(SWITCHLYChain), Equals, StageNet)

	addr, err = NewAddress("sswtcvaloper1p49kgdgytmuhl3hjgjk7pnu03tkhg3slm6l0xp")
	c.Assert(err, IsNil)
	c.Check(addr.IsChain(SWITCHLYChain), Equals, true)
	c.Check(addr.GetNetwork(SWITCHLYChain), Equals, StageNet)
}
