...
func (s *AddressSuite) TestAddress(c *C) {
	...
	// thor tests
	var addr Address
	addr, err = NewAddress("switch1le5fx58yn23rtuvq3t0ujna0vcw0tcreyca4ax")
	c.Assert(err, IsNil)
	...
	addr, err = NewAddress("tswitch1770gl4x5u7aauzlc7su7j3kl6jgduz2wck29yl")
	c.Assert(err, IsNil)
	c.Check(addr.IsChain(SWITCHLYChain), Equals, true)
	c.Check(addr.IsChain(GAIAChain), Equals, false)
	c.Check(addr.IsChain(ETHChain), Equals, false)
	...
	// mocknet p2pk compressed (0x02), UTXO SignTx unable to sign for (not a Switchly-supported format)
	_, err = NewAddress("02192d74d0cb94344c9569c2e77901573d8d7903c3ebec3a957724895dca52c6b4")
	c.Check(err, NotNil)
